 ...
 */
public class LoadRequest<T extends Key, V extends Value> extends Packet<T> {
  private V val;
  ...
  public LoadRequest() {
    super();
    setType(PacketType.LOADREQ.getType());
    this.val = (V) new Value();
    this.clientLogicalIdx = 0;
    this.fragseq = 0;
  }

  public LoadRequest(int methodid, T key, V value, short clientLogicalIdx, int fragseq) {
    super(methodid, PacketType.LOADREQ, key);
    this.val = value;
    this.clientLogicalIdx = clientLogicalIdx;
    this.fragseq = fragseq;
  }

  public LoadRequest(int methodid, byte[] data, int recvSize) {
    super();
    this.methodid = methodid;
    this.deserialize(data, recvSize);
    MyUtils.myAssert(type() == PacketType.LOADREQ.getType());
    // NOTE: val in LOADREQ can be any size
