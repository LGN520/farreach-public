 ...
 */
public class GetResponseLargeValue<T extends Key, V extends Value> extends Packet<T> {
  private V val;
  ...
  public GetResponseLargeValue() {
    super();
    setType(PacketType.GETRES_LARGEVALUE.getType());
  ...
  public GetResponseLargeValue(int methodid, T key, V val, boolean stat, short nodeIdxForEval) {
    super(methodid, PacketType.GETRES_LARGEVALUE, key);
    this.val = val;
    this.stat = stat;
    this.nodeIdxForEval = nodeIdxForEval;

    if (this.val.getValData().length <= Value.getSwitchMaxValLen()) {
      System.out.println(String.format("[GetResponseLargeValue] Value length too short: %d", this.val.getValData().length));
      System.exit(-1);
    }
  }

  public GetResponseLargeValue(int methodid, byte[] data, int recvSize) {
    super();
    this.methodid = methodid;
    this.val = (V) new Value();
    this.deserialize(data, recvSize);
    if (this.type != PacketType.GETRES_LARGEVALUE.getType()) {
      System.out.println(String.format("[GetResponseLargeValue] Wrong converted packet type 0x%x!", this.type));
      System.exit(-1);
    }
  ...
  public int size() {
    ...
    int size = getOphdrsize(this.methodid) + SIZEOF_UINT32 + Value.getSwitchMaxValLen() +
        SIZEOF_UINT8 + SIZEOF_UINT16 + getStatPaddingBytes(this.methodid);
