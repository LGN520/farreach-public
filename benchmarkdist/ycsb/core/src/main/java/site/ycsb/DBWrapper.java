...
public class DBWrapper extends DB {
  ...
  public void cleanup() throws DBException {
    try (final TraceScope span = tracer.newScope(scopeStringCleanup)) {
      ...
      measure("CLEANUP", null, false, Status.OK, ist, st, en);
    }
  }
  ...
  public Status read(String table, String key, Set<String> fields,
    ...
    try (final TraceScope span = tracer.newScope(scopeStringRead)) {
      ...
      if (res == Status.CACHE_HIT) {
        res = Status.OK;
        measure("READ", key, true, res, ist, st, en);
      } else {
        measure("READ", key, false, res, ist, st, en);
      }
  ...
  public Status scan(String table, String startkey, int recordcount,
    ...
    try (final TraceScope span = tracer.newScope(scopeStringScan)) {
      ...
      long en = System.nanoTime();
      measure("SCAN", startkey, false, res, ist, st, en);
      measurements.reportStatus("SCAN", res);
      return res;
    }
  }

  private void measure(String op, String key, boolean ishit, Status result, long intendedStartTimeNanos,
    ...
    measurements.measure(measurementName, key, ishit,
  ...
  public Status update(String table, String key,
    ...
    try (final TraceScope span = tracer.newScope(scopeStringUpdate)) {
      ...
      if (res == Status.CACHE_HIT) {
        res = Status.OK;
        measure("UPDATE", key, true, res, ist, st, en);
      } else {
        measure("UPDATE", key, false, res, ist, st, en);
      }
  ...
  public Status insert(String table, String key,
    ...
    try (final TraceScope span = tracer.newScope(scopeStringInsert)) {
      ...
      if (res == Status.CACHE_HIT) {
        res = Status.OK;
        measure("INSERT", key, true, res, ist, st, en);
      } else {
        measure("INSERT", key, false, res, ist, st, en);
      }
  ...
  public Status delete(String table, String key) {
    try (final TraceScope span = tracer.newScope(scopeStringDelete)) {
      ...
      if (res == Status.CACHE_HIT) {
        res = Status.OK;
        measure("DELETE", key, true, res, ist, st, en);
      } else {
        measure("DELETE", key, false, res, ist, st, en);
      }
