...
public final class Client {
  ...
  /**
   * An optional thread used to track inswitch cache client.
   */
  private static Thread watchInSwitchCacheThread = null;
  ...
  public static boolean checkRequiredProperties(Properties props) {
    ...
    boolean isInswitchCache = GlobalConfig.isInswitchCacheDatabase(dbname);
    if (!isInswitchCache && props.getProperty(WORKLOAD_PROPERTY) == null) {
  ...
  public static void main(String[] args) {
    ...
    if (target > 0) {
      GlobalConfig.setTestingLatency();
    }

    // In-switch cache initialization
    if (GlobalConfig.isInswitchCacheDatabase(dbname)) {
      Properties inSwitchCacheProps = new Properties();
      inSwitchCacheProps.put(InswitchCacheClient.PHYSICAL_CLIENT_INDEX,
          props.getProperty(InswitchCacheClient.PHYSICAL_CLIENT_INDEX, "-1"));
      inSwitchCacheProps.put(InswitchCacheClient.SINGLE_ROTATION,
          props.getProperty(InswitchCacheClient.SINGLE_ROTATION, "0"));
      // inSwitchCacheProps.put(InswitchCacheClient.WORKLOAD_DUMPING_FILE,
      // props.getProperty(InswitchCacheClient.WORKLOAD_DUMPING_FILE, ""));
      InswitchCacheClient.initBenchmark(dbname, inSwitchCacheProps);

      // overwrite YCSB parameters
      // (1) threadcount
      threadcount = GlobalConfig.getCurrentClientLogicalNum();
      System.out.println(String.format("[INFO][Client] threadcount: %d", threadcount));
      // (2) workload-related properties
      if (GlobalConfig.isStaticPattern()) { // server rotation for static pattern
        props.setProperty(WORKLOAD_PROPERTY, "site.ycsb.workloads.PregeneratedWorkload");
        System.out.println(String.format("[INFO][Client] use PregeneratedWorkload for server rotation"));
      } else { // keydump in preparation phase, recordload in loading phase, and all methods in
               // dynamic workload
        Properties tmpfileprops = new Properties();
        String workloadPatternFile = IOHelper.getWorkloadPatternFile(GlobalConfig.getWorkloadName());
        try {
          tmpfileprops.load(new FileInputStream(workloadPatternFile));
        } catch (IOException e) {
          System.out.println("Unable to open the properties file " + workloadPatternFile);
          System.out.println(e.getMessage());
          System.exit(0);
        }
        for (Enumeration e = tmpfileprops.propertyNames(); e.hasMoreElements();) {
          String tmpprop = (String) e.nextElement();
          props.setProperty(tmpprop, tmpfileprops.getProperty(tmpprop));
        }
        System.out.println(String.format("[INFO][Client] workload pattern file: %s", workloadPatternFile));
      }
      // props.setProperty("fieldlength", "128"); // ONLY for CoreWorkload
      // (3) maxExecutionTime
      if (!GlobalConfig.isCurmethodForTransaction()) {
        maxExecutionTime = 0; // not limit execution time for keydump and recordload
      } else {
        if (GlobalConfig.getWorkloadMode() == 0) { // static pattern
          maxExecutionTime = GlobalConfig.getDynamicPeriodInterval(); // 10s to limit static rotation evaluation time, as we only need overall
                                 // statistics
        } else { // dynamic pattern
          // 75s to get sufficient per-second statistics
          maxExecutionTime = GlobalConfig.getDynamicPeriodNum() * GlobalConfig.getDynamicPeriodInterval() + 5;
        }
      }
      // (4) measurement type
      if (GlobalConfig.isCurmethodForTransaction()) {
        props.setProperty("measurementtype", "customhistogram");
      }
      // (5) status thread
      if (GlobalConfig.isDynamicPattern()) {
        status = true;
        props.setProperty("status.interval", "1");
      }
    ...
    // InswitchCache-aware initialization of DB instances
    ...
    try (final TraceScope span = tracer.newScope(CLIENT_WORKLOAD_SPAN)) {
      ...
      // In-switch cache workload preparation.
      if (GlobalConfig.isInswitchCacheDatabase(dbname)) {
        InswitchCacheClient.preBenchmark();
      }
      ...
      if (maxExecutionTime > 0) {
        terminator = new TerminatorThread(maxExecutionTime, threads.keySet(), workload, clients);
        terminator.start();
      }

      // After all sub-threads starting packet sending yet before finishing
      if (GlobalConfig.isInswitchCacheDatabase(dbname)) {
        InswitchCacheClient.withinBenchmark();
      }
    ...
    // In-Switch cache wrap up
    if (GlobalConfig.isInswitchCacheDatabase(dbname)) {
      InswitchCacheClient.postBenchmark();
    }
  ...
  private static List<ClientThread> initDb(String dbname, Properties props, int threadcount,
    ...
    try (final TraceScope span = tracer.newScope(CLIENT_INIT_SPAN)) {
      ...
      for (int threadid = 0; threadid < threadcount; threadid++) {
        DB db;
        try {
          // Just call default constructor of DB, which has not invoked init().
          if (GlobalConfig.isInswitchCacheDatabase(dbname)) {
            Properties inSwitchCacheProperties = new Properties(); // BREAKPOINT
            inSwitchCacheProperties.setProperty(InswitchCacheClient.LOCAL_LOGICAL_CLIENT_INDEX, String.valueOf(
                threadid));
            db = DBFactory.newDB(dbname, inSwitchCacheProperties, tracer);
            if (GlobalConfig.getCurmethodId() > 0 && GlobalConfig.getCurmethodId() < 4 && GlobalConfig.isStaticPattern()){ // is methodid
              opcount = GlobalConfig.getopcount();
            }
          } else {
  ...
  private static Workload getWorkload(Properties props) {
    ...
    System.err.println();
    System.err.println("[INFO][Client] Loading workload...");
    try {
  ...
  private static Properties parseArguments(String[] args) {
    ...
    while (args[argindex].startsWith("-")) {
      if (args[argindex].compareTo("-threads") == 0) {
        ...
        argindex++;
      } else if (args[argindex].compareTo("-pi") == 0) {
        // Parse InswitchCache-related arguments.
        argindex++;
        if (argindex >= args.length) {
          usageMessage();
          System.out.println("Missing argument value for -pi.");
          System.exit(0);
        }
        props.setProperty(InswitchCacheClient.PHYSICAL_CLIENT_INDEX, args[argindex]);
        argindex++;
      } else if (args[argindex].compareTo("-sr") == 0) {
        // Parse InswitchCache-related arguments.
        argindex++;
        if (argindex >= args.length) {
          usageMessage();
          System.out.println("Missing argument value for -sr.");
          System.exit(0);
        }
        props.setProperty(InswitchCacheClient.SINGLE_ROTATION, args[argindex]);
        argindex++;
      } else {
        usageMessage();
      ...
      // else if (args[argindex].compareTo("-df") == 0) {
      // argindex++;
      // if (argindex >= args.length) {
      // usageMessage();
      // System.out.println("Missing argument value for -df.");
      // System.exit(0);
      // }
      // props.setProperty(InswitchCacheClient.WORKLOAD_DUMPING_FILE, args[argindex]);
      // argindex++;
      // }
