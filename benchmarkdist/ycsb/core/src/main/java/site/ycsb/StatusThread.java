...
public class StatusThread extends Thread {
  ...
  public void dumpstatistic(){
    ...
    for (int i = 1; i < switchAndServerLoadList.length; i++) {
      ...
      tmpsum += perserverOpsdone[i-1];
    }
    double processing = (double) tmpsum / 1000000.0;
    System.out.printf("%.2f serverOps: [%d",processing , perserverOpsdone[0]);
  ...
  public void run() {
    ...
    long lastTotalOps = 0;
    // System.out.println("[debug]"+sleeptimeNs);
    boolean alldone;
  ...
  private long computeStats(final long startTimeMs, long startIntervalMs, long endIntervalMs,
    ...
    if (GlobalConfig.isInswitchCacheMethod() && GlobalConfig.isDynamicPattern()
      ...
      if (GlobalConfig.isStaticPattern()) {
        ...
        long executionMillis = curExecutionMillis; // in unit of ms
        // System.out.println("[debug]executionMillis:"+ curExecutionMillis);
        //long executionMillis = maxExecutionTime * 1000; // in unit of ms
  
        // Get latency histogram from YCSB
        ConcurrentHashMap<String, OneMeasurement> myopToMesurementMap = Measurements.getMeasurements().getOpToMesurementMap();
        ...
        System.out.print("["+perserverOpsdone[0]);
        for (int i = 1; i < perserverOpsdone.length; i++) {
          System.out.print(","+perserverOpsdone[i]);
        }
        System.out.println("]");
        System.out.print("["+perserverCachehits[0]);
        for (int i = 1; i < perserverCachehits.length; i++) {
          System.out.print(","+perserverCachehits[i]);
        }
        System.out.println("]");
        // System.out.println(Arrays.toString(perserverOpsdone));
        // System.out.println(Arrays.toString(perserverCachehits));  
        // System.out.println(perserverOpsdone);
        // System.out.println(perserverCachehits);
        // printArray(perserverOpsdone)
        // printArray(perserverCachehits)
        // Collect and dump statistics by StatisticsHelper
        String tstrid = "0";
        // System.out.println("[debug]"+tstrid + " " + totalOpsdone);
        StatisticsHelper.collect(tstrid, totalOpsdone, perserverOpsdone, perserverCachehits, executionMillis,
