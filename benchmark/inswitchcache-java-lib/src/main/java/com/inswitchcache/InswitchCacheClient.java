...
public final class InswitchCacheClient {
    ...
    switch (GlobalConfig.getCurmethodId()) {
      ...
      case GlobalConfig.NETCACHE_ID:
        ...
        if (GlobalConfig.isNonStableDynamicPattern()) {
          ...
          // periodKeyKeyRecordsList = new ArrayList<HashMap<String, String>>();
          // syncPeriodKeyKeyRecordsList =
          // Collections.synchronizedList(periodKeyKeyRecordsList);

          // periodKeyKeyFreqList = new ArrayList<HashMap<String, Integer>>();
          // syncPeriodKeyKeyFreqList =
          // Collections.synchronizedList(periodKeyKeyFreqList);
          // for (int i = 0; i < GlobalConfig.getDynamicPeriodNum(); i++) {
          // syncPeriodKeyKeyRecordsList.add(new HashMap<String, String>());
          // syncPeriodKeyKeyFreqList.add(new HashMap<String, Integer>());
          // }
      ...
      case GlobalConfig.FARREACH_ID:
        try {
          if (GlobalConfig.getClientPhysicalIdx() == 0) {
            ...
            System.out.println("[INFO][InswitchCacheClient] enable snapshot successfully");
    ...
    while (true) {
      ...
      if (recvSize != -1) {
        ...
        if (ByteBuffer.wrap(rulemapSvrTmpBuf).getInt() == dynamicPeriodIdx) {
          ...
          System.out.println("[INFO][InSwitchCacheClient] Switch dynamic rulemap from period " + dynamicPeriodIdx
  ...
  public static void dumpHotKeys() {
    ...
    int maxnum = (sortedKeyFrequencyRecords.size() > 10000) ? 10000 : sortedKeyFrequencyRecords.size();
