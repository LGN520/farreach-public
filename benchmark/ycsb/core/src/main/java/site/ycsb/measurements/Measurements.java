...
public class Measurements {
  ...
  public Measurements(Properties props) {
    ...
    case "customhistogram":
      measurementType = MeasurementType.CUSTOMHISTOGRAM;
      break;
    default:
  ...
  private OneMeasurement constructOneMeasurement(String name) {
    ...
    case CUSTOMHISTOGRAM:
      return new OneMeasurementCustomHistogram(name, props);
    default:
  ...
  /**
   * measure with key for server rotation.
   */
  public void measure(String operation, String key, boolean ishit, int latency) {
    if (measurementInterval == 1) {
      return;
    }
    try {
      OneMeasurement m = getOpMeasurement(operation);
      if (m instanceof OneMeasurementCustomHistogram) {
        m.measure(key, ishit, latency);
      } else {
        m.measure(latency);
      }
    } catch (java.lang.ArrayIndexOutOfBoundsException e) {
  ...
  public ConcurrentHashMap<String, OneMeasurement> getOpToMesurementMap() {
    return opToMesurementMap;
  }
}
