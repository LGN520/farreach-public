...
public class CoreWorkload extends Workload {
  ...
  protected static NumberGenerator getFieldLengthGenerator(Properties p) throws WorkloadException {
    ...
    double zipfianconstant = Double.parseDouble(p.getProperty(ZIPFIAN_CONSTANT, ZIPFIAN_CONSTANT_DEFAULT));
    System.out.println("[DEBUG][CoreWorkload] zipfianconstant: " + zipfianconstant);
    ...
    if (fieldlengthdistribution.compareTo("constant") == 0) {
      ...
      fieldlengthgenerator = new ZipfianGenerator(minfieldlength, fieldlength, zipfianconstant);
  ...
  public void init(Properties p) throws WorkloadException {
    ...
    if (scanlengthdistrib.compareTo("uniform") == 0) {
      ...
      double zipfianconstant = Double.parseDouble(p.getProperty(ZIPFIAN_CONSTANT, ZIPFIAN_CONSTANT_DEFAULT));
      scanlength = new ZipfianGenerator(minscanlength, maxscanlength, zipfianconstant);
    } else {
      throw new WorkloadException(
  ...
  public boolean doInsert(DB db, Object threadstate) {
    ...
    //HashMap<String, ByteIterator> values = buildValues(dbkey); // YCSB default

    // fit for inswitchcachce databases
