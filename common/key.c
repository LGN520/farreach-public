...
uint16_t Key::hash_bycksum16() {
    char buf[16];
    uint32_t tmp_keysize = this->serialize(buf, 16);
    uint16_t hashresult = cksum16(reinterpret_cast<const char *>(buf), tmp_keysize);
    return hashresult;
}
...
uint32_t Key::get_rangepartition_idx(uint32_t servernum) {
#ifdef LARGE_KEY
    uint32_t keyhihihi = (keyhihi >> 16) & 0xFFFF;
    uint32_t targetidx = keyhihihi / (64 * 1024 / servernum);  // split 2^16 into servernum partitions
#else
...
uint32_t Key::get_spineswitch_idx(uint32_t partitionnum, uint32_t servernum) {
    uint16_t hashresult = hash_bycksum16() % partitionnum;
    uint32_t targetidx = hashresult / (partitionnum / servernum);
    if (targetidx == servernum)
        targetidx--;
    // printf("key: %x, crc32 result: %u, targetidx: %d\n", keyhihi, hashresult, targetidx);
...
//     uint16_t keyhihilo = uint16_t(keyhihi & 0xFFFF);
//     uint16_t keyhihihi = uint16_t((keyhihi >> 16) & 0xFFFF);
//     uint32_t hashresult = ((uint32_t(keyhihilo) << 16) | uint32_t(keyhihihi)) % partitionnum;

//     uint32_t targetidx = hashresult / (partitionnum / spineswitchnum);
//     if (targetidx == spineswitchnum)
//         targetidx--;
//     // printf("key: %x, crc32 result: %u, targetidx: %d\n", keyhihi, hashresult, targetidx);
