...
enum class PacketType {
	...
	BACKUP=0x009f,BACKUPACK=0x109f,
};
/*enum class PacketType {
...
class CachePopInswitch : public PutRequestSeq<key_t, val_t> { // ophdr + val + shadowtype + seq + inswitch_hdr + stat_hdr
	public: 
		CachePopInswitch(){}
		CachePopInswitch(method_t methodid, const char* data, uint32_t recvsize);
...
template<class key_t, class val_t>
class RecoverPkt : public CachePopInswitch<key_t, val_t> { // ophdr + val + shadowtype + seq + inswitch_hdr + stat_hdr + 12B backup_hdr
	public: 
		RecoverPkt(){}
		RecoverPkt(method_t methodid, const char* data, uint32_t recvsize);
		RecoverPkt(method_t methodid, key_t key, val_t val, uint32_t seq, uint16_t freeidx, bool stat);
	...
	protected:
		virtual uint32_t size();
		virtual void deserialize(const char * data, uint32_t recv_size);
		uint32_t cache_frequency;
    	uint32_t largevalueseq;
    	uint16_t vallen;
		uint16_t reg_meta;
};
template<class key_t>
