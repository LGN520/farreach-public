...
// Controller
const char* IniparserWrapper::get_controller_ip_for_server(uint32_t controller_idx) {
    const char* controller_ip_for_server;
    if (controller_idx == 0x3f3f3f3f) {
        controller_ip_for_server =
            iniparser_getstring(ini, "controller:controller_ip_for_server", nullptr);
    } else {
        char key[256];
        sprintf(key, "controller%u:controller_ip_for_server", controller_idx);
        controller_ip_for_server = iniparser_getstring(ini, key, nullptr);
    }
...
const char* IniparserWrapper::get_controller_ip_for_switchos(uint32_t controller_idx) {
    const char* controller_ip_for_switchos;
    if (controller_idx == 0x3f3f3f3f) {
        controller_ip_for_switchos =
            iniparser_getstring(ini, "controller:controller_ip_for_switchos", nullptr);
    } else {
        char key[256];
        sprintf(key, "controller%u:controller_ip_for_switchos", controller_idx);
        controller_ip_for_switchos = iniparser_getstring(ini, key, nullptr);
    }
...
short IniparserWrapper::get_controller_popserver_port_start(uint32_t controller_idx) {
    int tmp;
    if (controller_idx == 0x3f3f3f3f) {
        tmp = iniparser_getint(ini, "controller:controller_popserver_port_start", -1);
    } else {
        char key[256];
        sprintf(key, "controller%u:controller_popserver_port_start", controller_idx);
        tmp = iniparser_getint(ini, key, -1);
    }
    if (tmp == -1) {
...
short IniparserWrapper::get_controller_evictserver_port(uint32_t controller_idx) {
    int tmp;
    if (controller_idx == 0x3f3f3f3f) {
        tmp = iniparser_getint(ini, "controller:controller_evictserver_port", -1);
    } else {
        char key[256];
        sprintf(key, "controller%u:controller_evictserver_port", controller_idx);
        tmp = iniparser_getint(ini, key, -1);
    }
    if (tmp == -1) {
...
uint32_t IniparserWrapper::get_controller_snapshot_period(uint32_t controller_idx) {
    int tmp;
    if (controller_idx == 0x3f3f3f3f) {
        tmp = iniparser_getint(ini, "controller:controller_snapshot_period", -1);
    } else {
        char key[256];
        sprintf(key, "controller%u:controller_snapshot_period", controller_idx);
        tmp = iniparser_getint(ini, key, -1);
    }
    if (tmp == -1) {
...
short IniparserWrapper::get_controller_warmupfinishserver_port(uint32_t controller_idx) {
    int tmp;
    if (controller_idx == 0x3f3f3f3f) {
        tmp = iniparser_getint(ini, "controller:controller_warmupfinishserver_port", -1);
    } else {
        char key[256];
        sprintf(key, "controller%u:controller_warmupfinishserver_port", controller_idx);
        tmp = iniparser_getint(ini, key, -1);
    }
    if (tmp == -1) {
...
uint32_t IniparserWrapper::get_switch_partition_count(uint32_t switchos_idx) {
    int tmp;
    if (switchos_idx == 0x3f3f3f3f) {
        tmp = iniparser_getint(ini, "switch:switch_partition_count", -1);
    } else {
        char key[256];
        sprintf(key, "switch%u:switch_partition_count", switchos_idx);
        tmp = iniparser_getint(ini, key, -1);
    }
    if (tmp == -1) {
...
uint32_t IniparserWrapper::get_switch_kv_bucket_num(uint32_t switchos_idx) {
    int tmp;
    if (switchos_idx == 0x3f3f3f3f) {
        tmp = iniparser_getint(ini, "switch:switch_kv_bucket_num", -1);
    } else {
        char key[256];
        sprintf(key, "switch%u:switch_kv_bucket_num", switchos_idx);
        tmp = iniparser_getint(ini, key, -1);
    }
    if (tmp == -1) {
...
uint32_t IniparserWrapper::get_switch_pipeline_num(uint32_t switchos_idx) {
    int tmp;
    if (switchos_idx == 0x3f3f3f3f) {
        tmp = iniparser_getint(ini, "switch:switch_pipeline_num", -1);
    } else {
        char key[256];
        sprintf(key, "switch%u:switch_pipeline_num", switchos_idx);
        tmp = iniparser_getint(ini, key, -1);
    }
    if (tmp == -1) {
...
uint16_t IniparserWrapper::get_switch_max_vallen(uint32_t switchos_idx) {
    int tmp;
    if (switchos_idx == 0x3f3f3f3f) {
        tmp = iniparser_getint(ini, "switch:switch_max_vallen", -1);
    } else {
        char key[256];
        sprintf(key, "switch%u:switch_max_vallen", switchos_idx);
        tmp = iniparser_getint(ini, key, -1);
    }
    if (tmp == -1) {
...
const char* IniparserWrapper::get_switchos_ip(uint32_t switchos_idx) {
    const char* switchos_ip;
    if (switchos_idx == 0x3f3f3f3f) {
        switchos_ip = iniparser_getstring(ini, "switch:switchos_ip", nullptr);
    } else {
        char key[256];
        sprintf(key, "switch%u:switchos_ip", switchos_idx);
        switchos_ip = iniparser_getstring(ini, key, nullptr);
    }
...
// all switch has a sane num
uint32_t IniparserWrapper::get_switchos_sample_cnt(uint32_t switchos_idx) {
    int tmp;
    if (switchos_idx == 0x3f3f3f3f) {
        tmp = iniparser_getint(ini, "switch:switchos_sample_cnt", -1);
    } else {
        char key[256];
        sprintf(key, "switch%u:switchos_sample_cnt", switchos_idx);
        tmp = iniparser_getint(ini, key, -1);
    }
    if (tmp == -1) {
...
short IniparserWrapper::get_switchos_popserver_port(uint32_t switchos_idx) {
    int tmp;
    if (switchos_idx == 0x3f3f3f3f) {
        tmp = iniparser_getint(ini, "switch:switchos_popserver_port", -1);
    } else {
        char key[256];
        sprintf(key, "switch%u:switchos_popserver_port", switchos_idx);
        tmp = iniparser_getint(ini, key, -1);
    }
    if (tmp == -1) {
...
short IniparserWrapper::get_switchos_dppopserver_port(uint32_t switchos_idx) {
    int tmp;
    if (switchos_idx == 0x3f3f3f3f) {
        tmp = iniparser_getint(ini, "switch:switchos_dppopserver_port", -1);
    } else {
        char key[256];
        sprintf(key, "switch%u:switchos_dppopserver_port", switchos_idx);
        tmp = iniparser_getint(ini, key, -1);
    }
    if (tmp == -1) {
...
short IniparserWrapper::get_switchos_cppopserver_port(uint32_t switchos_idx) {
    int tmp;
    if (switchos_idx == 0x3f3f3f3f) {
        tmp = iniparser_getint(ini, "switch:switchos_cppopserver_port", -1);
    } else {
        char key[256];
        sprintf(key, "switch%u:switchos_cppopserver_port", switchos_idx);
        tmp = iniparser_getint(ini, key, -1);
    }
    if (tmp == -1) {
...
short IniparserWrapper::get_switchos_snapshotserver_port(uint32_t switchos_idx) {
    int tmp;
    if (switchos_idx == 0x3f3f3f3f) {
        tmp = iniparser_getint(ini, "switch:switchos_snapshotserver_port", -1);
    } else {
        char key[256];
        sprintf(key, "switch%u:switchos_snapshotserver_port", switchos_idx);
        tmp = iniparser_getint(ini, key, -1);
    }
    if (tmp == -1) {
...
short IniparserWrapper::get_switchos_specialcaseserver_port(uint32_t switchos_idx) {
    int tmp;
    if (switchos_idx == 0x3f3f3f3f) {
        tmp = iniparser_getint(ini, "switch:switchos_specialcaseserver_port", -1);
    } else {
        char key[256];
        sprintf(key, "switch%u:switchos_specialcaseserver_port", switchos_idx);
        tmp = iniparser_getint(ini, key, -1);
    }
    if (tmp == -1) {
...
short IniparserWrapper::get_switchos_ptf_popserver_port(uint32_t switchos_idx) {
    int tmp;
    if (switchos_idx == 0x3f3f3f3f) {
        tmp = iniparser_getint(ini, "switch:switchos_ptf_popserver_port", -1);
    } else {
        char key[256];
        sprintf(key, "switch%u:switchos_ptf_popserver_port", switchos_idx);
        tmp = iniparser_getint(ini, key, -1);
    }
    if (tmp == -1) {
...
short IniparserWrapper::get_switchos_ptf_snapshotserver_port(uint32_t switchos_idx) {
    int tmp;
    if (switchos_idx == 0x3f3f3f3f) {
        tmp = iniparser_getint(ini, "switch:switchos_ptf_snapshotserver_port", -1);
    } else {
        char key[256];
        sprintf(key, "switch%u:switchos_ptf_snapshotserver_port", switchos_idx);
        tmp = iniparser_getint(ini, key, -1);
    }
    if (tmp == -1) {
...
const char* IniparserWrapper::get_reflector_ip_for_switchos(uint32_t reflector_idx) {
    const char* reflector_ip_for_switchos;
    if (reflector_idx == 0x3f3f3f3f) {
        reflector_ip_for_switchos =
            iniparser_getstring(ini, "reflector:reflector_ip_for_switchos", nullptr);
    } else {
        char key[256];
        sprintf(key, "reflector%u:reflector_ip_for_switchos", reflector_idx);
        reflector_ip_for_switchos = iniparser_getstring(ini, key, nullptr);
    }
...
short IniparserWrapper::get_reflector_dp2cpserver_port(uint32_t reflector_idx) {
    int tmp;
    if (reflector_idx == 0x3f3f3f3f) {
        tmp = iniparser_getint(ini, "reflector:reflector_dp2cpserver_port", -1);
    } else {
        char key[256];
        sprintf(key, "reflector%u:reflector_dp2cpserver_port", reflector_idx);
        tmp = iniparser_getint(ini, key, -1);
    }
    if (tmp == -1) {
...
short IniparserWrapper::get_reflector_cp2dpserver_port(uint32_t reflector_idx) {
    int tmp;
    if (reflector_idx == 0x3f3f3f3f) {
        tmp = iniparser_getint(ini, "reflector:reflector_cp2dpserver_port", -1);
    } else {
        char key[256];
        sprintf(key, "reflector%u:reflector_cp2dpserver_port", reflector_idx);
        tmp = iniparser_getint(ini, key, -1);
    }
    if (tmp == -1) {
...
/* config.ini for netcache/distcache/distributed-scenario */
...
short IniparserWrapper::get_server_valueupdateserver_port_start() {
    int tmp = iniparser_getint(ini, "server:server_valueupdateserver_port_start", -1);
    if (tmp == -1) {
        printf("Invalid entry of [server:server_valueupdateserver_port_start]: %d\n", tmp);
        exit(-1);
    }
    return short(tmp);
}
...
std::vector<const char *> IniparserWrapper::get_spineswitchos_ips(int num_switches) {
    std::vector<const char *> spineswitchos_ips;
    for (int i = 0; i < num_switches; ++i) {
        char key[50];
        snprintf(key, sizeof(key), "spineswitch%d:switchos_ip", i);
        const char *spineswitchos_ip = iniparser_getstring(ini, key, nullptr);
        if (spineswitchos_ip == nullptr) {
            printf("Invalid entry of %s\n", key);
            exit(-1);
        }
        spineswitchos_ips.push_back(spineswitchos_ip);
    }
