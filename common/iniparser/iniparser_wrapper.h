...
class IniparserWrapper {
   public:
    ...
    short get_client_upstreambackupreleaser_port();
    ...
    // controller
    const char* get_controller_ip_for_server(uint32_t controller_idx = 0x3f3f3f3f);
    const char* get_controller_ip_for_switchos(uint32_t controller_idx = 0x3f3f3f3f);
    short get_controller_popserver_port_start(uint32_t controller_idx = 0x3f3f3f3f);
    short get_controller_evictserver_port(uint32_t controller_idx = 0x3f3f3f3f);
    uint32_t get_controller_snapshot_period(uint32_t controller_idx = 0x3f3f3f3f);
    short get_controller_warmupfinishserver_port(uint32_t controller_idx = 0x3f3f3f3f);

    // switch
    uint32_t get_switch_partition_count(uint32_t switchos_idx = 0x3f3f3f3f);
    uint32_t get_switch_kv_bucket_num(uint32_t switchos_idx = 0x3f3f3f3f);
    uint32_t get_switch_pipeline_num(uint32_t switchos_idx = 0x3f3f3f3f);
    uint16_t get_switch_max_vallen(uint32_t switchos_idx = 0x3f3f3f3f);
    const char* get_switchos_ip(uint32_t switchos_idx = 0x3f3f3f3f);
    uint32_t get_switchos_sample_cnt(uint32_t switchos_idx = 0x3f3f3f3f);
    short get_switchos_popserver_port(uint32_t switchos_idx = 0x3f3f3f3f);
    short get_switchos_dppopserver_port(uint32_t switchos_idx = 0x3f3f3f3f);
    short get_switchos_cppopserver_port(uint32_t switchos_idx = 0x3f3f3f3f);
    short get_switchos_snapshotserver_port(uint32_t switchos_idx = 0x3f3f3f3f);
    short get_switchos_specialcaseserver_port(uint32_t switchos_idx = 0x3f3f3f3f);
    short get_switchos_ptf_popserver_port(uint32_t switchos_idx = 0x3f3f3f3f);
    short get_switchos_ptf_snapshotserver_port(uint32_t switchos_idx = 0x3f3f3f3f);

    // reflector
    const char* get_reflector_ip_for_switchos(uint32_t reflector_idx = 0x3f3f3f3f);
    short get_reflector_dp2cpserver_port(uint32_t reflector_idx = 0x3f3f3f3f);
    short get_reflector_cp2dpserver_port(uint32_t reflector_idx = 0x3f3f3f3f);

    /* config.ini for netcache/distcache/distributed-scenario */

    // NOTE: unless specified explicitly, methods are provided for distributed scenario of each technique

    // server common configuration
    short get_server_popserver_port_start();          // netcache/distcache
    short get_server_valueupdateserver_port_start();  // netcache/distcache
    short get_server_invalidateserver_port_start();   // distcache

    // controller
    short get_controller_victimserver_port();  // distcache

    // switch
    short get_leafswitchos_victimserver_port();  // distcache
