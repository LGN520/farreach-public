...
void* run_recover_worker(void* param) {
    ...
    while (switchos_running) {
        ...
        fflush(stdout);
        // write all key to a file named prepared_key for recover
    ...
    // while (infile.read(reinterpret_cast<char*>(&idx_cached_meta[worker_send_counter]), sizeof(uint16_t))) {
    //     infile.read(reinterpret_cast<char*>(&idx_cached_key[worker_send_counter]->recoverkey.keylolo), sizeof(uint32_t));
    //     infile.read(reinterpret_cast<char*>(&idx_cached_key[worker_send_counter]->recoverkey.keylohi), sizeof(uint32_t));
    //     infile.read(reinterpret_cast<char*>(&idx_cached_key[worker_send_counter]->recoverkey.keyhilo), sizeof(uint32_t));
    //     infile.read(reinterpret_cast<char*>(&idx_cached_key[worker_send_counter]->recoverkey.keyhihi), sizeof(uint32_t));
    //     worker_send_counter++;
    // }
