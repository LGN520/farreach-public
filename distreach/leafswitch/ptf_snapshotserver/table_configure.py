...
class RegisterUpdate():
    ...
    def cleanup(self):
        ...
        entrynum = self.controller.table_num_entries('need_recirculate_tbl')
        if entrynum > 0:
            for i in range(len(self.unmatched_devports)):
                ...
                for tmpoptype in [PUTREQ, DELREQ, GETRES_LATEST_SEQ, GETRES_DELETED_SEQ, PUTREQ_LARGEVALUE]:
                    matchspec0 = [
                            hex(tmpoptype),
                            iport]
                    self.controller.table_delete_match('need_recirculate_tbl', matchspec0)
        print("Reset snapshot_flag=0 for all ingress pipelines")
        entrynum = self.controller.table_num_entries('snapshot_flag_tbl')
        ...
        if entrynum > 0:
            for tmpoptype in [PUTREQ, DELREQ, GETRES_LATEST_SEQ, GETRES_DELETED_SEQ, PUTREQ_LARGEVALUE]:
                matchspec0 = [
                        hex(tmpoptype),
                        hex(0)]
                self.controller.table_delete_match('snapshot_flag_tbl',matchspec0)

        print("Reset case1_reg for all pipelines")
        self.controller.register_reset('case1_reg')
        ...
        for i in range(len(self.unmatched_devports)):
            ...
            for tmpoptype in [PUTREQ, DELREQ, GETRES_LATEST_SEQ, GETRES_DELETED_SEQ, PUTREQ_LARGEVALUE]:
                matchspec0 = [
                        ...
                        iport]
                actnspec0 = [recirport]
                self.controller.table_add('need_recirculate_tbl','set_need_recirculate',matchspec0, actnspec0)
    ...
    def set_snapshot_flag(self, snapshotid):
        ...
        for tmpoptype in [PUTREQ, DELREQ, GETRES_LATEST_SEQ, GETRES_DELETED_SEQ, PUTREQ_LARGEVALUE]:
            ...
            actnspec0 = [
                    hex(snapshotid)]
            self.controller.table_add('snapshot_flag_tbl','set_snapshot_flag',matchspec0, actnspec0)
