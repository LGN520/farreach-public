...
partition_json_path = current_dir + "/" + "../partitionswitch/partitionswitch.json"
...
def create_network():
    ...
    clientsideswitch = net.addSwitch(
        "client_s{}".format(1),
        cls=P4Switch,
    ...
    for i in range(client_physical_num):
        # spine
        net.addLink(host[i], clientsideswitch)
    for i in range(rack_physical_num):
        # spine
        net.addLink(spineswitchs[i], clientsideswitch)
    for i in range(server_physical_num):
        # rack
        net.addLink(rackswitchs[int(i / 2)], host[client_physical_num + i])
    for i in range(rack_physical_num):
        # rack spine
        net.addLink(rackswitchs[i], spineswitchs[i])
    ...
    # for i in range(rack_physical_num):
        # net.addLink(leafswitchoses[i], nat_s1)
    # host[i]
    for i in range(client_physical_num + server_physical_num):
        host[i].cmdPrint(
