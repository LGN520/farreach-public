...
action get_deleted() {
	hdr.inswitch_hdr.is_deleted = deleted_reg_get_alu.execute((bit<32>)hdr.inswitch_hdr.idx);
}

action set_and_get_deleted() {
	hdr.inswitch_hdr.is_deleted = deleted_reg_get_set_alu.execute((bit<32>)hdr.inswitch_hdr.idx);
}


action reset_and_get_deleted() {
	hdr.inswitch_hdr.is_deleted =deleted_reg_get_reset_alu.execute((bit<32>)hdr.inswitch_hdr.idx);
}

action reset_is_deleted() {
	hdr.inswitch_hdr.is_deleted = 0;
}
