...
void run_warmuper() {
    ...
    printf("[warmup_client] cache size: %d\n", switch_kv_bucket_num * total_rack_num);
    fflush(stdout);
    int tmp_warmupcnt = 0;
    while (true) {
        if (!iter->next()) {
        ...
        tmpkey = iter->key();
        int switchidx = tmpkey.get_hashpartition_idx(switch_partition_count, max_server_total_logical_num) / perswitch_server_num;
        if (perswitch_cached_num[switchidx] >= switch_kv_bucket_num) {
            continue;
        } else {
            perswitch_cached_num[switchidx]++;
        }
    ...
    for (int i = 0; i < total_rack_num; i++) {
        printf("switch %d cached %d\n", i, perswitch_cached_num[i]);
    }
    iter->closeiter();
    delete iter;
    iter = NULL;
