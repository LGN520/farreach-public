...
void* main_recover(void* param) {
    ...
    // printf("[debug] persistent_ack_num > switch_kv_bucket_num finished\n");
    // CUR_TIME(recover_t2);
    // DELTA_TIME(recover_t2, recover_t1, recover_t3);
    // printf("[Statistics] persist server: %f s w/ cache size %d\n", GET_MICROSECOND(recover_t3) / 1000.0 / 1000.0, switch_kv_bucket_num);
    // fflush(stdout);
...
void* run_recover_worker(void* param) {
    ...
    std::vector<int> server_idx_belong_to;
    ...
    while (switchos_running) {
        ...
        // read keys from file prepared_keys.out
        // write flow
        // std::ofstream outfile("prepared_key.out", std::ios::binary);
        // for (int i = 0; i < switch_kv_bucket_num; ++i) {
        // if (idx_cached_key[i] != nullptr) {
        // outfile.write(reinterpret_cast<const char*>(&idx_cached_key[i]->idx), sizeof(uint16_t));

        // outfile.write(reinterpret_cast<const char*>(&idx_cached_key[i]->recoverkey.keylolo), sizeof(uint32_t));
        // outfile.write(reinterpret_cast<const char*>(&idx_cached_key[i]->recoverkey.keylohi), sizeof(uint32_t));
        // outfile.write(reinterpret_cast<const char*>(&idx_cached_key[i]->recoverkey.keyhilo), sizeof(uint32_t));
        // outfile.write(reinterpret_cast<const char*>(&idx_cached_key[i]->recoverkey.keyhihi), sizeof(uint32_t));
        // }
        // }
        // read them
        ...
        for (int i = 0; i < switch_kv_bucket_num; ++i) {
            if (idx_cached_key[i] == nullptr) {
                printf("[error] wrong read of prepared record\n");
                exit(-1);
            }
            int server_idx = idx_cached_key[i]->recoverkey.get_hashpartition_idx(switch_partition_count, max_server_total_logical_num);
            if (server_idx == local_server_idx) {
                server_idx_belong_to.push_back(i);
            } else {
                // skip it
                idx_cached_is_recovered[idx_cached_key[i]->idx] = true;
                idx_cached_is_persist[idx_cached_key[i]->idx] = true;
                recover_ack_num++;
                persistent_ack_num++;
            }
        }
        ...
        CUR_TIME(recover_t2);
        DELTA_TIME(recover_t2, recover_t1, recover_t3);
        printf("[Statistics] fetch key: %f s w/ cache size %d\n Skip %d\n", GET_MICROSECOND(recover_t3) / 1000.0 / 1000.0, switch_kv_bucket_num,persistent_ack_num);
        fflush(stdout);
            ...
            for (uint32_t start = 0; start < switch_kv_bucket_num; start += step) {
                ...
                while (true) {
                    ...
                    if ((htonl(((uint16_t*)buf)[0]) >> 16) == uint16_t(packet_type_t::BACKUP)) {
                        ...
                        printf("[error] wrong free of unrecover record %x\n",controltype);
                        // dump buf
        ...
        fflush(stdout);
        // write value to local file for persist
        // std::ofstream outfile("local_persist.out", std::ios::binary);
         CUR_TIME(recover_t1);
        // vector<vector<int>> server_idxs;
        for (int i = 0; i < switch_kv_bucket_num; i++) {
