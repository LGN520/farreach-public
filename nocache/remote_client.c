		...
		while (true) { // timeout-and-retry mechanism
			...
			if (tmptype == optype_t(packet_type_t::GETREQ)) { // get
				...
				for (int tmpscanidx = 0; tmpscanidx < received_scannum; tmpscanidx++) {
					//scan_response_split_t rsp(CURMETHOD_ID, scanbufs + tmpscanidx * MAX_BUFSIZE, scan_recvsizes[tmpscanidx]);
					scan_response_split_t rsp(CURMETHOD_ID, scanbufs[tmpscanidx].array(), scanbufs[tmpscanidx].size());
