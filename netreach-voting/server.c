			...
			case packet_type_t::SCAN_REQ:
				{
					...
					if (kvdata != nullptr) {
						rsp = new scan_response_t(req.thread_id(), req.key(), req.endkey(), merge_results.size(), merge_results);
					}
					else {
						rsp = new scan_response_t(req.thread_id(), req.key(), req.endkey(), tmp_num, results);
					}
					rsp_size = rsp->serialize(buf, MAX_BUFSIZE);
					//res = sendto(sockfd, buf, rsp_size, 0, (struct sockaddr *)&server_sockaddr, sizeof(struct sockaddr));
