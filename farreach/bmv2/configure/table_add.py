...
class TableConfigure():
    ...
    def configure_update_val_tbl(self, valname):
        ...
        for access_val_mode in access_val_mode_list:
            ...
            if access_val_mode == 1:
                controller.table_add('update_val{}_tbl'.format(
                    valname), 'get_val{}'.format(valname), matchspec0, [])
            elif access_val_mode == 2:
                controller.table_add('update_val{}_tbl'.format(
                    valname), 'set_and_get_val{}'.format(valname), matchspec0, [])
            elif access_val_mode == 3:
                controller.table_add('update_val{}_tbl'.format(
                    valname), 'reset_and_get_val{}'.format(valname), matchspec0, [])
    ...
    def create_mirror_session(self):
        ...
        for i in range(client_physical_num):
            print("Binding sid {} with client devport {} for both direction mirroring".format(
                self.client_sids[i], self.client_devports[i]))  # clone to client
            controller.mirroring_add(
                self.client_sids[i], self.client_devports[i])
        for i in range(server_physical_num):
            print("Binding sid {} with server devport {} for both direction mirroring".format(
                self.server_sids[i], self.server_devports[i]))  # clone to server
            controller.mirroring_add(
    ...
    def runTest(self):
        ...
        for i in range(client_physical_num):
            controller.table_add('l2l3_forward_tbl', 'l2l3_forward', [
                                 client_macs[i], client_ips[i]+'/32'], [self.client_devports[i]])
        for i in range(server_physical_num):
            controller.table_add('l2l3_forward_tbl', 'l2l3_forward', [
                                 server_macs[i], server_ips[i]+'/32'], [self.server_devports[i]])
        ...
        for tmpoptype in [PUTREQ, DELREQ, GETRES_LATEST_SEQ, GETRES_DELETED_SEQ, PUTREQ_LARGEVALUE]:
            ...
            controller.table_add(
                'recirculate_tbl', 'recirculate_pkt', matchspec0)
        ...
        if RANGE_SUPPORT == False:
            ...
            for tmpoptype in [GETREQ, CACHE_POP_INSWITCH, PUTREQ, DELREQ, WARMUPREQ, LOADREQ, CACHE_EVICT_LOADFREQ_INSWITCH, CACHE_EVICT_LOADDATA_INSWITCH, LOADSNAPSHOTDATA_INSWITCH, SETVALID_INSWITCH, GETRES_LATEST_SEQ, GETRES_DELETED_SEQ, PUTREQ_LARGEVALUE]:
                matchspec0 = [
                        hex(tmpoptype),
                        hex(0)]
                controller.table_add(
        ...
        for tmpoptype in [GETREQ, CACHE_POP_INSWITCH, PUTREQ, DELREQ, WARMUPREQ, LOADREQ, CACHE_EVICT_LOADFREQ_INSWITCH, CACHE_EVICT_LOADDATA_INSWITCH, LOADSNAPSHOTDATA_INSWITCH, SETVALID_INSWITCH, GETRES_LATEST_SEQ, GETRES_DELETED_SEQ, PUTREQ_LARGEVALUE]:
            ...
            for global_server_logical_idx in range(server_total_logical_num):
                ...
                if server_physical_idx == -1:
                    print("WARNING: no physical server covers global_server_logical_idx {} -> no corresponding MAT entries in hash_partition_tbl").format(
                    ...
                    if tmpoptype == GETRES_LATEST_SEQ or tmpoptype == GETRES_DELETED_SEQ:
                        ...
                        controller.table_add(
                            'hash_partition_tbl', 'hash_partition_for_special_response', matchspec0, actnspec0, 0)
                    else:
        ...
        for i in ["12", "34"]:
            ...
            for tmpoptype in [GETREQ]:
                ...
                controller.table_add('hash_for_cm{}_tbl'.format(i), 'hash_for_cm{}'.format(i),
                                     matchspec0)
        ...
        for tmpoptype in [PUTREQ, DELREQ, PUTREQ_LARGEVALUE]:
            matchspec0 = [
                    hex(tmpoptype),
                    hex(0)]
            controller.table_add('hash_for_seq_tbl',
                                 'hash_for_seq', matchspec0)
        ...
        for client_physical_idx in range(client_physical_num):
            ...
            for tmpoptype in [GETREQ, PUTREQ, DELREQ]:
                controller.table_add('prepare_for_cachehit_tbl', 'set_client_sid',
                                     [hex(tmpoptype), ''+client_ips[client_physical_idx]+'/32', hex(0)], [hex(tmp_clientsid)])
        ...
        for tmp_client_physical_idx in range(client_physical_num):
            ...
            for tmpoptype in [GETRES_LATEST_SEQ, GETRES_DELETED_SEQ]:
                ...
                controller.table_add(
                    'ipv4_forward_tbl', 'forward_special_get_response', matchspec0, actnspec0)
        ...
        matchspec0 = [
                hex(GETREQ),
                hex(0)]
        controller.table_add('ig_port_forward_tbl',
                             'update_getreq_to_getreq_inswitch', matchspec0)
        matchspec0 = [
                hex(GETRES_LATEST_SEQ),
                hex(0)]
        controller.table_add(
            'ig_port_forward_tbl', 'update_getres_latest_seq_to_getres_latest_seq_inswitch', matchspec0)
        matchspec0 = [
                hex(GETRES_DELETED_SEQ),
                hex(0)]
        controller.table_add(
            'ig_port_forward_tbl', 'update_getres_deleted_seq_to_getres_deleted_seq_inswitch', matchspec0)
        matchspec0 = [
                hex(PUTREQ),
                hex(0)]
        controller.table_add('ig_port_forward_tbl',
                             'update_putreq_to_putreq_inswitch', matchspec0)
        matchspec0 = [
                hex(DELREQ),
                hex(0)]
        controller.table_add('ig_port_forward_tbl',
                             'update_delreq_to_delreq_inswitch', matchspec0)
        if RANGE_SUPPORT:
            matchspec0 = [
                    hex(SCANREQ),
                    hex(0)]
            controller.table_add('ig_port_forward_tbl',
                                 'update_scanreq_to_scanreq_split', matchspec0)
        matchspec0 = [
                hex(PUTREQ_LARGEVALUE),
                hex(0)]
        controller.table_add(
            'ig_port_forward_tbl', 'update_putreq_largevalue_to_putreq_largevalue_inswitch', matchspec0)
        ...
        for i in range(1, cm_hashnum+1):
            ...
            for tmpoptype in [GETREQ_INSWITCH, PUTREQ_INSWITCH]:
                ...
                controller.table_add('access_cm{}_tbl'.format(i), 'update_cm{}'.format(i), matchspec0)
        ...
        for tmpoptype in [GETREQ_INSWITCH, PUTREQ_INSWITCH]:
            ...
            controller.table_add('access_cache_frequency_tbl',
                                 'update_cache_frequency', matchspec0)
        ...
        for tmpoptype in [GETREQ_INSWITCH, GETRES_LATEST_SEQ_INSWITCH, GETRES_DELETED_SEQ_INSWITCH, PUTREQ_INSWITCH, DELREQ_INSWITCH, PUTREQ_LARGEVALUE_INSWITCH]:
            matchspec0= [
                    hex(tmpoptype),
                    hex(1)]
            controller.table_add('access_validvalue_tbl',
                                 'get_validvalue', matchspec0)
        for is_cached in cached_list:
            # NOTE: set_validvalue does not change validvalue_hdr.validvalue
            matchspec0= [
                    hex(SETVALID_INSWITCH),
                    hex(is_cached)]  # key may or may not be cached for SETVALID_INSWITCH
        ...
        for tmpoptype in [GETREQ_INSWITCH, PUTREQ_INSWITCH, DELREQ_INSWITCH]:
            matchspec0= [
                    hex(tmpoptype)]
            controller.table_add('save_client_udpport_tbl',
                                 'save_client_udpport', matchspec0)
        ...
        for is_cached in cached_list:
            for validvalue in validvalue_list:
                ...
                matchspec0= [
                        ...
                        hex(validvalue),
                        hex(0)]
                controller.table_add('access_latest_tbl',
                                     'reset_and_get_latest', matchspec0)
        ...
        for is_cached in cached_list:
            for validvalue in validvalue_list:
                ...
                if is_cached == 1 and validvalue == 1:
                    controller.table_add(
                        'access_largevalueseq_and_save_assignedseq_tbl', 'get_largevalueseq', matchspec0)
                for tmpoptype in [GETRES_LATEST_SEQ_INSWITCH, GETRES_DELETED_SEQ_INSWITCH, PUTREQ_INSWITCH, DELREQ_INSWITCH]:
                    ...
                    if is_cached == 1 and validvalue == 1:
                        controller.table_add(
                            'access_largevalueseq_and_save_assignedseq_tbl', 'reset_largevalueseq', matchspec0)
                matchspec0= [
                        hex(CACHE_POP_INSWITCH),
                        hex(is_cached),
                        hex(validvalue),
                ...
                if is_cached == 1 and validvalue == 1:
                    controller.table_add(
                        'access_largevalueseq_and_save_assignedseq_tbl', 'set_largevalueseq', matchspec0)
            ...
            for validvalue in validvalue_list:
                for is_latest in latest_list:
                    matchspec0= [
                            ...
                            hex(is_latest)]
                    if is_cached == 1:
                        controller.table_add('update_vallen_tbl','get_vallen',matchspec0)
                    for tmpoptype in [GETRES_LATEST_SEQ_INSWITCH, GETRES_DELETED_SEQ_INSWITCH]:
                        matchspec0 = [
                                hex(tmpoptype),
                                hex(is_cached),
                        ...
                        if is_cached == 1 and validvalue == 1 and is_latest == 0:
                            controller.table_add('update_vallen_tbl','set_and_get_vallen',matchspec0)
                    matchspec0 = [
                            hex(PUTREQ_INSWITCH),
                            hex(is_cached),
                            hex(validvalue),
                    ...
                    matchspec0 = [
                            ...
                            hex(is_latest)]
                    if is_cached == 1 and validvalue == 1:
                        controller.table_add('update_vallen_tbl','reset_and_get_vallen',matchspec0)
                    matchspec0 = [
                            hex(CACHE_POP_INSWITCH),
                            hex(is_cached),
                            hex(validvalue),
        ...
        for is_cached in cached_list:
            for validvalue in validvalue_list:
                for is_latest in latest_list:
                    ...
                    if is_cached == 1:
                        controller.table_add('access_savedseq_tbl','get_savedseq',matchspec0)
                    for tmpoptype in [PUTREQ_INSWITCH, DELREQ_INSWITCH]:
                        matchspec0 = [
                                hex(tmpoptype),
                                hex(is_cached),
                                ...
                                hex(is_latest)]
                        if is_cached == 1 and validvalue == 1:
                            controller.table_add('access_savedseq_tbl','set_and_get_savedseq',matchspec0)
                    for tmpoptype in [GETRES_LATEST_SEQ_INSWITCH, GETRES_DELETED_SEQ_INSWITCH]:
                        matchspec0 = [
                                hex(tmpoptype),
                                hex(is_cached),
        ...
        for tmpoptype in [GETRES_LATEST_SEQ_INSWITCH_CASE1, GETRES_DELETED_SEQ_INSWITCH_CASE1, PUTREQ_SEQ_INSWITCH_CASE1, DELREQ_SEQ_INSWITCH_CASE1]:
            matchspec0 = [
                    hex(tmpoptype),
                    hex(0)]
            controller.table_add('lastclone_lastscansplit_tbl','set_is_lastclone',matchspec0)
        ...
        for i in range(int(int(switch_max_vallen/8 + 1))): # i from 0 to 16
            ...
            for tmpoptype in [GETRES_SEQ, GETREQ_BEINGEVICTED_RECORD, GETREQ_LARGEVALUEBLOCK_RECORD]:
                matchspec0 = [
                        hex(tmpoptype),
                        ''+hex(vallen_start)+'->'+hex(vallen_end)] # [vallen_start, vallen_end]
                actnspec0 = [hex(val_stat_seq_udplen),hex(val_stat_seq_iplen)]
                controller.table_add('update_pktlen_tbl','update_pktlen',matchspec0, actnspec0, 0) # 0 is priority (range may be overlapping]
            for tmpoptype in [GETRES_LATEST_SEQ_INSWITCH_CASE1, GETRES_DELETED_SEQ_INSWITCH_CASE1, PUTREQ_SEQ_INSWITCH_CASE1, DELREQ_SEQ_INSWITCH_CASE1]:
                matchspec0 = [
                        hex(tmpoptype),
                        ''+hex(vallen_start)+'->'+hex(vallen_end)] # [vallen_start, vallen_end]
                actnspec0 = [hex(val_seq_inswitch_stat_clone_udplen),hex(val_seq_inswitch_stat_clone_iplen)]
                controller.table_add('update_pktlen_tbl','update_pktlen',matchspec0, actnspec0, 0) # 0 is priority (range may be overlapping]
            ...
            matchspec0 = [
                    ...
                    ''+hex(vallen_start)+'->'+hex(vallen_end)] # [vallen_start, vallen_end]
            actnspec0 = [hex(val_seq_inswitch_stat_udplen),hex(val_seq_inswitch_stat_iplen)]
            controller.table_add('update_pktlen_tbl','update_pktlen',matchspec0, actnspec0, 0) # 0 is priority (range may be overlapping]
        ...
        matchspec0 = [
                ...
                ''+hex(0)+'->'+hex(switch_max_vallen)] # [0, 128]
        ...
        for tmpoptype in [PUTREQ_LARGEVALUE_SEQ, PUTREQ_LARGEVALUE_SEQ_CASE3, PUTREQ_LARGEVALUE_SEQ_BEINGEVICTED, PUTREQ_LARGEVALUE_SEQ_CASE3_BEINGEVICTED]:
            matchspec0 = [
                    hex(tmpoptype),
                    ''+hex(0)+'->'+hex(65535)] # [0, 65535] (NOTE: vallen MUST = 0 for PUTREQ_LARGEVALUE_INSWITCH]
            actnspec0 = [hex(shadowtype_seq_udp_delta), hex(shadowtype_seq_ip_delta)]
            controller.table_add('update_pktlen_tbl','add_pktlen',matchspec0, actnspec0, 0) # 0 is priority (range may be overlapping)
        ...
        for tmp_client_physical_idx in range(client_physical_num):
            ...
            for tmpoptype in [GETRES_SEQ, PUTRES_SEQ, DELRES_SEQ, SCANRES_SPLIT, WARMUPACK, LOADACK, GETRES_LARGEVALUE_SEQ]:
                matchspec0 = [
                        hex(tmpoptype), 
                        tmp_devport]
                controller.table_add('update_ipmac_srcport_tbl','update_ipmac_srcport_server2client',matchspec0, actnspec0)
        ...
        for tmpoptype in [GETRES_LATEST_SEQ_INSWITCH_CASE1, GETRES_DELETED_SEQ_INSWITCH_CASE1, PUTREQ_SEQ_INSWITCH_CASE1, DELREQ_SEQ_INSWITCH_CASE1, CACHE_POP_INSWITCH_ACK, CACHE_EVICT_LOADFREQ_INSWITCH_ACK, CACHE_EVICT_LOADDATA_INSWITCH_ACK, LOADSNAPSHOTDATA_INSWITCH_ACK, SETVALID_INSWITCH_ACK]:
            matchspec0 = [
                    hex(tmpoptype),
                    tmp_devport]
            controller.table_add('update_ipmac_srcport_tbl','update_ipmac_srcport_switch2switchos',matchspec0, actnspec2)
        ...
        for tmpoptype in [PUTREQ_SEQ, PUTREQ_POP_SEQ, PUTREQ_SEQ_CASE3, PUTREQ_POP_SEQ_CASE3, GETRES_LATEST_SEQ_INSWITCH_CASE1, GETRES_DELETED_SEQ_INSWITCH_CASE1, PUTREQ_SEQ_INSWITCH_CASE1, DELREQ_SEQ_INSWITCH_CASE1, GETRES_SEQ, CACHE_EVICT_LOADDATA_INSWITCH_ACK, LOADSNAPSHOTDATA_INSWITCH_ACK, PUTREQ_SEQ_BEINGEVICTED, PUTREQ_SEQ_CASE3_BEINGEVICTED, GETREQ_BEINGEVICTED_RECORD, GETREQ_LARGEVALUEBLOCK_RECORD]:
            for i in range(int(switch_max_vallen/8 + 1)): # i from 0 to 16
                ...
                matchspec0 = [
                        ...
                        ''+hex(vallen_start)+'->'+hex(vallen_end)] # [vallen_start, vallen_end]
                if i == 0:
                    controller.table_add('add_and_remove_value_header_tbl','add_only_vallen',matchspec0,[] , 0)
                else:
                    controller.table_add('add_and_remove_value_header_tbl','add_to_val{}'.format(i),matchspec0,[], 0)
                                    ...
                                    for snapshot_flag in snapshot_flag_list:
                                        for is_case1 in case1_list:
                                            ...
                                            if is_lastclone_for_pktloss == 0 and snapshot_flag == 0 and is_case1 == 0 and tmp_client_sid != 0:
                                                ...
                                                if is_cached == 0:
                                                    if is_hot == 1:
                                                        ...
                                                        controller.table_add('another_eg_port_forward_tbl','update_getreq_inswitch_to_getreq_pop',matchspec0)
                                                    else:
                                                        # Update GETREQ_INSWITCH as GETREQ to server
                                                        
                                                        controller.table_add('another_eg_port_forward_tbl','update_getreq_inswitch_to_getreq',matchspec0)
                                                else:
                                                    if validvalue == 0:
                                                        # Update GETREQ_INSWITCH as GETREQ to server
                                                        ...
                                                        if is_latest == 0:
                                                            ...
                                                            controller.table_add('another_eg_port_forward_tbl','update_getreq_inswitch_to_getreq_nlatest',matchspec0)
                                                        else:
                                                            # Update GETREQ_INSWITCH as GETRES_SEQ to client by mirroring
                                                            actnspec0 = [hex(tmp_client_sid), hex(server_worker_port_start), hex(tmpstat)]
                                                            controller.table_add('another_eg_port_forward_tbl','update_getreq_inswitch_to_getres_seq_by_mirroring',matchspec0, actnspec0)
                                                    elif validvalue == 3:
                                                        if is_latest == 0:
                                                            ...
                                                            controller.table_add('another_eg_port_forward_tbl','update_getreq_inswitch_to_getreq_beingevicted_record',matchspec0, actnspec0)
                                                        else:
                                                            # Update GETREQ_INSWITCH as GETRES_SEQ to client by mirroring
                                                            actnspec0 = [hex(tmp_client_sid), hex(server_worker_port_start), hex(tmpstat)]
                                                            controller.table_add('another_eg_port_forward_tbl','update_getreq_inswitch_to_getres_seq_by_mirroring',matchspec0, actnspec0)
                                            ...
                                            if is_cached == 0 and is_hot == 0 and validvalue == 0 and is_latest == 0 and is_deleted == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0 and snapshot_flag == 0 and is_case1 == 0:
                                                ...
                                                controller.table_add('another_eg_port_forward_tbl','update_getres_latest_seq_to_getres_seq',matchspec0)
                                            ...
                                            if is_hot == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0:
                                                ...
                                                if is_cached == 1 and validvalue == 1 and is_latest == 0 and snapshot_flag == 1 and is_case1 == 0:
                                                    ...
                                                    if is_deleted == 0: # is_deleted=0 -> stat=1
                                                        ...
                                                        actnspec0 = [hex(self.reflector_sid), hex(1), hex(reflector_dp2cpserver_port)]
                                                    elif is_deleted == 1: # is_deleted=1 -> stat=0
                                                        
                                                        actnspec0 = [hex(self.reflector_sid), hex(0), hex(reflector_dp2cpserver_port)]
                                                    controller.table_add('another_eg_port_forward_tbl','update_getres_latest_seq_inswitch_to_getres_latest_seq_inswitch_case1_clone_for_pktloss',matchspec0, actnspec0)
                                            ...
                                            if is_cached == 1 and snapshot_flag == 1 and is_hot == 0 and validvalue == 0 and is_latest == 0 and is_deleted == 0 and tmp_client_sid == 0 and is_case1 == 0:
                                                matchspec0 = [
                                                    hex(GETRES_LATEST_SEQ_INSWITCH_CASE1),
                                                    hex(is_cached),
                                                    hex(is_hot),
                                                    hex(validvalue),
                                                    hex(is_latest),
                                                ...
                                                if is_lastclone_for_pktloss == 0:
                                                    ...
                                                    controller.table_add('another_eg_port_forward_tbl','forward_getres_latest_seq_inswitch_case1_clone_for_pktloss',matchspec0, actnspec0)
                                            ...
                                            if is_cached == 0 and is_hot == 0 and validvalue == 0 and is_latest == 0 and is_deleted == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0 and snapshot_flag == 0 and is_case1 == 0:
                                                ...
                                                controller.table_add('another_eg_port_forward_tbl','update_getres_deleted_seq_to_getres_seq',matchspec0)
                                            ...
                                            if is_hot == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0:
                                                ...
                                                if is_cached == 1 and validvalue == 1 and is_latest == 0 and snapshot_flag == 1 and is_case1 == 0:
                                                    ...
                                                    if is_deleted == 0: # is_deleted=0 -> stat=1
                                                        ...
                                                        actnspec0 = [hex(self.reflector_sid), hex(0), hex(reflector_dp2cpserver_port)]
                                                    controller.table_add('another_eg_port_forward_tbl','update_getres_deleted_seq_inswitch_to_getres_deleted_seq_inswitch_case1_clone_for_pktloss',matchspec0, actnspec0)
                                            ...
                                            if is_cached == 1 and snapshot_flag == 1 and is_hot == 0 and validvalue == 0 and is_latest == 0 and is_deleted == 0 and tmp_client_sid == 0 and is_case1 == 0:
                                                matchspec0 = [
                                                    hex(GETRES_DELETED_SEQ_INSWITCH_CASE1),
                                                    hex(is_cached),
                                                    hex(is_hot),
                                                    hex(validvalue),
                                                    hex(is_latest),
                                                ...
                                                if is_lastclone_for_pktloss == 0:
                                                    ...
                                                    actnspec0 = [hex(self.reflector_sid)]
                                                    controller.table_add('another_eg_port_forward_tbl','forward_getres_deleted_seq_inswitch_case1_clone_for_pktloss',matchspec0, actnspec0)
                                                elif is_lastclone_for_pktloss == 1:
                                            ...
                                            if is_hot == 0 and is_deleted == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0 and is_case1 == 0:
                                                ...
                                                if is_cached == 1 and validvalue == 3:
                                                    if snapshot_flag == 0:
                                                        ...
                                                        controller.table_add('another_eg_port_forward_tbl','update_putreq_largevalue_inswitch_to_putreq_largevalue_seq_beingevicted',matchspec0)
                                                    elif snapshot_flag == 1:
                                                        # Update PUTREQ_LARGEVALUE_INSWITCH as PUTREQ_LARGEVALUE_SEQ_CASE3_BEINGEVICTED to server
                                                        controller.table_add('another_eg_port_forward_tbl','update_putreq_largevalue_inswitch_to_putreq_largevalue_seq_case3_beingevicted',matchspec0)
                                                else:
                                                    if snapshot_flag == 0:
                                                        # Update PUTREQ_LARGEVALUE_INSWITCH as PUTREQ_LARGEVALUE_SEQ to server
                                                        controller.table_add('another_eg_port_forward_tbl','update_putreq_largevalue_inswitch_to_putreq_largevalue_seq',matchspec0)
                                                    elif snapshot_flag == 1:
                                                        # Update PUTREQ_LARGEVALUE_INSWITCH as PUTREQ_LARGEVALUE_SEQ_CASE3 to server
                                                        controller.table_add('another_eg_port_forward_tbl','update_putreq_largevalue_inswitch_to_putreq_largevalue_seq_case3',matchspec0)
                                    ...
                                    for is_lastclone_for_pktloss in lastclone_list:
                                        for snapshot_flag in snapshot_flag_list:
                                            for is_case1 in case1_list:
                                                        ...
                                                        if validvalue == 0:
                                                            ...
                                                            if is_latest == 0:
                                                                if is_largevalueblock == 1:
                                                                    ...
                                                                    actnspec0 = [hex(tmpstat)]
                                                                    controller.table_add('another_eg_port_forward_tbl','update_getreq_inswitch_to_getreq_largevalueblock_record',matchspec0, actnspec0)
                                                ...
                                                # size = 1
                                                #if is_cached == 0 and is_hot == 0 and validvalue == 0 and is_latest == 0 and is_deleted == 0 and is_wrong_pipeline == 0 and is_lastclone_for_pktloss == 0 and snapshot_flag == 0 and is_case1 == 0:
                                                if is_cached == 0 and is_hot == 0 and validvalue == 0 and is_latest == 0 and is_largevalueblock == 0 and is_deleted == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0 and snapshot_flag == 0 and is_case1 == 0:
                                                    matchspec0 = [
                                                        hex(GETRES_LATEST_SEQ),
                                                        hex(is_cached),
                                                        hex(is_hot),
                                                ...
                                                # size = 128 -> 2
                                                #if is_hot == 0 and is_wrong_pipeline == 0 and is_lastclone_for_pktloss == 0:
                                                if is_hot == 0 and is_largevalueblock == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0:
                                                    matchspec0 = [
                                                        hex(GETRES_LATEST_SEQ_INSWITCH),
                                                        hex(is_cached),
                                                        hex(is_hot),
                                                ...
                                                # size = 1
                                                #if is_cached == 1 and is_wrong_pipeline == 0 and snapshot_flag == 1 and is_hot == 0 and validvalue == 0 and is_latest == 0 and is_deleted == 0 and is_case1 == 0:
                                                if is_cached == 1 and snapshot_flag == 1 and is_hot == 0 and validvalue == 0 and is_latest == 0 and is_largevalueblock == 0 and is_deleted == 0 and tmp_client_sid == 0 and is_case1 == 0:
                                                    matchspec0 = [
                                                        hex(GETRES_LATEST_SEQ_INSWITCH_CASE1),
                                                        hex(is_cached),
                                                        hex(is_hot),
                                                ...
                                                # NOTE: validvalue can be 0/1/3 for PUTREQ_LARGEVALUE_INSWITCH
                                                # size = 32
                                                if is_hot == 0 and is_largevalueblock == 0 and is_deleted == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0 and is_case1 == 0:
                                                    matchspec0 = [
                                                        hex(PUTREQ_LARGEVALUE_INSWITCH),
                                                        hex(is_cached),
                                                        hex(is_hot),
                                    ...
                                    for snapshot_flag in snapshot_flag_list:
                                        for is_case1 in case1_list:
                                            ...
                                            if is_cached == 0 and is_hot == 0 and validvalue == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0 and snapshot_flag == 0 and is_case1 == 0:
                                                ...
                                                actnspec0 = [hex(self.reflector_sid), hex(reflector_dp2cpserver_port)]
                                                controller.table_add('eg_port_forward_tbl','update_cache_pop_inswitch_to_cache_pop_inswitch_ack_drop_and_clone',matchspec0, actnspec0)
                                                ...
                                                if is_cached == 0:
                                                    if snapshot_flag == 1:
                                                        if is_hot == 1:
                                                            ...
                                                            controller.table_add('eg_port_forward_tbl','update_putreq_inswitch_to_putreq_pop_seq_case3',matchspec0)
                                                        elif is_hot == 0:
                                                            # Update PUTREQ_INSWITCH as PUTREQ_SEQ_CASE3 to server
                                                            controller.table_add('eg_port_forward_tbl','update_putreq_inswitch_to_putreq_seq_case3',matchspec0)
                                                    elif snapshot_flag == 0:
                                                        if is_hot == 1:
                                                            # Update PUTREQ_INSWITCH as PUTREQ_POP_SEQ to server
                                                            controller.table_add('eg_port_forward_tbl','update_putreq_inswitch_to_putreq_pop_seq',matchspec0)
                                                        elif is_hot == 0:
                                                            # Update PUTREQ_INSWITCH as PUTREQ_SEQ to server
                                                            controller.table_add('eg_port_forward_tbl','update_putreq_inswitch_to_putreq_seq',matchspec0)
                                                elif is_cached == 1:
                                                    if validvalue == 0:
                                                        if snapshot_flag == 1:
                                                            # Update PUTREQ_INSWITCH as PUTREQ_SEQ_CASE3 to server
                                                        ...
                                                        if snapshot_flag == 1:
                                                            ...
                                                            controller.table_add('eg_port_forward_tbl','update_putreq_inswitch_to_putreq_seq_case3_beingevicted',matchspec0)
                                                        elif snapshot_flag == 0:
                                                            # Update PUTREQ_INSWITCH as PUTREQ_SEQ_BEINGEVICTED to server
                                                            controller.table_add('eg_port_forward_tbl','update_putreq_inswitch_to_putreq_seq_beingevicted',matchspec0)
                                                    elif validvalue == 1:
                                                        if snapshot_flag == 1 and is_case1 == 0:
                                                            ...
                                                            if is_deleted == 0: # is_deleted=0 -> stat=1
                                                                ...
                                                                actnspec0 = [hex(self.reflector_sid), hex(0), hex(reflector_dp2cpserver_port)]
                                                            controller.table_add('eg_port_forward_tbl','update_putreq_inswitch_to_putreq_seq_inswitch_case1_clone_for_pktloss_and_putres',matchspec0, actnspec0)
                                                        else:
                                                            # Update PUTREQ_INSWITCH as PUTRES_SEQ to client by mirroring
                                                            actnspec0 = [hex(tmp_client_sid), hex(server_worker_port_start)]
                                                            controller.table_add('eg_port_forward_tbl','update_putreq_inswitch_to_putres_seq_by_mirroring',matchspec0, actnspec0)
                                            ...
                                            if is_cached == 1 and is_hot == 0 and validvalue == 0 and is_latest == 0 and is_deleted == 0 and snapshot_flag == 1 and is_case1 == 0:
                                                ...
                                                if is_lastclone_for_pktloss == 0:
                                                    ...
                                                    actnspec0 = [hex(self.reflector_sid)]
                                                    controller.table_add('eg_port_forward_tbl','forward_putreq_seq_inswitch_case1_clone_for_pktloss_and_putres',matchspec0, actnspec0)
                                                elif is_lastclone_for_pktloss == 1:
                                                    # Update PUTREQ_SEQ_INSWITCH_CASE1 as PUTRES_SEQ to client by mirroring
                                                    actnspec0 = [hex(tmp_client_sid), hex(server_worker_port_start)]
                                                    controller.table_add('eg_port_forward_tbl','update_putreq_seq_inswitch_case1_to_putres_seq_by_mirroring',matchspec0, actnspec0)
                                            ...
                                            if is_hot == 0 and is_lastclone_for_pktloss == 0 and tmp_client_sid != 0:
                                                ...
                                                if is_cached == 0:
                                                    if snapshot_flag == 1:
                                                        ...
                                                        controller.table_add('eg_port_forward_tbl','update_delreq_inswitch_to_delreq_seq_case3',matchspec0)
                                                    elif snapshot_flag == 0:
                                                        # Update DELREQ_INSWITCH as DELREQ_SEQ to server
                                                        controller.table_add('eg_port_forward_tbl','update_delreq_inswitch_to_delreq_seq',matchspec0)
                                                elif is_cached == 1:
                                                    if validvalue == 0:
                                                        if snapshot_flag == 1:
                                                            # Update DELREQ_INSWITCH as DELREQ_SEQ_CASE3 to server
                                                        ...
                                                        if snapshot_flag == 1:
                                                            ...
                                                            controller.table_add('eg_port_forward_tbl','update_delreq_inswitch_to_delreq_seq_case3_beingevicted',matchspec0)
                                                        elif snapshot_flag == 0:
                                                            # Update DELREQ_INSWITCH as DELREQ_SEQ_BEINGEVICTED to server
                                                            controller.table_add('eg_port_forward_tbl','update_delreq_inswitch_to_delreq_seq_beingevicted',matchspec0)
                                                    elif validvalue == 1:
                                                        if snapshot_flag == 1 and is_case1 == 0:
                                                            ...
                                                            if is_deleted == 0: # is_deleted=0 -> stat=1
                                                                ...
                                                                actnspec0 = [hex(self.reflector_sid), hex(0), hex(reflector_dp2cpserver_port)]
                                                            controller.table_add('eg_port_forward_tbl','update_delreq_inswitch_to_delreq_seq_inswitch_case1_clone_for_pktloss_and_delres',matchspec0, actnspec0)
                                                        else:
                                                            # Update DELREQ_INSWITCH as DELRES_SEQ to client by mirroring
                                                            actnspec0 = [hex(tmp_client_sid), hex(server_worker_port_start)]
                                                            controller.table_add('eg_port_forward_tbl','update_delreq_inswitch_to_delres_seq_by_mirroring',matchspec0, actnspec0)
                                            ...
                                            if is_cached == 1 and is_hot == 0 and validvalue == 0 and is_latest == 0 and is_deleted == 0 and snapshot_flag == 1 and is_case1 == 0:
                                                ...
                                                if is_lastclone_for_pktloss == 0:
                                                    ...
                                                    actnspec0 = [hex(self.reflector_sid)]
                                                    controller.table_add('eg_port_forward_tbl','forward_delreq_seq_inswitch_case1_clone_for_pktloss_and_delres',matchspec0, actnspec0)
                                                elif is_lastclone_for_pktloss == 1:
                                                    # Update DELREQ_SEQ_INSWITCHCASE1 as DELRES_SEQ to client by mirroring
                                                    actnspec0 = [hex(tmp_client_sid), hex(server_worker_port_start)]
                                                    controller.table_add('eg_port_forward_tbl','update_delreq_seq_inswitch_case1_to_delres_seq_by_mirroring',matchspec0, actnspec0)
                                            ...
                                            if is_cached == 1 and is_hot == 0 and validvalue == 0 and is_latest == 0 and is_deleted == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0 and snapshot_flag == 0 and is_case1 == 0:
                                                ...
                                                actnspec0 = [hex(self.reflector_sid), hex(reflector_dp2cpserver_port)]
                                                controller.table_add('eg_port_forward_tbl','update_cache_evict_loadfreq_inswitch_to_cache_evict_loadfreq_inswitch_ack_drop_and_clone',matchspec0, actnspec0)
                                            ...
                                            if is_cached == 1 and is_hot == 0 and validvalue == 0 and is_latest == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0 and snapshot_flag == 0 and is_case1 == 0:
                                                ...
                                                actnspec0 = [hex(self.reflector_sid), hex(reflector_dp2cpserver_port), hex(tmpstat)]
                                                controller.table_add('eg_port_forward_tbl','update_cache_evict_loaddata_inswitch_to_cache_evict_loaddata_inswitch_ack_drop_and_clone',matchspec0, actnspec0)
                                            ...
                                            if is_hot == 0 and validvalue == 0 and is_latest == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0 and snapshot_flag == 0 and is_case1 == 0:
                                                ...
                                                actnspec0 = [hex(self.reflector_sid), hex(reflector_dp2cpserver_port), hex(tmpstat)]
                                                controller.table_add('eg_port_forward_tbl','update_loadsnapshotdata_inswitch_to_loadsnapshotdata_inswitch_ack_drop_and_clone',matchspec0, actnspec0)
                                            ...
                                            if is_hot == 0 and is_latest == 0 and is_deleted == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0 and snapshot_flag == 0 and is_case1 == 0:
                                                ...
                                                actnspec0 = [hex(self.reflector_sid), hex(reflector_dp2cpserver_port)]
                                                controller.table_add('eg_port_forward_tbl','update_setvalid_inswitch_to_setvalid_inswitch_ack_drop_and_clone',matchspec0, actnspec0)
                                                ...
                                                for tmp_server_sid in tmp_server_sids:
                                                    ...
                                                    if is_cached == 0 and is_hot == 0 and validvalue == 0 and is_latest == 0 and is_deleted == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0 and snapshot_flag == 0 and is_case1 == 0 and tmp_server_sid != 0:
                                                        ...
                                                        if is_last_scansplit == 1:
                                                            controller.table_add('eg_port_forward_tbl','forward_scanreq_split',matchspec0)
                                                        elif is_last_scansplit == 0:
                                                            actnspec0 = [hex(tmp_server_sid)]
                                                            controller.table_add('eg_port_forward_tbl','forward_scanreq_split_and_clone',matchspec0, actnspec0)
                                                    # is_cached=1 (key must be cached in cache_lookup_tbl for CACHE_EVICT_LOADFREQ_INSWITCH), is_hot (cm_predicate=1), validvalue, is_latest, is_deleted, is_wrong_pipeline, tmp_client_sid=0, is_lastclone_for_pktloss, snapshot_flag, is_case1 should be 0 for CACHE_EVICT_LOADFREQ_INSWITCH
                                                    # NOTE: is_cached must be 1 (CACHE_EVCIT must match an entry in cache_lookup_tbl)
                                                    # size = 1
