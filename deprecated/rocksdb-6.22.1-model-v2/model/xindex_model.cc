...
void VarlenLinearModel::prepare(const std::vector<std::string> &keys,
  ...
  for (uint32_t i = 0; i < keys.size(); i++) {
    ...
    rocksdb::Slice(keys[i]).to_model_key(model_keys[i].data(), max_key_len);
...
void VarlenLinearModel::prepare(
  ...
  for (size_t i = 0; i < size; i++) {
	model_keys[i].resize(max_key_len);
	rocksdb::Slice(*(keys_begin + i)).to_model_key(model_keys[i].data(), max_key_len);
    key_ptrs[i] = model_keys[i].data();
    positions[i] = i;
  }
...
void VarlenLinearModel::prepare_model(
    const std::vector<double *> &model_key_ptrs,
    const std::vector<size_t> &positions) {
  if (positions.size() == 0) return;
  if (positions.size() == 1) {
    VarlenLinearModel::weights[max_key_len] = positions[0];
    return;
  }
...
size_t VarlenLinearModel::predict(const rocksdb::Slice &curkey) const {
  if (unlikely(weights == nullptr)) return 0;
...
uint32_t VarlenLinearModel::get_max_key_len(
		const std::vector<std::string> &keys) {
	uint32_t result = 0;
	uint32_t tmp_len = 0;
	for (size_t key_i = 0; key_i < keys.size(); key_i++) {
		tmp_len = rocksdb::Slice(keys[key_i]).get_key_len();
		if (tmp_len > result) {
			result = tmp_len;
		}
	}
...
uint32_t VarlenLinearModel::get_max_key_len(
    const typename std::vector<std::string>::const_iterator &keys_begin,
	uint32_t size) {
	typename std::vector<std::string>::const_iterator iter = keys_begin;
	uint32_t result = 0;
	uint32_t tmp_len = 0;
	for (size_t i = 0; i < size; i++) {
		tmp_len = rocksdb::Slice(*iter).get_key_len();
		if (tmp_len > result) {
			result = tmp_len;
		}
		iter++;
...
size_t VarlenLinearModel::get_error_bound(
    const std::vector<std::string> &keys, const std::vector<size_t> &positions) {
  int max = 0;

  for (size_t key_i = 0; key_i < keys.size(); ++key_i) {
    long long int pos_actual = positions[key_i];
    long long int pos_pred = predict(rocksdb::Slice(keys[key_i]));
...
/*
size_t VarlenLinearModel::get_error_bound(
    const std::vector<std::string> &keys, const std::vector<size_t> &positions, const uint32_t limit) {
  int max = 0;
...
size_t VarlenLinearModel::get_error_bound(
  ...
  for (size_t key_i = 0; key_i < size; ++key_i) {
    ...
    long long int pos_pred = predict(rocksdb::Slice(*(keys_begin + key_i)));
    int error = std::abs(pos_actual - pos_pred);
