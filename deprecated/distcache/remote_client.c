...
//#endif
		...
		while (true) { // timeout-and-retry mechanism
			...
			if (tmptype == optype_t(packet_type_t::GETREQ)) { // get
				...
				get_request_t req(CURMETHOD_ID, tmp_spineswitchidx, tmp_leafswitchidx, tmpkey);
				...
				if ((getres_cnt % update_trafficload_threshold) == 0) {
					...
					distcache_update_trafficload_t tmp_distcache_update_trafficload(CURMETHOD_ID, tmp_rsp_spineswitchidx, tmp_rsp_leafswitchidx, tmpkey, tmp_rsp_spineload, tmp_rsp_leafload);
				...
				is_timeout = udprecvlarge_multisrc_ipfrag_dist(CURMETHOD_ID, client_udpsock_list[local_client_logical_idx], perswitch_perserver_scanbufs, 0, perswitch_perserver_addrs, perswitch_perserver_addrlens, "remote_client.worker", true, optype_t(packet_type_t::SCANRES_SPLIT), tmpkey);
				CUR_TIME(wait_t1);
