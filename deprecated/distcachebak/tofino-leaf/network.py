...
def create_network():
    ...
    #         )
    #     )
    leafswitch = net.addSwitch(
        "p4_sleaf",
        cls=P4Switch,
        json_path="distcacheleaf.json",
        thrift_port=9090,
        pcap_dump=debug,
        pcap_dir="./pcap",
        log_enabled=debug,
    ...
    spineswitch = net.addSwitch(
        "p4_sspine",
        cls=P4Switch,
        json_path="../tofino-spine/distcachespine.json",
        thrift_port=9091,
        pcap_dump=debug,
        pcap_dir="./pcap",
        log_enabled=debug,
    ...
    for i in range(rack_physical_num * 2):
        switchoses.append(
            net.addHost(
                "switchos{}".format(1 + i),
    ...
    net.addLink(spineswitch, leafswitch)
    for i in range(client_physical_num + server_physical_num):
        net.addLink(host[i], leafswitch)
    # for i in range(rack_physical_num):
