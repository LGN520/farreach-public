...
ssh ${USER}@dl16 "cd ${SERVER_ROOTPATH}/${DIRNAME}; rm tmp.out"

echo "[part 1] run single bottleneck server thread"

echo "stop servers"
ssh ${USER}@dl13 "cd ${SERVER_ROOTPATH}/${DIRNAME}; bash localstop.sh server >/dev/null 2>&1"
ssh ${USER}@dl16 "cd ${SERVER_ROOTPATH}/${DIRNAME}; bash localstop.sh server >/dev/null 2>&1"
echo "stop clients"
bash localstop.sh client >/dev/null 2>&1
ssh ${USER}@${SECONDARY_CLIENT} "cd ${CLIENT_ROOTPATH}/${DIRNAME}; bash localstop.sh client >/dev/null 2>&1"
sleep 1s
...
echo "start servers"
ssh ${USER}@dl13 "cd ${SERVER_ROOTPATH}/${DIRNAME}; nohup ./server 0 >tmp0.out 2>&1 &"
sleep 5s
...
do
	...
	echo "stop servers"
	ssh ${USER}@dl13 "cd ${SERVER_ROOTPATH}/${DIRNAME}; bash localstop.sh server >/dev/null"
	ssh ${USER}@dl16 "cd ${SERVER_ROOTPATH}/${DIRNAME}; bash localstop.sh server >/dev/null"
	echo "stop clients"
	bash localstop.sh client >/dev/null 2>&1
	ssh ${USER}@${SECONDARY_CLIENT} "cd ${CLIENT_ROOTPATH}/${DIRNAME}; bash localstop.sh client >/dev/null"
	sleep 1s
	...
	echo "start servers"
	ssh ${USER}@dl13 "cd ${SERVER_ROOTPATH}/${DIRNAME}; nohup ./server 0 >>tmp.out 2>&1 &"
	ssh ${USER}@dl16 "cd ${SERVER_ROOTPATH}/${DIRNAME}; nohup ./server 1 >>tmp.out 2>&1 &"
	sleep 5s
