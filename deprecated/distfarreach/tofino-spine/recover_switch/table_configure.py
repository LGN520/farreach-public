...
class RegisterUpdate(pd_base_tests.ThriftInterfaceDataPlane):
    ...
    def runTest(self):
        ...
        snapshotid_path = "/tmp/distfarreachspine/controller.snapshotid"
        with open(snapshotid_path, "rb") as f:
            buf = f.read(4)
            snapshotid = struct.unpack("=i", buf)[0]

        # extract snapshot data
        snapshotdata_path = "/tmp/distfarreachspine/controller.snapshotdata{}".format(snapshotid)
        key_list_list = [] # n*5
        vallen_list = [] # n
        ...
        for idx in range(n):
            ...
            matchspec0 = distfarreachspine_cache_lookup_tbl_match_spec_t(\
            ...
            actnspec0 = distfarreachspine_cached_action_action_spec_t(idx)
