...
class RegisterUpdate(pd_base_tests.ThriftInterfaceDataPlane):
    ...
    def runTest(self):
        #flags = distfarreachlimitspine_register_flags_t(read_hw_sync=True)

        snapshotid_path = "/tmp/distfarreachlimitspine/controller.snapshotid"
        with open(snapshotid_path, "rb") as f:
            buf = f.read(4)
            snapshotid = struct.unpack("=i", buf)[0]

        # extract snapshot data
        snapshotdata_path = "/tmp/distfarreachlimitspine/controller.snapshotdata{}".format(snapshotid)
        key_list_list = [] # n*5
        vallen_list = [] # n
        ...
        for idx in range(n):
            ...
            matchspec0 = distfarreachlimitspine_cache_lookup_tbl_match_spec_t(\
            ...
            actnspec0 = distfarreachlimitspine_cached_action_action_spec_t(idx)
