...
action set_ratelimitload_threshold(threshold) {
	modify_field(meta.ratelimitload_threshold, threshold);
}

@pragma stage 0
table ratelimitload_threshold_tbl {
	actions {
		set_ratelimitload_threshold;
	}
	// 3.2 GOPS / 100 switches = 32 MOPS
	default_action: set_ratelimitload_threshold(32000000);
	size: 1;
}
...
action drop_for_ratelimit() {
	drop();
}

@pragma stage 7
table drop_for_ratelimit_tbl {
	reads {
		op_hdr.optype: exact;
		meta.ratelimitload_predicate: exact;
	}
	actions {
		drop_for_ratelimit;
		nop;
