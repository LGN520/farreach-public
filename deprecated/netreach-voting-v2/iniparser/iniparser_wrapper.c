...
short IniparserWrapper::get_server_populator_port() {
	int tmp = iniparser_getint(ini, "server:server_populator_port", -1);
	if (tmp == -1) {
		printf("Invalid entry of [server:server_populator_port]: %d\n", tmp);
		exit(-1);
	}
	return short(tmp);
}
