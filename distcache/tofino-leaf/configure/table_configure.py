...
#                                                    self.sess_hdl, self.dev_tgt, matchspec0)
                                        ...
                                        # size: 4
                                        if is_report == 0 and is_hot == 0 and is_deleted == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0 and tmp_server_sid == 0:
                                            matchspec0 = distcacheleaf_eg_port_forward_tbl_match_spec_t(\
...
#                                                self.client.eg_port_forward_tbl_table_add_with_update_distcache_leaf_valueupdate_inswitch_to_distcache_leaf_valueupdate_inswitch_ack(\
#                                                        self.sess_hdl, self.dev_tgt, matchspec0)
                                            # is_hot (cm_predicate=1), is_report, is_deleted, tmp_client_sid=0, is_lastclone_for_pktloss, tmp_server_sid, is_last_scansplit should be 0 for PUTREQ_LARGEVALUE_SEQ_INSWITCH
                                            # size: 4
                                            if is_report == 0 and is_hot == 0 and is_deleted == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0 and tmp_server_sid == 0 and is_last_scansplit == 0:
                                                matchspec0 = distcacheleaf_eg_port_forward_tbl_match_spec_t(\
                                                    op_hdr_optype = PUTREQ_LARGEVALUE_SEQ_INSWITCH,
                                                    inswitch_hdr_is_cached = is_cached,
                                                    meta_is_hot = is_hot,
