...
void* run_reflector_dp2cpserver(void* param) {
    ...
    std::vector<struct sockaddr_in> spineswitchos_popserver_addrs;
    std::vector<socklen_t> spineswitchos_popserver_addrlens;
    for (auto p : spineswitchos_ips) {
        struct sockaddr_in tmp_addr;
        set_sockaddr(tmp_addr, inet_addr(p), switchos_popserver_port);
        socklen_t tmp_addrlen = sizeof(struct sockaddr_in);
        spineswitchos_popserver_addrs.push_back(tmp_addr);
        spineswitchos_popserver_addrlens.push_back(tmp_addrlen);
    }

    printf("[reflector.dp2cpserver] ready %s\n", switchos_ip);
    transaction_ready_threads++;
    ...
    while (transaction_running) {
        ...
        // printf("[run_reflector_dp2cpserver] recv %d\n", recvsize);
        if (is_timeout) {
            continue;
        }
        INVARIANT(recvsize > 0);
        ...
        switch (pkt_type) {
            ...
            case packet_type_t::NETCACHE_GETREQ_POP:{
                // printf("recieved NETCACHE_GETREQ_POP\n");
            }
            case packet_type_t::NETCACHE_WARMUPREQ_INSWITCH_POP: {
                ...
                uint32_t tmp_spineswitchidx = get_packet_key(CURMETHOD_ID, buf, recvsize).get_spineswitch_idx(switch_partition_count,server_physical_num/2);
                // COUT_VAR(tmp_spineswitchidx);
                udpsendto(reflector_dp2cpserver_popclient_udpsock, buf, recvsize, 0, &spineswitchos_popserver_addrs[tmp_spineswitchidx], spineswitchos_popserver_addrlens[tmp_spineswitchidx], "reflector.dp2cpserver.popclient");
