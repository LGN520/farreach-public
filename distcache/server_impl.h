		...
		case packet_type_t::PUTREQ_LARGEVALUE_SEQ:
			{
				...
				if (unlikely(is_cached)) { // already cached
					...
					// NOTE: PUTREQ_LARGEVALUE_SEQ does NOT need to update in-switch value via phase 2
#ifdef DEBUG_SERVER
		...
		case packet_type_t::PUTREQ_LARGEVALUE_SEQ_CACHED:
			{
				...
				if (likely(is_cached)) { // already cached
					...
					// Phase 2 of cache coherence: update in-switch value in background
					// NOTE: PUTREQ_LARGEVALUE_SEQ_CACHED does NOT need to update in-switch value via phase 2
#ifdef DEBUG_SERVER
					CUR_TIME(beingupdated_t1);
#endif
