...
from distcache.p4_pd_rpc.ttypes import *
...
class TableConfigure(pd_base_tests.ThriftInterfaceDataPlane):
    def __init__(self):
        ...
        pd_base_tests.ThriftInterfaceDataPlane.__init__(self, ["distcache"])

    def configure_update_val_tbl(self, valname):
#        # size: 30
#        for is_cached in cached_list:
#            for is_latest in latest_list:
#                matchspec0 = eval("distcache_update_val{}_tbl_match_spec_t".format(valname))(
...
#            self.mirror.mirror_session_create(self.sess_hdl, self.dev_tgt, info)
            ...
            for i in range(client_physical_num):
                matchspec0 = distcache_l2l3_forward_tbl_match_spec_t(\
                ...
                actnspec0 = distcache_l2l3_forward_action_spec_t(self.client_devports[i])
            ...
            for i in range(server_physical_num):
                matchspec0 = distcache_l2l3_forward_tbl_match_spec_t(\
                        ...
                        ipv4_hdr_dstAddr_prefix_length = 32)
                actnspec0 = distcache_l2l3_forward_action_spec_t(self.server_devports[i])
                self.client.l2l3_forward_tbl_table_add_with_l2l3_forward(\
                        self.sess_hdl, self.dev_tgt, matchspec0, actnspec0)

            # Table: set_hot_threshold_tbl (default: set_hot_threshold; size: 1)
            print "Configuring set_hot_threshold_tbl"
            actnspec0 = distcache_set_hot_threshold_action_spec_t(hot_threshold)
            ...
            if RANGE_SUPPORT == False:
                ...
                for tmpoptype in [GETREQ, CACHE_POP_INSWITCH, PUTREQ, DELREQ, WARMUPREQ, LOADREQ, CACHE_EVICT_LOADFREQ_INSWITCH, SETVALID_INSWITCH]:
                    matchspec0 = distcache_hash_for_partition_tbl_match_spec_t(\
            ...
            if RANGE_SUPPORT == True:
                ...
                for tmpoptype in [GETREQ, CACHE_POP_INSWITCH, PUTREQ, DELREQ, WARMUPREQ, SCANREQ, LOADREQ, CACHE_EVICT_LOADFREQ_INSWITCH, SETVALID_INSWITCH]:
                    ...
                    for global_server_logical_idx in range(server_total_logical_num):
                        ...
                        matchspec0 = distcache_range_partition_tbl_match_spec_t(\
                                op_hdr_optype = tmpoptype,
                        ...
                        if server_physical_idx == -1:
                            ...
                            if tmpoptype != SCANREQ:
                                actnspec0 = distcache_range_partition_action_spec_t(udp_dstport, eport)
                                ...
                                actnspec0 = distcache_range_partition_for_scan_action_spec_t(udp_dstport, eport, global_server_logical_idx)
                ...
                for tmpoptype in [GETREQ, CACHE_POP_INSWITCH, PUTREQ, DELREQ, WARMUPREQ, LOADREQ, CACHE_EVICT_LOADFREQ_INSWITCH, SETVALID_INSWITCH]:
                    ...
                    for global_server_logical_idx in range(server_total_logical_num):
                        ...
                        # NOTE: both start and end are included
                        matchspec0 = distcache_hash_partition_tbl_match_spec_t(\
                                op_hdr_optype = convert_u16_to_i16(tmpoptype),
                        ...
                        if server_physical_idx == -1:
                            ...
                            udp_dstport = server_worker_port_start + local_server_logical_idx
                            eport = self.server_devports[server_physical_idx]
                            actnspec0 = distcache_hash_partition_action_spec_t(udp_dstport, eport)
            ...
            if RANGE_SUPPORT == True:
                ...
                for global_server_logical_idx in range(server_total_logical_num):
                    ...
                    # NOTE: both start and end are included
                    matchspec0 = distcache_range_partition_for_scan_endkey_tbl_match_spec_t(\
                    ...
                    #actnspec0 = distcache_range_partition_for_scan_endkey_action_spec_t(last_udpport_plus_one)
                    end_globalserveridx_plus_one = global_server_logical_idx + 1 # used to calculate max_scannum in data plane
                    actnspec0 = distcache_range_partition_for_scan_endkey_action_spec_t(end_globalserveridx_plus_one)
            ...
            for i in range(1, 5):
                ...
                for tmpoptype in [GETREQ]:
                    matchspec0 = eval("distcache_hash_for_cm{}_tbl_match_spec_t".format(i))(\
                            op_hdr_optype = tmpoptype)
            ...
            for tmpoptype in [PUTREQ, DELREQ]:
                matchspec0 = distcache_hash_for_seq_tbl_match_spec_t(\
                        op_hdr_optype = tmpoptype)
            ...
            for tmpoptype in [GETREQ, WARMUPREQ]:
                for tmp_client_physical_idx in range(client_physical_num):
                    matchspec0 = distcache_prepare_for_cachehit_tbl_match_spec_t(\
                            op_hdr_optype = tmpoptype,
                            ig_intr_md_ingress_port = self.client_devports[tmp_client_physical_idx])
                    actnspec0 = distcache_set_client_sid_action_spec_t(self.client_sids[tmp_client_physical_idx])
                ...
                #    actnspec0 = distcache_set_client_sid_action_spec_t(self.server_sids[tmp_server_physical_idx])
                #    self.client.prepare_for_cachehit_tbl_table_add_with_set_client_sid(\
                #            self.sess_hdl, self.dev_tgt, matchspec0, actnspec0)
            # set default sid as sids[0]
            #actnspec0 = distcache_set_client_sid_action_spec_t(self.sids[0])
            ...
            for tmp_client_physical_idx in range(client_physical_num):
                ...
                for tmpoptype in [GETRES, PUTRES, DELRES, WARMUPACK, SCANRES_SPLIT, LOADACK]:
                    matchspec0 = distcache_ipv4_forward_tbl_match_spec_t(\
                            op_hdr_optype = convert_u16_to_i16(tmpoptype),
                            ipv4_hdr_dstAddr = ipv4addr0,
                            ipv4_hdr_dstAddr_prefix_length = 32)
                    actnspec0 = distcache_forward_normal_response_action_spec_t(eport)
            ...
            for tmpoptype in [GETREQ]:
                matchspec0 = distcache_sample_tbl_match_spec_t(\
                        op_hdr_optype = tmpoptype)
            ...
            matchspec0 = distcache_ig_port_forward_tbl_match_spec_t(\
            ...
            for is_cached in cached_list:
                matchspec0 = distcache_access_latest_tbl_match_spec_t(\
                        op_hdr_optype = GETREQ_INSWITCH,
                        inswitch_hdr_is_cached = is_cached)
                if is_cached == 1:
            ...
            for tmpoptype in [PUTREQ_INSWITCH, DELREQ_INSWITCH]:
                matchspec0 = distcache_access_seq_tbl_match_spec_t(\
                        op_hdr_optype = tmpoptype)
            ...
            for tmpoptype in[GETREQ_INSWITCH, NETCACHE_WARMUPREQ_INSWITCH]:
                matchspec0 = distcache_save_client_udpport_tbl_match_spec_t(\
                        op_hdr_optype = tmpoptype)
            ...
            if RANGE_SUPPORT:
                ...
                for is_clone in [0, 1]:
                    for global_server_logical_idx in range(server_total_logical_num):
                        ...
                        matchspec0 = distcache_process_scanreq_split_tbl_match_spec_t(\
                        ...
                        if is_clone == 0:
                            ...
                            if server_physical_idx == -1:
                                ...
                                actnspec0 = distcache_process_scanreq_split_action_spec_t(tmp_server_sid)
                            ...
                            if current_local_server_logical_idx == -1 or next_server_physical_idx == -1:
                                ...
                                actnspec0 = distcache_process_cloned_scanreq_split_action_spec_t(tmp_udpport, tmp_server_sid)
            ...
            for tmpoptype in [GETREQ_INSWITCH, SCANREQ_SPLIT]:
                for tmp_server_physical_idx in range(server_physical_num):
                    ...
                    matchspec0 = distcache_prepare_for_cachepop_tbl_match_spec_t(\
                            op_hdr_optype = tmpoptype,
                            eg_intr_md_egress_port = tmp_devport)
                    if tmpoptype == GETREQ_INSWITCH:
                        actnspec0 = distcache_set_server_sid_and_port_action_spec_t(tmp_server_sid)
                    ...
                    for is_cached in cached_list:
                        for is_latest in latest_list:
                            if is_cached == 1 and is_latest == 1: # follow algorithm 1 in NetCache paper to update CM
                                ...
                                matchspec0 = eval("distcache_access_cm{}_tbl_match_spec_t".format(i))(\
                                        op_hdr_optype = tmpoptype,
            ...
            matchspec0 = distcache_is_hot_tbl_match_spec_t(\
            ...
            for tmpoptype in [GETREQ_INSWITCH]:
                matchspec0 = distcache_access_cache_frequency_tbl_match_spec_t(\
                        op_hdr_optype = tmpoptype,
                        inswitch_hdr_is_sampled = 1,
            ...
            for is_cached in cached_list:
                for is_latest in latest_list:
                    for is_stat in stat_list:
                        matchspec0 = distcache_access_deleted_tbl_match_spec_t(\
                                op_hdr_optype = GETREQ_INSWITCH,
                                inswitch_hdr_is_cached = is_cached,
                                meta_is_latest = is_latest,
            ...
            for is_cached in cached_list:
                for is_latest in latest_list:
                    matchspec0 = distcache_access_savedseq_tbl_match_spec_t(\
                            op_hdr_optype = CACHE_POP_INSWITCH,
                            inswitch_hdr_is_cached = is_cached,
                            meta_is_latest = is_latest)
            ...
            for is_cached in cached_list:
                for is_latest in latest_list:
                    matchspec0 = distcache_update_vallen_tbl_match_spec_t(\
                            op_hdr_optype = GETREQ_INSWITCH,
                            inswitch_hdr_is_cached = is_cached,
                            meta_is_latest = is_latest)
                    if is_cached == 1:
            ...
            for i in range(1, bf_hashnum+1):
                ...
                for tmpoptype in [GETREQ_INSWITCH]:
                    matchspec0 = eval("distcache_access_bf{}_tbl_match_spec_t".format(i))(\
                            op_hdr_optype = tmpoptype,
                            meta_is_hot = 1)
            ...
            matchspec0 = distcache_is_report_tbl_match_spec_t(\
                    meta_is_report1 = 1,
                    meta_is_report2 = 1,
                    meta_is_report3 = 1)
            ...
            if RANGE_SUPPORT == False:
                for tmpoptype in [NETCACHE_GETREQ_POP, NETCACHE_WARMUPREQ_INSWITCH_POP]:
                    matchspec0 = distcache_lastclone_lastscansplit_tbl_match_spec_t(\
                            op_hdr_optype = tmpoptype,
            ...
            for i in range(switch_max_vallen/8 + 1): # i from 0 to 16
                ...
                matchspec0 = distcache_update_pktlen_tbl_match_spec_t(\
                        op_hdr_optype=GETRES,
                        vallen_hdr_vallen_start=vallen_start,
                        vallen_hdr_vallen_end=vallen_end) # [vallen_start, vallen_end]
                actnspec0 = distcache_update_pktlen_action_spec_t(val_stat_udplen, val_stat_iplen)
                ...
                for tmpoptype in [PUTREQ_SEQ, NETCACHE_PUTREQ_SEQ_CACHED]:
                    matchspec0 = distcache_update_pktlen_tbl_match_spec_t(\
                            ...
                            vallen_hdr_vallen_start=vallen_start,
                            vallen_hdr_vallen_end=vallen_end) # [vallen_start, vallen_end]
                    actnspec0 = distcache_update_pktlen_action_spec_t(val_seq_udplen, val_seq_iplen)
                    self.client.update_pktlen_tbl_table_add_with_update_pktlen(\
                            self.sess_hdl, self.dev_tgt, matchspec0, 0, actnspec0) # 0 is priority (range may be overlapping)
            onlyop_udplen = 26
            onlyop_iplen = 46
            ...
            for tmpoptype in [CACHE_POP_INSWITCH_ACK, GETREQ, WARMUPACK, NETCACHE_VALUEUPDATE_ACK]:
                ...
                actnspec0 = distcache_update_pktlen_action_spec_t(onlyop_udplen, onlyop_iplen)
                self.client.update_pktlen_tbl_table_add_with_update_pktlen(\
                        self.sess_hdl, self.dev_tgt, matchspec0, 0, actnspec0) # 0 is priority (range may be overlapping)
            for tmpoptype in [DELREQ_SEQ, NETCACHE_DELREQ_SEQ_CACHED]:
                matchspec0 = distcache_update_pktlen_tbl_match_spec_t(\
                        ...
                        vallen_hdr_vallen_start=0,
                        vallen_hdr_vallen_end=switch_max_vallen) # [0, 128]
                actnspec0 = distcache_update_pktlen_action_spec_t(seq_udplen, seq_iplen)
                self.client.update_pktlen_tbl_table_add_with_update_pktlen(\
                        self.sess_hdl, self.dev_tgt, matchspec0, 0, actnspec0) # 0 is priority (range may be overlapping)
            matchspec0 = distcache_update_pktlen_tbl_match_spec_t(\
                    op_hdr_optype=SCANREQ_SPLIT,
                    vallen_hdr_vallen_start=0,
                    vallen_hdr_vallen_end=switch_max_vallen) # [0, 128]
            actnspec0 = distcache_update_pktlen_action_spec_t(scanreqsplit_udplen, scanreqsplit_iplen)
            self.client.update_pktlen_tbl_table_add_with_update_pktlen(\
                    self.sess_hdl, self.dev_tgt, matchspec0, 0, actnspec0) # 0 is priority (range may be overlapping)
            matchspec0 = distcache_update_pktlen_tbl_match_spec_t(\
                    ...
                    vallen_hdr_vallen_start=0,
                    vallen_hdr_vallen_end=switch_max_vallen) # [0, 128]
            actnspec0 = distcache_update_pktlen_action_spec_t(frequency_udplen, frequency_iplen)
            self.client.update_pktlen_tbl_table_add_with_update_pktlen(\
                    self.sess_hdl, self.dev_tgt, matchspec0, 0, actnspec0) # 0 is priority (range may be overlapping)
            matchspec0 = distcache_update_pktlen_tbl_match_spec_t(\
                    op_hdr_optype=NETCACHE_GETREQ_POP,
                    vallen_hdr_vallen_start=0,
                    vallen_hdr_vallen_end=switch_max_vallen) # [0, 128]
            actnspec0 = distcache_update_pktlen_action_spec_t(op_clone_udplen, op_clone_iplen)
            self.client.update_pktlen_tbl_table_add_with_update_pktlen(\
                    self.sess_hdl, self.dev_tgt, matchspec0, 0, actnspec0) # 0 is priority (range may be overlapping)
            matchspec0 = distcache_update_pktlen_tbl_match_spec_t(\
                    ...
                    vallen_hdr_vallen_start=0,
                    vallen_hdr_vallen_end=switch_max_vallen) # [0, 128]
            actnspec0 = distcache_update_pktlen_action_spec_t(op_inswitch_clone_udplen, op_inswitch_clone_iplen)
            self.client.update_pktlen_tbl_table_add_with_update_pktlen(\
                    self.sess_hdl, self.dev_tgt, matchspec0, 0, actnspec0) # 0 is priority (range may be overlapping)
            ...
            for tmp_client_physical_idx in range(client_physical_num):
                ...
                actnspec0 = distcache_update_ipmac_srcport_server2client_action_spec_t(\
                ...
                for tmpoptype in [GETRES, PUTRES, DELRES, SCANRES_SPLIT, WARMUPACK, LOADACK]:
                    matchspec0 = distcache_update_ipmac_srcport_tbl_match_spec_t(\
                            op_hdr_optype = convert_u16_to_i16(tmpoptype), 
                            eg_intr_md_egress_port = tmp_devport)
            ...
            for tmp_server_physical_idx in range(server_physical_num):
                ...
                actnspec1 = distcache_update_dstipmac_client2server_action_spec_t(\
                        macAddr_to_string(tmp_server_mac), \
                        ipv4Addr_to_i32(tmp_server_ip))
                ...
                actnspec1 = distcache_update_ipmac_srcport_client2server_action_spec_t(\
                        macAddr_to_string(tmp_client_mac), \
                        macAddr_to_string(tmp_server_mac), \
                        ipv4Addr_to_i32(tmp_client_ip), \
                        ipv4Addr_to_i32(tmp_server_ip), \
            ...
            actnspec2 = distcache_update_ipmac_srcport_switch2switchos_action_spec_t(\
                    macAddr_to_string(tmp_client_mac), \
            ...
            actnspec2 = distcache_update_dstipmac_switch2switchos_action_spec_t(\
                    macAddr_to_string(self.reflector_mac_for_switch), \
                    ipv4Addr_to_i32(self.reflector_ip_for_switch))
            for tmpoptype in [NETCACHE_GETREQ_POP, NETCACHE_WARMUPREQ_INSWITCH_POP]:
                matchspec0 = distcache_update_ipmac_srcport_tbl_match_spec_t(\
            ...
            for tmpoptype in [PUTREQ_SEQ, NETCACHE_PUTREQ_SEQ_CACHED, GETRES, LOADREQ]:
                for i in range(switch_max_vallen/8 + 1): # i from 0 to 16
                    if i == 0:
                        ...
                        vallen_start = (i-1)*8+1 # 1, 9, ..., 121
                        vallen_end = (i-1)*8+8 # 8, 16, ..., 128
                    matchspec0 = distcache_add_and_remove_value_header_tbl_match_spec_t(\
                            op_hdr_optype=tmpoptype,
                            vallen_hdr_vallen_start=vallen_start,
                            vallen_hdr_vallen_end=vallen_end) # [vallen_start, vallen_end]
                    if i == 0:
                                ...
                                for is_lastclone_for_pktloss in lastclone_list:
                                    for tmp_server_sid in tmp_server_sids: # Only work for NETCACHE_GETREQ_POP
                                        ...
                                        if is_hot == 0 and is_report == 0 and is_latest == 0 and is_deleted == 0 and is_lastclone_for_pktloss == 0 and tmp_server_sid == 0 and tmp_client_sid != 0:
                                            ...
                                            matchspec0 = distcache_eg_port_forward_tbl_match_spec_t(\
                                            ...
                                            actnspec0 = distcache_update_netcache_warmupreq_inswitch_to_netcache_warmupreq_inswitch_pop_clone_for_pktloss_and_warmupack_action_spec_t(self.reflector_sid, reflector_dp2cpserver_port)
                                        ...
                                        if is_hot == 0 and is_report == 0 and is_latest == 0 and is_deleted == 0 and tmp_server_sid == 0 and tmp_client_sid != 0:
                                            ...
                                            if is_lastclone_for_pktloss == 0:
                                                ...
                                                actnspec0 = distcache_forward_netcache_warmupreq_inswitch_pop_clone_for_pktloss_and_warmupack_action_spec_t(self.reflector_sid)
                                                ...
                                                actnspec0 = distcache_update_netcache_warmupreq_inswitch_pop_to_warmupack_by_mirroring_action_spec_t(tmp_client_sid, server_worker_port_start)
                                        ...
                                        if is_lastclone_for_pktloss == 0 and tmp_client_sid != 0 and tmp_server_sid != 0:
                                            ...
                                            if is_cached == 0:
                                                if is_hot == 1 and is_report == 0:
                                                    ...
                                                    actnspec0 = distcache_update_getreq_inswitch_to_netcache_getreq_pop_clone_for_pktloss_and_getreq_action_spec_t(self.reflector_sid, reflector_dp2cpserver_port)
                                                    ...
                                                    #actnspec0 = distcache_update_getreq_inswitch_to_getreq_action_spec_t(self.devPorts[1])
                                                ...
                                                if is_latest == 0: # follow algorithm 1 in NetCache paper to report hot key if necessary
                                                    ...
                                                    actnspec0 = distcache_update_getreq_inswitch_to_getres_by_mirroring_action_spec_t(tmp_client_sid, server_worker_port_start, tmpstat)
                                        ...
                                        if is_cached == 0 and is_hot == 0 and is_report == 0 and is_latest == 0 and is_deleted == 0 and tmp_client_sid == 0 and tmp_server_sid != 0:
                                            ...
                                            matchspec0 = distcache_eg_port_forward_tbl_match_spec_t(\
                                                ...
                                                clone_hdr_server_sid = tmp_server_sid)
                                            if is_lastclone_for_pktloss == 0:
                                                actnspec0 = distcache_forward_netcache_getreq_pop_clone_for_pktloss_and_getreq_action_spec_t(self.reflector_sid)
                                                self.client.eg_port_forward_tbl_table_add_with_forward_netcache_getreq_pop_clone_for_pktloss_and_getreq(\
                                                        self.sess_hdl, self.dev_tgt, matchspec0, actnspec0)
                                            else:
                                                actnspec0 = distcache_update_netcache_getreq_pop_to_getreq_by_mirroring_action_spec_t(tmp_server_sid)
                                        ...
                                        if is_cached == 0 and is_hot == 0 and is_report == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0 and tmp_server_sid == 0:
                                            ...
                                            actnspec0 = distcache_update_cache_pop_inswitch_to_cache_pop_inswitch_ack_drop_and_clone_action_spec_t(self.reflector_sid, reflector_dp2cpserver_port)
                                        ...
                                        if is_cached == 0 and is_hot == 0 and is_report == 0 and is_latest == 0 and is_deleted == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0 and tmp_server_sid == 0:
                                            ...
                                            #    actnspec0 = distcache_forward_cache_pop_inswitch_ack_clone_for_pktloss_action_spec_t(self.reflector_sid)
                                        ...
                                        if is_cached == 1 and is_report == 0 and is_hot == 0 and is_latest == 0 and is_deleted == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0 and tmp_server_sid == 0:
                                            ...
                                            actnspec0 = distcache_update_cache_evict_loadfreq_inswitch_to_cache_evict_loadfreq_inswitch_ack_drop_and_clone_action_spec_t(self.reflector_sid, reflector_dp2cpserver_port)
                                        ...
                                        for tmp_server_sid in tmp_server_sids: # Only work for NETCACHE_GETREQ_POP and SCANREQ_SPLIT
                                            ...
                                            if is_cached == 0 and is_hot == 0 and is_report == 0 and is_latest == 0 and is_deleted == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0 and tmp_server_sid != 0:
                                                ...
                                                if is_last_scansplit == 1:
                                                    ...
                                                    actnspec0 = distcache_forward_scanreq_split_and_clone_action_spec_t(tmp_server_sid)
