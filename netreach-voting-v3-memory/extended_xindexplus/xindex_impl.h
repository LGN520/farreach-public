...
XIndex<key_t, val_t, seq>::XIndex(const std::vector<key_t> &keys,
  ...
  root = new root_t();
  root->init(keys, vals, snapshot_id); // Set create_id = 0, ss_id = -1 for each atomic value
  init_snapshot(); // Set ss_id = 1 for each atomic value
  start_bg();
}
...
inline bool XIndex<key_t, val_t, seq>::put(const key_t &key, const val_t &val,
  ...
  result_t res;
  rcu_progress(worker_id);
  while ((res = root->put(key, val, worker_id, snapshot_id)) == result_t::retry) {
    rcu_progress(worker_id);
  }
...
inline bool XIndex<key_t, val_t, seq>::remove(const key_t &key,
                                              const uint32_t worker_id) {
  rcu_progress(worker_id);
  return root->remove(key, snapshot_id) == result_t::ok;
}
...
template <class key_t, class val_t, bool seq>
void XIndex<key_t, val_t, seq>::stop_snapshot() {
