...
class AltBtreeBuffer {
 ...
 public:
  ...
  inline bool update(const key_t &key, const val_t &val, int32_t snapshot_id);
  inline void insert(const key_t &key, const val_t &val, int32_t snapshot_id);
  inline bool remove(const key_t &key, int32_t snapshot_id);
 ...
 private:
  ...
  void insert_leaf(const key_t &key, const val_t &val, leaf_t *target, int32_t snapshot_id);
