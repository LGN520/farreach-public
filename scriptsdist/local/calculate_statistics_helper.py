...
def staticprocess(localjsonarray, remotejsonarray, bottleneckidx):
    ...
    for i in range(1, len(aggjsonarray)):
        (
            tmp_bottleneck_totalthpt,
            tmp_bottleneck_switchthpt,
            tmp_bottleneck_serverthpt,
        ) = get_total_switch_server_thpts(
    ...
    print(
        "[STATIC] average bottleneck totalthpt: {} MOPS; switchthpt: {} MOPS; serverthpt: {} MOPS".format(
            avgbottleneck_totalthpt, avgbottleneck_switchthpt, avgbottleneck_serverthpt
        )
    )

    totalthpt = avgbottleneck_totalthpt
    max_serverthpt = avgbottleneck_serverthpt
    min_serverthpt = avgbottleneck_serverthpt
    ...
    for i in range(1, len(aggjsonarray)):
        ...
        (
            tmp_rotate_totalthpt,
            tmp_rotate_switchthpt,
            tmp_rotate_serverthpt,
        ) = get_total_switch_server_thpts(
        ...
        if tmp_rotate_serverthpt < min_serverthpt:
            min_serverthpt = tmp_rotate_serverthpt
    normalized_thpt = float(totalthpt) / float(max_serverthpt)
    imbalanceratio = float(max_serverthpt) / float(min_serverthpt)
    print(
        "[STATIC] aggregate throughput: {} MOPS; normalized throughput: {}, imbalanceratio: {}".format(
            totalthpt, normalized_thpt, imbalanceratio
        )
    )
    ...
    for i in range(len(aggjsonarray)):
        ...
        tmp_bottleneckhist, _, _ = get_total_cachehit_cachemiss_latencyhist(
            aggjsonarray, i, 0
        )
    ...
    for i in range(1, len(aggjsonarray)):
        tmp_rotatehist, _, _ = get_total_cachehit_cachemiss_latencyhist(
            aggjsonarray, i, 1
        )
    ...
    (
        ...
        _,
        _,
    ) = calculatelatency(totallatencyhist)
    print(
        "[STATIC] average latency {} us, medium latency {} us, 90P latency {} us, 95P latency {} us, 99P latency {} us".format(
            avglatency, latencymedium, latency90p, latency95p, latency99p
        )
    )
...
def dynamicprocess(localjsonarray, remotejsonarray):
    ...
    persec_imbalanceratio = [0] * seconds
    ...
    for i in range(seconds):
        tmpjsonobj = aggjsonarray[i]
        persecthpt[i] = getmops(
            tmpjsonobj[TOTAL_OPSDONE] / tmpjsonobj[EXECUTION_MILLIS]
        )

        tmp_maxserverthpt = 0
        tmp_minserverthpt = 0
        for j in range(len(tmpjsonobj[PERSERVER_OPSDONE])):
            tmp_serverthpt = getmops(
                tmpjsonobj[PERSERVER_OPSDONE][j] / tmpjsonobj[EXECUTION_MILLIS]
            )
            if tmp_maxserverthpt == 0 or tmp_serverthpt > tmp_maxserverthpt:
                tmp_maxserverthpt = tmp_serverthpt
            if tmp_minserverthpt == 0 or tmp_serverthpt < tmp_minserverthpt:
                tmp_minserverthpt = tmp_serverthpt
        if tmp_maxserverthpt != 0:
            persec_normalizedthpt[i] = persecthpt[i] / float(tmp_maxserverthpt)
            persec_imbalanceratio[i] = tmp_maxserverthpt / float(tmp_minserverthpt)
        else:
            persec_normalizedthpt[i] = 0
            persec_imbalanceratio[i] = 0.0

        tmp_server0_totallatencyhist, _, _ = get_total_cachehit_cachemiss_latencyhist(
            aggjsonarray, i, 0
        )
        tmp_server1_totallatencyhist, _, _ = get_total_cachehit_cachemiss_latencyhist(
            aggjsonarray, i, 1
        )
        tmp_totallatencyhist = [0] * len(tmp_server0_totallatencyhist)
        for j in range(len(tmp_server0_totallatencyhist)):
            tmp_totallatencyhist[j] = (
                tmp_server0_totallatencyhist[j] + tmp_server1_totallatencyhist[j]
            )
        (
            perseclatencyavg[i],
            perseclatencymedium[i],
            perseclatency90p[i],
            perseclatency95p[i],
            perseclatency99p[i],
            _,
    ...
    print("imbalanceratio: {}".format(persec_imbalanceratio))
    ...
    (
        ...
        _,
        _,
    ) = calculatelatency(totalhistogram)
    print(
        "[DYNAMIC][OVERALL] avgthpt {} MOPS, avelat {} us, medlat {} us, 90Plat {} us, 95Plat {} us, 99Plat {} us".format(
            avgthpt, avglatency, latencymedium, latency90p, latency95p, latency99p
        )
    )
...
if len(sys.argv) != 5:
    print(
        "[ERROR] Usage: python calculate_statistics_helper.py <workloadmode> <main-client-file> <secondary-client-file> <bottleneckidx>"
    )
