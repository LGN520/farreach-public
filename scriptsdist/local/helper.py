STRID = "strid"
TOTAL_OPSDONE = (
    "totalOpsdone"  # including cache hits and cache misses of all logical servers
)
...
def aggregate_1dlist(locallist, remotelist, objidx, role):
    ...
    if len(locallist) != len(remotelist):
        print(
            "[ERROR][aggregate] for {}th obj, localjsonobj[{}] size {} != remotejsonobj[{}] size {}".format(
                objidx, role, len(locallist), role, len(remotelist)
            )
        )
...
def aggregate_2dlist(locallist, remotelist, objidx, role):
    ...
    for i in range(len(locallist)):
        ...
        if len(tmplocalsublist) != len(tmpremotesublist):
            print(
                "[ERROR][aggregate] for {}th obj {}th histogram, localsublist size {} != remotesublist size {}".format(
                    objidx, i, len(tmplocalsublist), len(tmpremotesublist)
                )
            )
...
def misscheck(localjsonarray, remotejsonarray):
    ...
    print(
        "[WARN] If you are not running exp_dynamic, please refer to README.md Section 3.2 to perform a single iteration for each missing number by scriptsdist/exps/run_makeup_rotation_exp.sh"
    )
...
def aggregate(localjsonarray, remotejsonarray, length):
    ...
    for i in range(length):
        ...
        if localstrid == remotestrid:
            ...
            print(
                "[ERROR][STRID] for STRID of the {}th JsonObject, {} != {}".format(
                    i, localstrid, remotestrid
                )
            )
            exit(-1)

        aggobj[TOTAL_OPSDONE] = (
            localjsonobj[TOTAL_OPSDONE] + remotejsonobj[TOTAL_OPSDONE]
        )
        aggobj[PERSERVER_OPSDONE] = aggregate_1dlist(
            localjsonobj[PERSERVER_OPSDONE],
            remotejsonobj[PERSERVER_OPSDONE],
            i,
            PERSERVER_OPSDONE,
        )
        aggobj[PERSERVER_CACHEHITS] = aggregate_1dlist(
            localjsonobj[PERSERVER_CACHEHITS],
            remotejsonobj[PERSERVER_CACHEHITS],
            i,
            PERSERVER_CACHEHITS,
        )

        aggobj[EXECUTION_MILLIS] = (
            localjsonobj[EXECUTION_MILLIS] + remotejsonobj[EXECUTION_MILLIS]
        ) / 2
        # aggobj[EXECUTION_MILLIS] = GLOBAL_PEROBJ_EXECUTION_MILLIS

        aggobj[PERSERVER_CACHEHIT_TOTAL_LATENCY] = aggregate_1dlist(
            localjsonobj[PERSERVER_CACHEHIT_TOTAL_LATENCY],
            remotejsonobj[PERSERVER_CACHEHIT_TOTAL_LATENCY],
            i,
            PERSERVER_CACHEHIT_TOTAL_LATENCY,
        )
        aggobj[PERSERVER_CACHEHIT_TOTAL_LATENCYNUM] = aggregate_1dlist(
            localjsonobj[PERSERVER_CACHEHIT_TOTAL_LATENCYNUM],
            remotejsonobj[PERSERVER_CACHEHIT_TOTAL_LATENCYNUM],
            i,
            PERSERVER_CACHEHIT_TOTAL_LATENCYNUM,
        )
        aggobj[PERSERVER_CACHEHIT_TOTAL_HISTOGRAM] = aggregate_2dlist(
            localjsonobj[PERSERVER_CACHEHIT_TOTAL_HISTOGRAM],
            remotejsonobj[PERSERVER_CACHEHIT_TOTAL_HISTOGRAM],
            i,
            PERSERVER_CACHEHIT_TOTAL_HISTOGRAM,
        )

        aggobj[PERSERVER_CACHEMISS_TOTAL_LATENCY] = aggregate_1dlist(
            localjsonobj[PERSERVER_CACHEMISS_TOTAL_LATENCY],
            remotejsonobj[PERSERVER_CACHEMISS_TOTAL_LATENCY],
            i,
            PERSERVER_CACHEMISS_TOTAL_LATENCY,
        )
        aggobj[PERSERVER_CACHEMISS_TOTAL_LATENCYNUM] = aggregate_1dlist(
            localjsonobj[PERSERVER_CACHEMISS_TOTAL_LATENCYNUM],
            remotejsonobj[PERSERVER_CACHEMISS_TOTAL_LATENCYNUM],
            i,
            PERSERVER_CACHEMISS_TOTAL_LATENCYNUM,
        )
        aggobj[PERSERVER_CACHEMISS_TOTAL_HISTOGRAM] = aggregate_2dlist(
            localjsonobj[PERSERVER_CACHEMISS_TOTAL_HISTOGRAM],
            remotejsonobj[PERSERVER_CACHEMISS_TOTAL_HISTOGRAM],
            i,
            PERSERVER_CACHEMISS_TOTAL_HISTOGRAM,
        )
...
def calculatelatency(totalhistogram):
    ...
    if totallatencynum > 0:
        for i in range(len(totalhistogram)):
            ...
            if (
                donemedium == False
                and float(curlatencynum) / float(totallatencynum) >= 0.50
            ):
                donemedium = True
                latencymedium = i
            if (
                done90p == False
                and float(curlatencynum) / float(totallatencynum) >= 0.90
            ):
                done90p = True
                latency90p = i
            if (
                done95p == False
                and float(curlatencynum) / float(totallatencynum) >= 0.95
            ):
                done95p = True
                latency95p = i
...
def get_total_switch_server_thpts(
    jsonarray, rotationidx, physicalidx, workloadmode, bottleneckidx
):
    tmp_logical_serveridx = get_logical_serveridx(
        rotationidx, physicalidx, workloadmode, bottleneckidx
    )

    tmpjsonobj = jsonarray[rotationidx]
    tmptotaltime = tmpjsonobj[EXECUTION_MILLIS]
    tmptotalthpt = getmops(
        (
            tmpjsonobj[PERSERVER_CACHEHITS][tmp_logical_serveridx]
            + tmpjsonobj[PERSERVER_OPSDONE][tmp_logical_serveridx]
        )
        / tmptotaltime
    )
    tmpswitchthpt = getmops(
        tmpjsonobj[PERSERVER_CACHEHITS][tmp_logical_serveridx] / tmptotaltime
    )
    tmpserverthpt = getmops(
        tmpjsonobj[PERSERVER_OPSDONE][tmp_logical_serveridx] / tmptotaltime
    )
...
def get_total_cachehit_cachemiss_latencyhist(jsonarray, rotationidx, physicalidx):
    tmp_cachehit_totallatencyhist = jsonarray[rotationidx][
        PERSERVER_CACHEHIT_TOTAL_HISTOGRAM
    ][physicalidx]
    tmp_cachemiss_totallatencyhist = jsonarray[rotationidx][
        PERSERVER_CACHEMISS_TOTAL_HISTOGRAM
    ][physicalidx]
    tmp_totallatencyhist = [0] * len(tmp_cachehit_totallatencyhist)
    for i in range(len(tmp_totallatencyhist)):
        tmp_totallatencyhist[i] = (
            tmp_cachehit_totallatencyhist[i] + tmp_cachemiss_totallatencyhist[i]
        )
    return (
        tmp_totallatencyhist,
        tmp_cachehit_totallatencyhist,
        tmp_cachemiss_totallatencyhist,
    )
...
def calculate_perobjstat(aggjsonarray, workloadmode, bottleneckidx):
    for i in range(len(aggjsonarray)):
        ...
        if tmp_maxserverops == 0:
            print(
                "[{}] tmp_maxserverops is 0, tmp_perserverops: {}".format(
                    tmpstrid, tmp_perserverops
                )
            )
        else:
            ...
            if DEBUG_MODE == True:
                print(
                    "[{}] overall thpt {} MOPS; cache hit rate {}; cache miss rate {}; normalized thpt {}".format(
                        tmpstrid,
                        getmops(float(tmptotalops) / float(tmptotaltime)),
                        float(tmp_cachehitcnt) / float(tmptotalops),
                        float(tmp_cachemisscnt) / float(tmptotalops),
                        tmp_normalizedthpt,
                    )
                )
        ...
        for j in range(len(tmpjsonobj[PERSERVER_CACHEHIT_TOTAL_LATENCY])):
            ...
            (
                ...
                tmp_cachehit_totallatencyhist,
                tmp_cachemiss_totallatencyhist,
            ) = get_total_cachehit_cachemiss_latencyhist(aggjsonarray, i, j)

            (
                tmp_cachehit_avglatency,
                tmp_cachehit_latencymedium,
                tmp_cachehit_latency90p,
                tmp_cachehit_latency95p,
                tmp_cachehit_latency99p,
                tmp_cachehit_minlatency,
                tmp_cachehit_maxlatency,
            ) = calculatelatency(tmp_cachehit_totallatencyhist)
            if DEBUG_MODE == True:
                print(
                    "cache hits: avglat {} us, midlat {} us, 90Plat {} us, 95Plat {} us, 99Plat {} us, minlat {} us, maxlat {} us".format(
                        tmp_cachehit_avglatency,
                        tmp_cachehit_latencymedium,
                        tmp_cachehit_latency90p,
                        tmp_cachehit_latency95p,
            ...
            (
                tmp_cachemiss_avglatency,
                tmp_cachemiss_latencymedium,
                tmp_cachemiss_latency90p,
                tmp_cachemiss_latency95p,
                tmp_cachemiss_latency99p,
                tmp_cachemiss_minlatency,
                tmp_cachemiss_maxlatency,
            ) = calculatelatency(tmp_cachemiss_totallatencyhist)
            if DEBUG_MODE == True:
                print(
                    "cache misses: avglat {} us, midlat {} us, 90Plat {} us, 95Plat {} us, 99Plat {} us, minlat {} us, maxlat {} us".format(
                        tmp_cachemiss_avglatency,
                        tmp_cachemiss_latencymedium,
                        tmp_cachemiss_latency90p,
                        tmp_cachemiss_latency95p,
            ...
            (
                ...
                tmp_minlatency,
                tmp_maxlatency,
            ) = calculatelatency(tmp_totallatencyhist)
            if DEBUG_MODE == True:
                print(
                    "cache hits+misses: avglat {} us, midlat {} us, 90Plat {} us, 95Plat {} us, 99Plat {} us, minlat {} us, maxlat {} us".format(
                        tmp_avglatency,
                        tmp_latencymedium,
                        tmp_latency90p,
                        tmp_latency95p,
            ...
            tmptotalthpt, tmpswitchthpt, tmpserverthpt = get_total_switch_server_thpts(
                aggjsonarray, i, j, workloadmode, bottleneckidx
            )
            if DEBUG_MODE == True:
                print(
                    "totalthpt {} MOPS, switchthpt {} MOPS, serverthpt {} MOPS, overflow latencynum: {}".format(
                        tmptotalthpt,
                        tmpswitchthpt,
                        tmpserverthpt,
                        tmp_totallatencyhist[-1],
                    )
                )
    return
