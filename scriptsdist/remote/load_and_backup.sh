...
if [ $# -ne 0 ]
then
	echo "Usage: bash scriptsdist/remote/load_and_backup.sh"
	exit
fi
...
then
	echo "[ERROR] DIRNAME (${DIRNAME}) is not nocache in scriptsdist/common.sh; please update and sync scriptsdist/common.sh, and re-launch switch"
	exit
fi
