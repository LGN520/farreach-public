...
echo "make clients"
source scriptsdist/local/makeclient.sh # use source to avoid including common.sh twice
...
#ssh ${USER}@${SPINESWITCH} "cd ${SWITCH_ROOTPATH}; bash scriptsdist/local/makeswitchos.sh"

echo "make servers"
# ssh ${USER}@dl16 "
cd ${CLIENT_ROOTPATH}; bash scriptsdist/local/makeserver.sh
# ssh ${USER}@dl13 "cd ${CLIENT_ROOTPATH}; bash scriptsdist/local/makeserver.sh"
