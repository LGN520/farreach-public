...
then
	echo "[ERROR] you should change DIRNAME as farreach in scriptsdist/common.sh before running test_recovery_time.sh"
	exit
fi
...
#ssh ${USER}@${SERVER0} "cd ${SERVER_ROOTPATH}/${DIRNAME}; bash localscriptsdist/fetchbackup_client2server.sh"
#end_time_2_1=`date +%s.%N`
#ssh ${USER}@${SERVER1} "cd ${SERVER_ROOTPATH}/${DIRNAME}; bash localscriptsdist/fetchbackup_client2server.sh"
...
${SERVER_ROOTPATH}/${DIRNAME}/localscriptsdist/fetchbackup_client2server.sh 
# mx h4 bash ${SERVER_ROOTPATH}/${DIRNAME}/localscriptsdist/fetchbackup_client2server.sh 
end_time_2=`date +%s.%N`
...
then
	source scriptsdist/remote/prepare_server_rotation.sh
else
...
source scriptsdist/remote/launchservertestbed.sh recover
...
# Fetch all to switch
# ssh ${USER}@${LEAFSWITCH} "
cd ${SWITCH_ROOTPATH}/${DIRNAME}; bash localscriptsdist/fetchall_all2switch.sh
...
cd ${SWITCH_ROOTPATH}/${DIRNAME}/bmv2; rm tmp_switch.out; nohup bash start_switch.sh >tmp_switch.out 2>&1 &
sleep 10s
...
# ssh -i /root/${SWITCH_PRIVATEKEY} root@${LEAFSWITCH} "
cd ${SWITCH_ROOTPATH}/${DIRNAME}; rm tmp_launchswitchostestbed.out; bash localscriptsdist/launchswitchostestbed.sh recover >tmp_launchswitchostestbed.out 2>&1
sleep 5s
...
# ssh -i /root/${SWITCH_PRIVATEKEY} root@${LEAFSWITCH} "
cd ${SWITCH_ROOTPATH}/${DIRNAME}; bash localscriptsdist/stopswitchtestbed.sh >/dev/null 2>&1
cd ${SWITCH_ROOTPATH}
