...
if [ $# -lt 7 ]; then
	echo "Usage: bash scriptsdist/exps/run_makeup_rotation_exp.sh <expname> <roundnumber> <methodname> <workloadname> <serverscale> <bottleneckidx> <targetrotation> [targetthpt]"
	exit
fi
expname=$1
roundnumber=$2
...
### Stop running switch
ssh -i /root/${SWITCH_PRIVATEKEY} root@${LEAFSWITCH} "cd ${SWITCH_ROOTPATH}/${methodname}; bash localscriptsdist/stopswitchtestbed.sh"
...
ssh -i /root/${SWITCH_PRIVATEKEY} root@${LEAFSWITCH} "cd ${SWITCH_ROOTPATH}/${methodname}; bash localscriptsdist/launchswitchostestbed.sh"
...
if [ $# -eq 8 ]; then
	bash scriptsdist/remote/test_server_rotation_p2.sh 1 ${targetrotation} ${targetthpt}
else
	bash scriptsdist/remote/test_server_rotation_p2.sh 1 ${targetrotation}
fi
...
if [ $# -eq 8 ]; then
	bash scriptsdist/remote/calculate_statistics.sh 1
else
