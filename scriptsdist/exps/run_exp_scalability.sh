...
if [ $# -ne 1 ]; then
	echo "Usage: bash scriptsdist/exps/run_exp_scalability.sh <roundnumber>"
	exit
fi
roundnumber=$1
...
for exp3_method in ${exp3_method_list[@]}; do
  sed -i "/^DIRNAME=/s/=.*/=\"${exp3_method}\"/" ${CLIENT_ROOTPATH}/scriptsdist/common.sh
  ...
  for exp3_scalability in ${exp3_scalability_list[@]}; do
    ...
    ssh -i /root/${SWITCH_PRIVATEKEY} root@${LEAFSWITCH} "cd ${SWITCH_ROOTPATH}/${exp3_method}; bash localscriptsdist/stopswitchtestbed.sh"
    ...
    ssh -i /root/${SWITCH_PRIVATEKEY} root@${LEAFSWITCH} "cd ${SWITCH_ROOTPATH}/${exp3_method}; bash localscriptsdist/launchswitchostestbed.sh"
