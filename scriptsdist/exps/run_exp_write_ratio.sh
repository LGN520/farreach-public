...
if [ $# -ne 1 ]; then
	echo "Usage: bash scriptsdist/exps/run_exp_write_ratio.sh <roundnumber>"
	exit
fi
roundnumber=$1
...
for exp4_method in ${exp4_method_list[@]}; do
  sed -i "/^DIRNAME=/s/=.*/=\"${exp4_method}\"/" ${CLIENT_ROOTPATH}/scriptsdist/common.sh
  cd ${CLIENT_ROOTPATH}
  ...
  for exp4_workload in ${exp4_workload_list[@]}; do
    ...
    ssh -i /root/${SWITCH_PRIVATEKEY} root@${LEAFSWITCH} "cd /${SWITCH_ROOTPATH}/${exp4_method}; bash localscriptsdist/stopswitchtestbed.sh"
    ...
    ssh -i /root/${SWITCH_PRIVATEKEY} root@${LEAFSWITCH} "cd /${SWITCH_ROOTPATH}/${exp4_method}; bash localscriptsdist/launchswitchostestbed.sh"
