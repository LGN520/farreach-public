...
if [ $# -ne 1 ]; then
	echo "Usage: bash scriptsdist/exps/run_exp_latency.sh <roundnumber>"
	exit
fi
roundnumber=$1
...
for exp2_method in ${exp2_method_list[@]}; do
  ...
  sed -i "/^DIRNAME=/s/=.*/=\"${exp2_method}\"/" ${CLIENT_ROOTPATH}/scriptsdist/common.sh
  cd ${CLIENT_ROOTPATH}
...
for exp2_method in ${exp2_method_list[@]}; do
  ...
  for exp2_target_thpt in ${exp2_target_thpt_list[@]}; do
    ...
    sed -i "/^TARGET_AGGTHPT=/s/=.*/="${exp2_target_thpt}"/" scriptsdist/local/calculate_target_helper.py
    ...
    bash scriptsdist/remote/test_server_rotation_latency.sh
