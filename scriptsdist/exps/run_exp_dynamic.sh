...
if [ $# -ne 1 ]; then
	echo "Usage: bash scriptsdist/exps/run_exp_dynamic.sh <roundnumber>"
	exit
fi
roundnumber=$1
...
for exp7_method in ${exp7_method_list[@]}; do
  ### Preparation
  sed -i "/^DIRNAME=/s/=.*/=\"${exp7_method}\"/" ${CLIENT_ROOTPATH}/scriptsdist/common.sh
  cd ${CLIENT_ROOTPATH}
  ...
  for exp7_rule in ${exp7_dynamic_rule_list[@]}; do
    ...
    # ssh -i /root/${SWITCH_PRIVATEKEY} root@${LEAFSWITCH} "
    cd ${SWITCH_ROOTPATH}/${exp7_method}; bash localscriptsdist/stopswitchtestbed.sh
    # exit
  done
done
