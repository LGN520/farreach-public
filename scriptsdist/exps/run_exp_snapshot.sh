...
if [ $# -ne 1 ]; then
	echo "Usage: bash scriptsdist/exps/run_exp_snapshot.sh <roundnumber>"
	exit
fi
roundnumber=$1
...
### Prepare for DIRNAME
sed -i "/^DIRNAME=/s/=.*/=\"${exp8_method}\"/" ${CLIENT_ROOTPATH}/scriptsdist/common.sh
cd ${CLIENT_ROOTPATH}
...
for exp8_rule in ${exp8_dynamic_rule_list[@]}; do
  for exp8_snapshot in ${exp8_snapshot_list[@]}; do
    ...
    bash scriptsdist/remote/sync_file.sh ${exp8_method} config.ini
