...
if [ $# -ne 0 ]; then
	echo "Usage: bash scriptsdist/exps/run_exp_recovery.sh"
	exit
fi
exp9_workloadmode=0
exp9_recoveryonly=0
...
### Prepare for DIRNAME
sed -i "/^DIRNAME=/s/=.*/=\"${exp9_method}\"/" ${CLIENT_ROOTPATH}/scriptsdist/common.sh
cd ${CLIENT_ROOTPATH}
...
for exp9_cachesize in ${exp9_cachesize_list[@]}; do
  ...
  # bash scriptsdist/remote/sync_file.sh ${exp9_method} config.ini
  ...
  if [ ${exp9_recoveryonly} -eq 0 ]; then
	...
	then
      ...
      cd ${SWITCH_ROOTPATH}
      bash scriptsdist/remote/test_dynamic.sh
    else
  ...
  for exp9_roundnumber in ${exp9_round_list[@]}; do
    ...
    cd ${CLIENT_ROOTPATH}
    bash scriptsdist/remote/test_recovery_time.sh > tmp_test_recovery_time.out 2>&1
    ...
    python scriptsdist/local/calculate_recovery_time_helper.py ${exp9_output_path}/${exp9_cachesize} ${exp9_server_scale}
  done
done
