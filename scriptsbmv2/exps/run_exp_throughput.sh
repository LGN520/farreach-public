...
for exp1_method in ${exp1_method_list[@]}; do
  ...
  for exp1_workload in ${exp1_core_workload_list[@]}; do
    ...
    cp ${CLIENT_ROOTPATH}/${exp1_method}/configs/config.ini.bmv2 ${CLIENT_ROOTPATH}/${exp1_method}/config.ini
    ...
    ssh -i /root/${SWITCH_PRIVATEKEY} root@${LEAFSWITCH} "cd ${SWITCH_ROOTPATH}/${exp1_method}; bash localscriptsbmv2/stopswitchtestbed.sh"
  done
done
