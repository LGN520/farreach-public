...
exp7_server_scale=4
exp7_server_scale_for_rotation=4
...
for exp7_method in ${exp7_method_list[@]}; do
  ...
  for exp7_rule in ${exp7_dynamic_rule_list[@]}; do
    ...
    cp ${CLIENT_ROOTPATH}/${exp7_method}/configs/config.ini.bmv2 ${CLIENT_ROOTPATH}/${exp7_method}/config.ini
    ...
    bash scriptsbmv2/remote/sync_file.sh ${exp7_method} config.ini
