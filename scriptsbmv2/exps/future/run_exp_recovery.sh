...
if [ $# -ne 2 ]; then
	echo "Usage: bash scriptsbmv2/exps/run_exp_recovery.sh <workloadmode = 0> <recoveryonly>"
	exit
fi
exp9_workloadmode=$1
exp9_recoveryonly=$2
...
for exp9_cachesize in ${exp9_cachesize_list[@]}; do
  ...
  if [ ${exp9_recoveryonly} -eq 0 ]; then
    ...
    ssh -i /root/${SWITCH_PRIVATEKEY} root@${LEAFSWITCH} "cd ${SWITCH_ROOTPATH}/${exp9_method}/bmv2; python network.py > tmp_start_switch.out 2>&1 &"
