...
if [ $# -ne 0 ]; then
	echo "Usage: bash scriptsbmv2/remote/test_server_rotation_latency.sh"
	exit
fi
...
source scriptsbmv2/remote/test_server_rotation_p1.sh 0 ${perrotation_targets[0]}
...
for rotateidx in $(seq 0 $(expr ${server_total_logical_num_for_rotation} - 1)); do
	...
	source scriptsbmv2/remote/test_server_rotation_p2.sh 0 ${rotateidx} ${perrotation_targets[${rotatecnt}]}
	rotatecnt=$((++rotatecnt))
done
