...
then
	echo "[ERROR] you should change DIRNAME as farreach in scriptsbmv2/common.sh before running test_recovery_time.sh"
	exit
fi
...
#ssh ${USER}@${SERVER0} "cd ${SERVER_ROOTPATH}/${DIRNAME}; bash localscriptsbmv2/fetchbackup_client2server.sh"
#end_time_2_1=`date +%s.%N`
#ssh ${USER}@${SERVER1} "cd ${SERVER_ROOTPATH}/${DIRNAME}; bash localscriptsbmv2/fetchbackup_client2server.sh"
...
${SERVER_ROOTPATH}/${DIRNAME}/localscriptsbmv2/fetchbackup_client2server.sh 
# mx h4 bash ${SERVER_ROOTPATH}/${DIRNAME}/localscriptsbmv2/fetchbackup_client2server.sh 
end_time_2=`date +%s.%N`
...
then
	source scriptsbmv2/remote/prepare_server_rotation.sh
else
...
source scriptsbmv2/remote/launchservertestbed.sh recover
...
# Fetch all to switch
# ssh ${USER}@${LEAFSWITCH} "
cd ${SWITCH_ROOTPATH}/${DIRNAME}; bash localscriptsbmv2/fetchall_all2switch.sh
...
# ssh -i /root/${SWITCH_PRIVATEKEY} root@${LEAFSWITCH} "
cd ${SWITCH_ROOTPATH}/${DIRNAME}; rm tmp_launchswitchostestbed.out; bash localscriptsbmv2/launchswitchostestbed.sh recover >tmp_launchswitchostestbed.out 2>&1
sleep 5s
