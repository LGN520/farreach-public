...
echo "stop servers"
ssh ${USER}@dl13 "cd ${SERVER_ROOTPATH}/${DIRNAME}; bash stop_server.sh >/dev/null 2>&1"
ssh ${USER}@dl16 "cd ${SERVER_ROOTPATH}/${DIRNAME}; bash stop_server.sh >/dev/null 2>&1"
echo "stop clients"
bash stop_client.sh >/dev/null 2>&1
ssh ${USER}@${SECONDARY_CLIENT} "cd ${CLIENT_ROOTPATH}/${DIRNAME}; bash stop_client.sh >/dev/null 2>&1"
sleep 1s
...
do
	...
	echo "stop servers"
	ssh ${USER}@dl13 "cd ${SERVER_ROOTPATH}/${DIRNAME}; bash stop_server.sh >/dev/null"
	ssh ${USER}@dl16 "cd ${SERVER_ROOTPATH}/${DIRNAME}; bash stop_server.sh >/dev/null"
	echo "stop clients"
	bash stop_client.sh >/dev/null 2>&1
	ssh ${USER}@${SECONDARY_CLIENT} "cd ${CLIENT_ROOTPATH}/${DIRNAME}; bash stop_client.sh >/dev/null"
	sleep 1s
