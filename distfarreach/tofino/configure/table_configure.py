...
from distfarreach.p4_pd_rpc.ttypes import *
...
class TableConfigure(pd_base_tests.ThriftInterfaceDataPlane):
    def __init__(self):
        ...
        pd_base_tests.ThriftInterfaceDataPlane.__init__(self, ["distfarreach"])
...
#                    matchspec0 = eval("distfarreach_update_val{}_tbl_match_spec_t".format(valname))(
...
#            self.mirror.mirror_session_create(self.sess_hdl, self.dev_tgt, info)
            ...
            for i in range(client_physical_num):
                matchspec0 = distfarreach_l2l3_forward_tbl_match_spec_t(\
                ...
                actnspec0 = distfarreach_l2l3_forward_action_spec_t(self.client_devports[i])
            ...
            for i in range(server_physical_num):
                matchspec0 = distfarreach_l2l3_forward_tbl_match_spec_t(\
                        ...
                        ipv4_hdr_dstAddr_prefix_length = 32)
                actnspec0 = distfarreach_l2l3_forward_action_spec_t(self.server_devports[i])
                self.client.l2l3_forward_tbl_table_add_with_l2l3_forward(\
                        self.sess_hdl, self.dev_tgt, matchspec0, actnspec0)
            ...
            #        matchspec0 = distfarreach_need_recirculate_tbl_match_spec_t(\
            #                op_hdr_optype = tmpoptype,
            ...
            actnspec0 = distfarreach_set_hot_threshold_action_spec_t(hot_threshold)
            ...
            for tmpoptype in [PUTREQ, DELREQ]:
                matchspec0 = distfarreach_recirculate_tbl_match_spec_t(\
                        op_hdr_optype = tmpoptype,
                        meta_need_recirculate = 1)
                # recirculate to the pipeline of the first physical client for atomicity of setting snapshot flag
                actnspec0 = distfarreach_recirculate_pkt_action_spec_t(self.recirPorts[client_pipeidxes[0]])
            ...
            if RANGE_SUPPORT == False:
                ...
                for tmpoptype in [GETREQ, CACHE_POP_INSWITCH, PUTREQ, DELREQ, WARMUPREQ, LOADREQ, CACHE_EVICT_LOADFREQ_INSWITCH, CACHE_EVICT_LOADDATA_INSWITCH, LOADSNAPSHOTDATA_INSWITCH, SETVALID_INSWITCH]:
                    matchspec0 = distfarreach_hash_for_partition_tbl_match_spec_t(\
            ...
            if RANGE_SUPPORT == True:
                ...
                for tmpoptype in [GETREQ, CACHE_POP_INSWITCH, PUTREQ, DELREQ, WARMUPREQ, SCANREQ, LOADREQ, CACHE_EVICT_LOADFREQ_INSWITCH, CACHE_EVICT_LOADDATA_INSWITCH, LOADSNAPSHOTDATA_INSWITCH, SETVALID_INSWITCH]:
                    ...
                    for global_server_logical_idx in range(server_total_logical_num):
                        ...
                        matchspec0 = distfarreach_range_partition_tbl_match_spec_t(\
                                op_hdr_optype = tmpoptype,
                        ...
                        if server_physical_idx == -1:
                            ...
                            if tmpoptype != SCANREQ:
                                actnspec0 = distfarreach_range_partition_action_spec_t(udp_dstport, eport)
                                ...
                                actnspec0 = distfarreach_range_partition_for_scan_action_spec_t(udp_dstport, eport, global_server_logical_idx)
                ...
                for tmpoptype in [GETREQ, CACHE_POP_INSWITCH, PUTREQ, DELREQ, WARMUPREQ, LOADREQ, CACHE_EVICT_LOADFREQ_INSWITCH, CACHE_EVICT_LOADDATA_INSWITCH, LOADSNAPSHOTDATA_INSWITCH, SETVALID_INSWITCH]:
                    ...
                    for global_server_logical_idx in range(server_total_logical_num):
                        ...
                        # NOTE: both start and end are included
                        matchspec0 = distfarreach_hash_partition_tbl_match_spec_t(\
                                op_hdr_optype = convert_u16_to_i16(tmpoptype),
                        ...
                        if server_physical_idx == -1:
                            ...
                            udp_dstport = server_worker_port_start + local_server_logical_idx
                            eport = self.server_devports[server_physical_idx]
                            actnspec0 = distfarreach_hash_partition_action_spec_t(udp_dstport, eport)
            ...
            if RANGE_SUPPORT == True:
                ...
                for global_server_logical_idx in range(server_total_logical_num):
                    ...
                    # NOTE: both start and end are included
                    matchspec0 = distfarreach_range_partition_for_scan_endkey_tbl_match_spec_t(\
                    ...
                    #actnspec0 = distfarreach_range_partition_for_scan_endkey_action_spec_t(last_udpport_plus_one)
                    end_globalserveridx_plus_one = global_server_logical_idx + 1 # used to calculate max_scannum in data plane
                    actnspec0 = distfarreach_range_partition_for_scan_endkey_action_spec_t(end_globalserveridx_plus_one)
            ...
            for i in range(1, 5):
                ...
                for tmpoptype in [GETREQ, PUTREQ]:
                    matchspec0 = eval("distfarreach_hash_for_cm{}_tbl_match_spec_t".format(i))(\
                            op_hdr_optype = tmpoptype,
                            meta_need_recirculate = 0)
            ...
            for tmpoptype in [PUTREQ, DELREQ]:
                matchspec0 = distfarreach_hash_for_seq_tbl_match_spec_t(\
                        op_hdr_optype = tmpoptype,
                        meta_need_recirculate = 0)
            ...
            #    matchspec0 = distfarreach_snapshot_flag_tbl_match_spec_t(\
            #            op_hdr_optype = tmpoptype,
            #            meta_need_recirculate = 0)
            ...
            for tmpoptype in [GETREQ, PUTREQ, DELREQ]:
                for tmp_client_physical_idx in range(client_physical_num):
                    matchspec0 = distfarreach_prepare_for_cachehit_tbl_match_spec_t(\
                            op_hdr_optype = tmpoptype,
                            ig_intr_md_ingress_port = self.client_devports[tmp_client_physical_idx],
                            meta_need_recirculate = 0)
                    actnspec0 = distfarreach_set_client_sid_action_spec_t(self.client_sids[tmp_client_physical_idx])
                ...
                #            meta_need_recirculate = 0)
                #    actnspec0 = distfarreach_set_client_sid_action_spec_t(self.server_sids[tmp_server_physical_idx])
                #    self.client.prepare_for_cachehit_tbl_table_add_with_set_client_sid(\
                #            self.sess_hdl, self.dev_tgt, matchspec0, actnspec0)
            # set default sid as sids[0]
            #actnspec0 = distfarreach_set_client_sid_action_spec_t(self.sids[0])
            ...
            for tmp_client_physical_idx in range(client_physical_num):
                ...
                for tmpoptype in [GETRES, PUTRES, DELRES, WARMUPACK, SCANRES_SPLIT, LOADACK]:
                    matchspec0 = distfarreach_ipv4_forward_tbl_match_spec_t(\
                            op_hdr_optype = convert_u16_to_i16(tmpoptype),
                            ipv4_hdr_dstAddr = ipv4addr0,
                            ipv4_hdr_dstAddr_prefix_length = 32,
                            meta_need_recirculate = 0)
                    actnspec0 = distfarreach_forward_normal_response_action_spec_t(eport)
                ...
                for tmpoptype in [GETRES_LATEST_SEQ, GETRES_DELETED_SEQ]:
                    matchspec0 = distfarreach_ipv4_forward_tbl_match_spec_t(\
                            ...
                            ipv4_hdr_dstAddr_prefix_length = 32,
                            meta_need_recirculate = 0)
                    actnspec0 = distfarreach_forward_special_get_response_action_spec_t(tmpsid)
            ...
            for tmpoptype in [GETREQ, PUTREQ]:
                matchspec0 = distfarreach_sample_tbl_match_spec_t(\
                        op_hdr_optype = tmpoptype,
                        meta_need_recirculate = 0)
            ...
            matchspec0 = distfarreach_ig_port_forward_tbl_match_spec_t(\
            ...
            for i in range(1, cm_hashnum+1):
                ...
                for tmpoptype in [GETREQ_INSWITCH, PUTREQ_INSWITCH]:
                    matchspec0 = eval("distfarreach_access_cm{}_tbl_match_spec_t".format(i))(\
                            op_hdr_optype = tmpoptype,
            ...
            if RANGE_SUPPORT:
                ...
                for is_clone in [0, 1]:
                    for global_server_logical_idx in range(server_total_logical_num):
                        ...
                        matchspec0 = distfarreach_process_scanreq_split_tbl_match_spec_t(\
                        ...
                        if is_clone == 0:
                            ...
                            if server_physical_idx == -1:
                                ...
                                actnspec0 = distfarreach_process_scanreq_split_action_spec_t(tmp_server_sid)
                            ...
                            if current_local_server_logical_idx == -1 or next_server_physical_idx == -1:
                                ...
                                actnspec0 = distfarreach_process_cloned_scanreq_split_action_spec_t(tmp_udpport, tmp_server_sid)
            ...
            matchspec0 = distfarreach_is_hot_tbl_match_spec_t(\
            ...
            for tmpoptype in [GETREQ_INSWITCH, PUTREQ_INSWITCH]:
                matchspec0 = distfarreach_access_cache_frequency_tbl_match_spec_t(\
                        op_hdr_optype = tmpoptype,
                        inswitch_hdr_is_sampled = 1,
            ...
            for tmpoptype in [GETREQ_INSWITCH, GETRES_LATEST_SEQ_INSWITCH, GETRES_DELETED_SEQ_INSWITCH, PUTREQ_INSWITCH, DELREQ_INSWITCH]:
                matchspec0 = distfarreach_access_validvalue_tbl_match_spec_t(\
                        op_hdr_optype = tmpoptype,
                        inswitch_hdr_is_cached = 1)
            ...
            for tmpoptype in [PUTREQ_INSWITCH, DELREQ_INSWITCH]:
                matchspec0 = distfarreach_access_seq_tbl_match_spec_t(\
                        op_hdr_optype = tmpoptype)
            ...
            for tmpoptype in[GETREQ_INSWITCH, PUTREQ_INSWITCH, DELREQ_INSWITCH]:
                matchspec0 = distfarreach_save_client_udpport_tbl_match_spec_t(\
                        op_hdr_optype = tmpoptype)
            ...
            for is_cached in cached_list:
                for validvalue in validvalue_list:
                    matchspec0 = distfarreach_access_latest_tbl_match_spec_t(\
                            op_hdr_optype = GETREQ_INSWITCH,
                            inswitch_hdr_is_cached = is_cached,
                            validvalue_hdr_validvalue = validvalue)
                    if is_cached == 1:
                ...
                for validvalue in validvalue_list:
                    for is_latest in latest_list:
                        for is_stat in stat_list:
                            matchspec0 = distfarreach_access_deleted_tbl_match_spec_t(\
                                    op_hdr_optype = GETREQ_INSWITCH,
                                    inswitch_hdr_is_cached = is_cached,
                                    validvalue_hdr_validvalue = validvalue,
                                    meta_is_latest = is_latest,
            ...
            for is_cached in cached_list:
                for validvalue in validvalue_list:
                    for is_latest in latest_list:
                        matchspec0 = distfarreach_update_vallen_tbl_match_spec_t(\
                                op_hdr_optype = GETREQ_INSWITCH,
                                inswitch_hdr_is_cached = is_cached,
                                validvalue_hdr_validvalue = validvalue,
                                meta_is_latest = is_latest)
            ...
            for is_cached in cached_list:
                for validvalue in validvalue_list:
                    for is_latest in latest_list:
                        for tmpoptype in [PUTREQ_INSWITCH, DELREQ_INSWITCH]:
                            matchspec0 = distfarreach_access_savedseq_tbl_match_spec_t(\
                                    op_hdr_optype = tmpoptype,
                                    inswitch_hdr_is_cached = is_cached,
                                    validvalue_hdr_validvalue = validvalue,
                                    meta_is_latest = is_latest)
            ...
            for is_latest in latest_list:
                for tmpoptype in [GETRES_LATEST_SEQ_INSWITCH, GETRES_DELETED_SEQ_INSWITCH]:
                    matchspec0 = distfarreach_access_case1_tbl_match_spec_t(\
                            op_hdr_optype = tmpoptype,
                            inswitch_hdr_is_cached = 1,
            ...
            if RANGE_SUPPORT == False:
                ...
                for tmpoptype in [GETRES_LATEST_SEQ_INSWITCH_CASE1, GETRES_DELETED_SEQ_INSWITCH_CASE1, PUTREQ_SEQ_INSWITCH_CASE1, DELREQ_SEQ_INSWITCH_CASE1]:
                    matchspec0 = distfarreach_lastclone_lastscansplit_tbl_match_spec_t(\
                            op_hdr_optype = tmpoptype,
            ...
            for i in range(switch_max_vallen/8 + 1): # i from 0 to 16
                ...
                matchspec0 = distfarreach_update_pktlen_tbl_match_spec_t(\
                        op_hdr_optype=GETRES,
                        vallen_hdr_vallen_start=vallen_start,
                        vallen_hdr_vallen_end=vallen_end) # [vallen_start, vallen_end]
                actnspec0 = distfarreach_update_pktlen_action_spec_t(val_stat_udplen, val_stat_iplen)
                ...
                for tmpoptype in [GETRES_LATEST_SEQ_INSWITCH_CASE1, GETRES_DELETED_SEQ_INSWITCH_CASE1, PUTREQ_SEQ_INSWITCH_CASE1, DELREQ_SEQ_INSWITCH_CASE1]:
                    matchspec0 = distfarreach_update_pktlen_tbl_match_spec_t(\
                            ...
                            vallen_hdr_vallen_start=vallen_start,
                            vallen_hdr_vallen_end=vallen_end) # [vallen_start, vallen_end]
                    actnspec0 = distfarreach_update_pktlen_action_spec_t(val_seq_inswitch_stat_clone_udplen, val_seq_inswitch_stat_clone_iplen)
                    self.client.update_pktlen_tbl_table_add_with_update_pktlen(\
                            self.sess_hdl, self.dev_tgt, matchspec0, 0, actnspec0) # 0 is priority (range may be overlapping)
                for tmpoptype in [PUTREQ_SEQ, PUTREQ_POP_SEQ, PUTREQ_SEQ_CASE3, PUTREQ_POP_SEQ_CASE3]:
                    matchspec0 = distfarreach_update_pktlen_tbl_match_spec_t(\
                            ...
                            vallen_hdr_vallen_start=vallen_start,
                            vallen_hdr_vallen_end=vallen_end) # [vallen_start, vallen_end]
                    actnspec0 = distfarreach_update_pktlen_action_spec_t(val_seq_udplen, val_seq_iplen)
                    self.client.update_pktlen_tbl_table_add_with_update_pktlen(\
                            self.sess_hdl, self.dev_tgt, matchspec0, 0, actnspec0) # 0 is priority (range may be overlapping)
                matchspec0 = distfarreach_update_pktlen_tbl_match_spec_t(\
                        ...
                        vallen_hdr_vallen_start=vallen_start,
                        vallen_hdr_vallen_end=vallen_end) # [vallen_start, vallen_end]
                actnspec0 = distfarreach_update_pktlen_action_spec_t(val_seq_stat_udplen, val_seq_stat_iplen)
                self.client.update_pktlen_tbl_table_add_with_update_pktlen(\
                        self.sess_hdl, self.dev_tgt, matchspec0, 0, actnspec0) # 0 is priority (range may be overlapping)
                matchspec0 = distfarreach_update_pktlen_tbl_match_spec_t(\
                        ...
                        vallen_hdr_vallen_start=vallen_start,
                        vallen_hdr_vallen_end=vallen_end) # [vallen_start, vallen_end]
                actnspec0 = distfarreach_update_pktlen_action_spec_t(val_seq_inswitch_stat_udplen, val_seq_inswitch_stat_iplen)
                self.client.update_pktlen_tbl_table_add_with_update_pktlen(\
                        self.sess_hdl, self.dev_tgt, matchspec0, 0, actnspec0) # 0 is priority (range may be overlapping)
            ## DEPRECATED: including 1B debug_hdr
            #onlyop_udplen = 27
            ...
            actnspec0 = distfarreach_update_pktlen_action_spec_t(onlyop_udplen, onlyop_iplen)
            self.client.update_pktlen_tbl_table_add_with_update_pktlen(\
                    self.sess_hdl, self.dev_tgt, matchspec0, 0, actnspec0) # 0 is priority (range may be overlapping)
            for tmpoptype in [PUTRES, DELRES]:
                matchspec0 = distfarreach_update_pktlen_tbl_match_spec_t(\
                        ...
                        vallen_hdr_vallen_start=0,
                        vallen_hdr_vallen_end=switch_max_vallen) # [0, 128]
                actnspec0 = distfarreach_update_pktlen_action_spec_t(stat_udplen, stat_iplen)
                self.client.update_pktlen_tbl_table_add_with_update_pktlen(\
                        self.sess_hdl, self.dev_tgt, matchspec0, 0, actnspec0) # 0 is priority (range may be overlapping)
            for tmpoptype in [DELREQ_SEQ, DELREQ_SEQ_CASE3]:
                matchspec0 = distfarreach_update_pktlen_tbl_match_spec_t(\
                        ...
                        vallen_hdr_vallen_start=0,
                        vallen_hdr_vallen_end=switch_max_vallen) # [0, 128]
                actnspec0 = distfarreach_update_pktlen_action_spec_t(seq_udplen, seq_iplen)
                self.client.update_pktlen_tbl_table_add_with_update_pktlen(\
                        self.sess_hdl, self.dev_tgt, matchspec0, 0, actnspec0) # 0 is priority (range may be overlapping)
            matchspec0 = distfarreach_update_pktlen_tbl_match_spec_t(\
                    op_hdr_optype=SCANREQ_SPLIT,
                    vallen_hdr_vallen_start=0,
                    vallen_hdr_vallen_end=switch_max_vallen) # [0, 128]
            actnspec0 = distfarreach_update_pktlen_action_spec_t(scanreqsplit_udplen, scanreqsplit_iplen)
            self.client.update_pktlen_tbl_table_add_with_update_pktlen(\
                    self.sess_hdl, self.dev_tgt, matchspec0, 0, actnspec0) # 0 is priority (range may be overlapping)
            matchspec0 = distfarreach_update_pktlen_tbl_match_spec_t(\
                    ...
                    vallen_hdr_vallen_start=0,
                    vallen_hdr_vallen_end=switch_max_vallen) # [0, 128]
            actnspec0 = distfarreach_update_pktlen_action_spec_t(frequency_udplen, frequency_iplen)
            self.client.update_pktlen_tbl_table_add_with_update_pktlen(\
                    self.sess_hdl, self.dev_tgt, matchspec0, 0, actnspec0) # 0 is priority (range may be overlapping)
            matchspec0 = distfarreach_update_pktlen_tbl_match_spec_t(\
            ...
            for tmp_client_physical_idx in range(client_physical_num):
                ...
                actnspec0 = distfarreach_update_ipmac_srcport_server2client_action_spec_t(\
                ...
                matchspec0 = distfarreach_update_ipmac_srcport_tbl_match_spec_t(\
                        op_hdr_optype = convert_u16_to_i16(GETRES), 
                        eg_intr_md_egress_port = tmp_devport)
                matchspec1 = distfarreach_update_ipmac_srcport_tbl_match_spec_t(\
                        op_hdr_optype=convert_u16_to_i16(PUTRES), 
                        eg_intr_md_egress_port = tmp_devport)
                matchspec2 = distfarreach_update_ipmac_srcport_tbl_match_spec_t(\
                        op_hdr_optype=convert_u16_to_i16(DELRES),
                        eg_intr_md_egress_port = tmp_devport)
                matchspec3 = distfarreach_update_ipmac_srcport_tbl_match_spec_t(\
                        op_hdr_optype=convert_u16_to_i16(SCANRES_SPLIT),
                        eg_intr_md_egress_port = tmp_devport)
                matchspec4 = distfarreach_update_ipmac_srcport_tbl_match_spec_t(\
                        op_hdr_optype=convert_u16_to_i16(WARMUPACK),
                        eg_intr_md_egress_port = tmp_devport)
                matchspec5 = distfarreach_update_ipmac_srcport_tbl_match_spec_t(\
            ...
            for tmp_server_physical_idx in range(server_physical_num):
                ...
                actnspec1 = distfarreach_update_dstipmac_client2server_action_spec_t(\
                        macAddr_to_string(tmp_server_mac), \
                        ipv4Addr_to_i32(tmp_server_ip))
            ...
            actnspec2 = distfarreach_update_ipmac_srcport_switch2switchos_action_spec_t(\
                    macAddr_to_string(tmp_client_mac), \
                    macAddr_to_string(self.reflector_mac_for_switch), \
                    ipv4Addr_to_i32(tmp_client_ip), \
                    ipv4Addr_to_i32(self.reflector_ip_for_switch), \
                    tmp_client_port)
            matchspec6 = distfarreach_update_ipmac_srcport_tbl_match_spec_t(\
                    op_hdr_optype=convert_u16_to_i16(GETRES_LATEST_SEQ_INSWITCH_CASE1), 
                    eg_intr_md_egress_port=tmp_devport)
            matchspec7 = distfarreach_update_ipmac_srcport_tbl_match_spec_t(\
                    op_hdr_optype=convert_u16_to_i16(GETRES_DELETED_SEQ_INSWITCH_CASE1),
                    eg_intr_md_egress_port=tmp_devport)
            matchspec8 = distfarreach_update_ipmac_srcport_tbl_match_spec_t(\
                    op_hdr_optype=convert_u16_to_i16(CACHE_POP_INSWITCH_ACK),
                    eg_intr_md_egress_port=tmp_devport)
            matchspec9 = distfarreach_update_ipmac_srcport_tbl_match_spec_t(\
                    op_hdr_optype=convert_u16_to_i16(CACHE_EVICT_LOADFREQ_INSWITCH_ACK),
                    eg_intr_md_egress_port=tmp_devport)
            matchspec10 = distfarreach_update_ipmac_srcport_tbl_match_spec_t(\
                    op_hdr_optype=convert_u16_to_i16(CACHE_EVICT_LOADDATA_INSWITCH_ACK),
                    eg_intr_md_egress_port=tmp_devport)
            matchspec11 = distfarreach_update_ipmac_srcport_tbl_match_spec_t(\
                    op_hdr_optype=convert_u16_to_i16(LOADSNAPSHOTDATA_INSWITCH_ACK),
                    eg_intr_md_egress_port=tmp_devport)
            matchspec12 = distfarreach_update_ipmac_srcport_tbl_match_spec_t(\
            ...
            for tmpoptype in [PUTREQ_SEQ, PUTREQ_POP_SEQ, PUTREQ_SEQ_CASE3, PUTREQ_POP_SEQ_CASE3, GETRES_LATEST_SEQ_INSWITCH_CASE1, GETRES_DELETED_SEQ_INSWITCH_CASE1, PUTREQ_SEQ_INSWITCH_CASE1, DELREQ_SEQ_INSWITCH_CASE1, GETRES, LOADREQ, CACHE_EVICT_LOADDATA_INSWITCH_ACK, LOADSNAPSHOTDATA_INSWITCH_ACK]:
                for i in range(switch_max_vallen/8 + 1): # i from 0 to 16
                    if i == 0:
                        ...
                        vallen_start = (i-1)*8+1 # 1, 9, ..., 121
                        vallen_end = (i-1)*8+8 # 8, 16, ..., 128
                    matchspec0 = distfarreach_add_and_remove_value_header_tbl_match_spec_t(\
                            op_hdr_optype=tmpoptype,
                            vallen_hdr_vallen_start=vallen_start,
                            vallen_hdr_vallen_end=vallen_end) # [vallen_start, vallen_end]
                    if i == 0:
            ...
            matchspec0 = distfarreach_drop_tbl_match_spec_t(\
                    op_hdr_optype = GETRES_LATEST_SEQ_INSWITCH)
                                    ...
                                    for snapshot_flag in snapshot_flag_list:
                                        for is_case1 in case1_list:
                                            ...
                                            if is_lastclone_for_pktloss == 0 and snapshot_flag == 0 and is_case1 == 0 and tmp_client_sid != 0:
                                                ...
                                                matchspec0 = distfarreach_eg_port_forward_tbl_match_spec_t(\
                                                    op_hdr_optype = GETREQ_INSWITCH,
                                                    inswitch_hdr_is_cached = is_cached,
                                                    meta_is_hot = is_hot,
                                                    validvalue_hdr_validvalue = validvalue,
                                                ...
                                                if is_cached == 0:
                                                    if is_hot == 1:
                                                        ...
                                                        #actnspec0 = distfarreach_update_getreq_inswitch_to_getreq_pop_action_spec_t(self.devPorts[1])
                                                        ...
                                                        #actnspec0 = distfarreach_update_getreq_inswitch_to_getreq_action_spec_t(self.devPorts[1])
                                                    ...
                                                    if validvalue == 0:
                                                        ...
                                                        if is_latest == 0:
                                                            ...
                                                            #actnspec0 = distfarreach_update_getreq_inswitch_to_getreq_nlatest_action_spec_t(self.devPorts[1])
                                                            ...
                                                            actnspec0 = distfarreach_update_getreq_inswitch_to_getres_by_mirroring_action_spec_t(tmp_client_sid, server_worker_port_start, tmpstat)
                                            ...
                                            if is_hot == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0:
                                                ...
                                                if is_cached == 1 and validvalue == 1 and is_latest == 0 and snapshot_flag == 1 and is_case1 == 0:
                                                    ...
                                                    if is_deleted == 0: # is_deleted=0 -> stat=1
                                                        #actnspec0 = distfarreach_update_getres_latest_seq_inswitch_to_getres_latest_seq_inswitch_case1_clone_for_pktloss_action_spec_t(self.reflector_sid, self.devPorts[1], 1)
                                                        actnspec0 = distfarreach_update_getres_latest_seq_inswitch_to_getres_latest_seq_inswitch_case1_clone_for_pktloss_action_spec_t(self.reflector_sid, 1, reflector_dp2cpserver_port)
                                                    elif is_deleted == 1: # is_deleted=1 -> stat=0
                                                        #actnspec0 = distfarreach_update_getres_latest_seq_inswitch_to_getres_latest_seq_inswitch_case1_clone_for_pktloss_action_spec_t(self.reflector_sid, self.devPorts[1], 0)
                                                        actnspec0 = distfarreach_update_getres_latest_seq_inswitch_to_getres_latest_seq_inswitch_case1_clone_for_pktloss_action_spec_t(self.reflector_sid, 0, reflector_dp2cpserver_port)
                                            ...
                                            if is_cached == 1 and snapshot_flag == 1 and is_hot == 0 and validvalue == 0 and is_latest == 0 and is_deleted == 0 and tmp_client_sid == 0 and is_case1 == 0:
                                                ...
                                                if is_lastclone_for_pktloss == 0:
                                                    ...
                                                    actnspec0 = distfarreach_forward_getres_latest_seq_inswitch_case1_clone_for_pktloss_action_spec_t(self.reflector_sid)
                                            ...
                                            if is_hot == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0:
                                                ...
                                                if is_cached == 1 and validvalue == 1 and is_latest == 0 and snapshot_flag == 1 and is_case1 == 0:
                                                    ...
                                                    if is_deleted == 0: # is_deleted=0 -> stat=1
                                                        #actnspec0 = distfarreach_update_getres_deleted_seq_inswitch_to_getres_deleted_seq_inswitch_case1_clone_for_pktloss_action_spec_t(self.reflector_sid, self.devPorts[1], 1)
                                                        actnspec0 = distfarreach_update_getres_deleted_seq_inswitch_to_getres_deleted_seq_inswitch_case1_clone_for_pktloss_action_spec_t(self.reflector_sid, 1, reflector_dp2cpserver_port)
                                                    elif is_deleted == 1: # is_deleted=1 -> stat=0
                                                        #actnspec0 = distfarreach_update_getres_deleted_seq_inswitch_to_getres_deleted_seq_inswitch_case1_clone_for_pktloss_action_spec_t(self.reflector_sid, self.devPorts[1], 0)
                                                        actnspec0 = distfarreach_update_getres_deleted_seq_inswitch_to_getres_deleted_seq_inswitch_case1_clone_for_pktloss_action_spec_t(self.reflector_sid, 0, reflector_dp2cpserver_port)
                                            ...
                                            if is_cached == 1 and snapshot_flag == 1 and is_hot == 0 and validvalue == 0 and is_latest == 0 and is_deleted == 0 and tmp_client_sid == 0 and is_case1 == 0:
                                                ...
                                                if is_lastclone_for_pktloss == 0:
                                                    ...
                                                    actnspec0 = distfarreach_forward_getres_deleted_seq_inswitch_case1_clone_for_pktloss_action_spec_t(self.reflector_sid)
                                            ...
                                            if is_cached == 0 and is_hot == 0 and validvalue == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0 and snapshot_flag == 0 and is_case1 == 0:
                                                ...
                                                actnspec0 = distfarreach_update_cache_pop_inswitch_to_cache_pop_inswitch_ack_drop_and_clone_action_spec_t(self.reflector_sid, reflector_dp2cpserver_port)
                                            ...
                                            if is_cached == 0 and is_hot == 0 and validvalue == 0 and is_latest == 0 and is_deleted == 0 and tmp_client_sid == 0 and snapshot_flag == 0 and is_case1 == 0 and is_lastclone_for_pktloss == 0:
                                                ...
                                                #    actnspec0 = distfarreach_forward_cache_pop_inswitch_ack_clone_for_pktloss_action_spec_t(self.reflector_sid)
                                                ...
                                                if is_cached == 0:
                                                    ...
                                                    if validvalue == 0 or validvalue == 3:
                                                        ...
                                                        if snapshot_flag == 1 and is_case1 == 0:
                                                            ...
                                                            if is_deleted == 0: # is_deleted=0 -> stat=1
                                                                #actnspec0 = distfarreach_update_putreq_inswitch_to_putreq_seq_inswitch_case1_clone_for_pktloss_and_putres_action_spec_t(self.reflector_sid, self.devPorts[1], 1)
                                                                actnspec0 = distfarreach_update_putreq_inswitch_to_putreq_seq_inswitch_case1_clone_for_pktloss_and_putres_action_spec_t(self.reflector_sid, 1, reflector_dp2cpserver_port)
                                                            elif is_deleted == 1: # is_deleted=1 -> stat=0
                                                                #actnspec0 = distfarreach_update_putreq_inswitch_to_putreq_seq_inswitch_case1_clone_for_pktloss_and_putres_action_spec_t(self.reflector_sid, self.devPorts[1], 0)
                                                                actnspec0 = distfarreach_update_putreq_inswitch_to_putreq_seq_inswitch_case1_clone_for_pktloss_and_putres_action_spec_t(self.reflector_sid, 0, reflector_dp2cpserver_port)
                                                            ...
                                                            #            self.sess_hdl, self.dev_tgt, matchspec0)
                                                            # Update PUTREQ_INSWITCH as PUTRES to client by mirroring
                                                            actnspec0 = distfarreach_update_putreq_inswitch_to_putres_by_mirroring_action_spec_t(tmp_client_sid, server_worker_port_start)
                                                            self.client.eg_port_forward_tbl_table_add_with_update_putreq_inswitch_to_putres_by_mirroring(\
                                                                    self.sess_hdl, self.dev_tgt, matchspec0, actnspec0)
                                            ...
                                            if is_cached == 1 and is_hot == 0 and validvalue == 0 and is_latest == 0 and is_deleted == 0 and snapshot_flag == 1 and is_case1 == 0:
                                                ...
                                                if is_lastclone_for_pktloss == 0:
                                                    ...
                                                    actnspec0 = distfarreach_forward_putreq_seq_inswitch_case1_clone_for_pktloss_and_putres_action_spec_t(self.reflector_sid)
                                                    ...
                                                    #            self.sess_hdl, self.dev_tgt, matchspec0)
                                                    # Update PUTREQ_SEQ_INSWITCH_CASE1 as PUTRES to client by mirroring
                                                    actnspec0 = distfarreach_update_putreq_seq_inswitch_case1_to_putres_by_mirroring_action_spec_t(tmp_client_sid, server_worker_port_start)
                                                    self.client.eg_port_forward_tbl_table_add_with_update_putreq_seq_inswitch_case1_to_putres_by_mirroring(\
                                                            self.sess_hdl, self.dev_tgt, matchspec0, actnspec0)
                                                ...
                                                if is_cached == 0:
                                                    ...
                                                    if validvalue == 0 or validvalue == 3:
                                                        ...
                                                        if snapshot_flag == 1 and is_case1 == 0:
                                                            ...
                                                            if is_deleted == 0: # is_deleted=0 -> stat=1
                                                                #actnspec0 = distfarreach_update_delreq_inswitch_to_delreq_seq_inswitch_case1_clone_for_pktloss_and_delres_action_spec_t(self.reflector_sid, self.devPorts[1], 1)
                                                                actnspec0 = distfarreach_update_delreq_inswitch_to_delreq_seq_inswitch_case1_clone_for_pktloss_and_delres_action_spec_t(self.reflector_sid, 1, reflector_dp2cpserver_port)
                                                            elif is_deleted == 1: # is_deleted=1 -> stat=0
                                                                #actnspec0 = distfarreach_update_delreq_inswitch_to_delreq_seq_inswitch_case1_clone_for_pktloss_and_delres_action_spec_t(self.reflector_sid, self.devPorts[1], 0)
                                                                actnspec0 = distfarreach_update_delreq_inswitch_to_delreq_seq_inswitch_case1_clone_for_pktloss_and_delres_action_spec_t(self.reflector_sid, 0, reflector_dp2cpserver_port)
                                                            ...
                                                            #            self.sess_hdl, self.dev_tgt, matchspec0)
                                                            # Update DELREQ_INSWITCH as DELRES to client by mirroring
                                                            actnspec0 = distfarreach_update_delreq_inswitch_to_delres_by_mirroring_action_spec_t(tmp_client_sid, server_worker_port_start)
                                                            self.client.eg_port_forward_tbl_table_add_with_update_delreq_inswitch_to_delres_by_mirroring(\
                                                                    self.sess_hdl, self.dev_tgt, matchspec0, actnspec0)
                                            ...
                                            if is_cached == 1 and is_hot == 0 and validvalue == 0 and is_latest == 0 and is_deleted == 0 and snapshot_flag == 1 and is_case1 == 0:
                                                ...
                                                if is_lastclone_for_pktloss == 0:
                                                    ...
                                                    actnspec0 = distfarreach_forward_delreq_seq_inswitch_case1_clone_for_pktloss_and_delres_action_spec_t(self.reflector_sid)
                                                    ...
                                                    #            self.sess_hdl, self.dev_tgt, matchspec0)
                                                    # Update DELREQ_SEQ_INSWITCHCASE1 as DELRES to client by mirroring
                                                    actnspec0 = distfarreach_update_delreq_seq_inswitch_case1_to_delres_by_mirroring_action_spec_t(tmp_client_sid, server_worker_port_start)
                                                    self.client.eg_port_forward_tbl_table_add_with_update_delreq_seq_inswitch_case1_to_delres_by_mirroring(\
                                                            self.sess_hdl, self.dev_tgt, matchspec0, actnspec0)
                                            ...
                                            if is_cached == 1 and is_hot == 0 and validvalue == 0 and is_latest == 0 and is_deleted == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0 and snapshot_flag == 0 and is_case1 == 0:
                                                ...
                                                actnspec0 = distfarreach_update_cache_evict_loadfreq_inswitch_to_cache_evict_loadfreq_inswitch_ack_drop_and_clone_action_spec_t(self.reflector_sid, reflector_dp2cpserver_port)
                                            ...
                                            if is_cached == 1 and is_hot == 0 and validvalue == 0 and is_latest == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0 and snapshot_flag == 0 and is_case1 == 0:
                                                ...
                                                actnspec0 = distfarreach_update_cache_evict_loaddata_inswitch_to_cache_evict_loaddata_inswitch_ack_drop_and_clone_action_spec_t(self.reflector_sid, reflector_dp2cpserver_port, tmpstat)
                                            ...
                                            if is_hot == 0 and validvalue == 0 and is_latest == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0 and snapshot_flag == 0 and is_case1 == 0:
                                                ...
                                                actnspec0 = distfarreach_update_loadsnapshotdata_inswitch_to_loadsnapshotdata_inswitch_ack_drop_and_clone_action_spec_t(self.reflector_sid, reflector_dp2cpserver_port, tmpstat)
                                            ...
                                            if is_hot == 0 and is_latest == 0 and is_deleted == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0 and snapshot_flag == 0 and is_case1 == 0:
                                                ...
                                                actnspec0 = distfarreach_update_setvalid_inswitch_to_setvalid_inswitch_ack_drop_and_clone_action_spec_t(self.reflector_sid, reflector_dp2cpserver_port)
                                                ...
                                                for tmp_server_sid in tmp_server_sids:
                                                    ...
                                                    if is_cached == 0 and is_hot == 0 and validvalue == 0 and is_latest == 0 and is_deleted == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0 and snapshot_flag == 0 and is_case1 == 0 and tmp_server_sid != 0:
                                                        ...
                                                        if is_last_scansplit == 1:
                                                            ...
                                                            actnspec0 = distfarreach_forward_scanreq_split_and_clone_action_spec_t(tmp_server_sid)
