...
#            self.mirror.mirror_session_create(self.sess_hdl, self.dev_tgt, info)
            ...
            if RANGE_SUPPORT == True:
                ...
                for global_server_logical_idx in range(server_total_logical_num):
                    ...
                    #actnspec0 = distfarreachleaf_range_partition_for_scan_endkey_action_spec_t(last_udpport_plus_one)
            ...
            actnspec2 = distfarreachleaf_update_ipmac_srcport_switch2switchos_action_spec_t(\
                    ...
                    tmp_client_port)
            matchspec6 = distfarreachleaf_update_ipmac_srcport_tbl_match_spec_t(\
                    op_hdr_optype=convert_u16_to_i16(GETRES_LATEST_SEQ_INSWITCH_CASE1), 
                    eg_intr_md_egress_port=tmp_devport)
            matchspec7 = distfarreachleaf_update_ipmac_srcport_tbl_match_spec_t(\
                    op_hdr_optype=convert_u16_to_i16(GETRES_DELETED_SEQ_INSWITCH_CASE1),
                    eg_intr_md_egress_port=tmp_devport)
            matchspec8 = distfarreachleaf_update_ipmac_srcport_tbl_match_spec_t(\
                    op_hdr_optype=convert_u16_to_i16(CACHE_POP_INSWITCH_ACK),
                    eg_intr_md_egress_port=tmp_devport)
            matchspec9 = distfarreachleaf_update_ipmac_srcport_tbl_match_spec_t(\
                    op_hdr_optype=convert_u16_to_i16(CACHE_EVICT_LOADFREQ_INSWITCH_ACK),
                    eg_intr_md_egress_port=tmp_devport)
            matchspec10 = distfarreachleaf_update_ipmac_srcport_tbl_match_spec_t(\
                    op_hdr_optype=convert_u16_to_i16(CACHE_EVICT_LOADDATA_INSWITCH_ACK),
                    eg_intr_md_egress_port=tmp_devport)
            matchspec11 = distfarreachleaf_update_ipmac_srcport_tbl_match_spec_t(\
                    op_hdr_optype=convert_u16_to_i16(LOADSNAPSHOTDATA_INSWITCH_ACK),
                    eg_intr_md_egress_port=tmp_devport)
            matchspec12 = distfarreachleaf_update_ipmac_srcport_tbl_match_spec_t(\
                                                ...
                                                if is_cached == 0:
                                                    ...
                                                    if validvalue == 0 or validvalue == 3:
                                                        ...
                                                        if snapshot_flag == 1 and is_case1 == 0:
                                                            ...
                                                            #            self.sess_hdl, self.dev_tgt, matchspec0)
                                                            # Update PUTREQ_INSWITCH as PUTRES to client by mirroring
                                                            actnspec0 = distfarreachleaf_update_putreq_inswitch_to_putres_by_mirroring_action_spec_t(tmp_client_sid, server_worker_port_start)
                                                            self.client.eg_port_forward_tbl_table_add_with_update_putreq_inswitch_to_putres_by_mirroring(\
                                                                    self.sess_hdl, self.dev_tgt, matchspec0, actnspec0)
                                                ...
                                                if is_cached == 0:
                                                    ...
                                                    if validvalue == 0 or validvalue == 3:
                                                        ...
                                                        if snapshot_flag == 1 and is_case1 == 0:
                                                            ...
                                                            #            self.sess_hdl, self.dev_tgt, matchspec0)
                                                            # Update DELREQ_INSWITCH as DELRES to client by mirroring
                                                            actnspec0 = distfarreachleaf_update_delreq_inswitch_to_delres_by_mirroring_action_spec_t(tmp_client_sid, server_worker_port_start)
                                                            self.client.eg_port_forward_tbl_table_add_with_update_delreq_inswitch_to_delres_by_mirroring(\
                                                                    self.sess_hdl, self.dev_tgt, matchspec0, actnspec0)
