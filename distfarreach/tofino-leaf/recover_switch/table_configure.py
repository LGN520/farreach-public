...
class RegisterUpdate(pd_base_tests.ThriftInterfaceDataPlane):
    ...
    def runTest(self):
        ...
        snapshotid_path = "/tmp/distfarreachleaf/controller.snapshotid"
        with open(snapshotid_path, "rb") as f:
            buf = f.read(4)
            snapshotid = struct.unpack("=i", buf)[0]

        # extract snapshot data
        snapshotdata_path = "/tmp/distfarreachleaf/controller.snapshotdata{}".format(snapshotid)
        key_list_list = [] # n*5
        vallen_list = [] # n
        ...
        for idx in range(n):
            ...
            actnspec0 = distfarreachleaf_cached_action_action_spec_t(idx)
