...
from distfarreachlimitspine.p4_pd_rpc.ttypes import *
...
class TableConfigure(pd_base_tests.ThriftInterfaceDataPlane):
    def __init__(self):
        ...
        pd_base_tests.ThriftInterfaceDataPlane.__init__(self, ["distfarreachlimitspine"])
...
#                    matchspec0 = eval("distfarreachlimitspine_update_val{}_tbl_match_spec_t".format(valname))(
    ...
    def runTest(self):
        if test_param_get('cleanup') != True:
            ...
            for i in range(client_physical_num):
                matchspec0 = distfarreachlimitspine_l2l3_forward_tbl_match_spec_t(\
                ...
                actnspec0 = distfarreachlimitspine_l2l3_forward_action_spec_t(self.clientleafswitch_devport)
            ...
            for i in range(server_physical_num):
                matchspec0 = distfarreachlimitspine_l2l3_forward_tbl_match_spec_t(\
                        ...
                        ipv4_hdr_dstAddr_prefix_length = 32)
                actnspec0 = distfarreachlimitspine_l2l3_forward_action_spec_t(self.serverleafswitch_devport)
                self.client.l2l3_forward_tbl_table_add_with_l2l3_forward(\
                        self.sess_hdl, self.dev_tgt, matchspec0, actnspec0)
            ...
            actnspec0 = distfarreachlimitspine_set_hot_threshold_action_spec_t(hot_threshold)
            ...
            for tmpoptype in [PUTREQ, DELREQ, GETRES_LATEST_SEQ, GETRES_DELETED_SEQ, PUTREQ_SEQ, DELREQ_SEQ, GETRES_LATEST_SEQ_SERVER, GETRES_DELETED_SEQ_SERVER]:
                matchspec0 = distfarreachlimitspine_recirculate_tbl_match_spec_t(\
                        op_hdr_optype = tmpoptype,
                        meta_need_recirculate = 1)
                # recirculate to the pipeline of the first physical client for atomicity of setting snapshot flag
                actnspec0 = distfarreachlimitspine_recirculate_pkt_action_spec_t(convert_u8_to_i8(self.recirPorts[leafswitch_pipeidx]))
            ...
            if RANGE_SUPPORT == False:
                # Table: hash_for_partition_tbl (default: nop; size: 16)
                print "Configuring hash_for_partition_tbl"
                for tmpoptype in [GETREQ, CACHE_POP_INSWITCH, PUTREQ, DELREQ, WARMUPREQ, LOADREQ, CACHE_EVICT_LOADFREQ_INSWITCH, CACHE_EVICT_LOADDATA_INSWITCH, LOADSNAPSHOTDATA_INSWITCH, SETVALID_INSWITCH, PUTREQ_SEQ, DELREQ_SEQ, GETRES_LATEST_SEQ_SERVER, GETRES_DELETED_SEQ_SERVER, GETRES_LATEST_SEQ, GETRES_DELETED_SEQ]:
                    matchspec0 = distfarreachlimitspine_hash_for_partition_tbl_match_spec_t(\
            ...
            if RANGE_SUPPORT == True:
                # Table: range_partition_tbl (default: nop; size <= 17 * 128)
                print "Configuring range_partition_tbl"
                key_range_per_leafswitch = pow(2, 16) / leafswitch_total_logical_num
                for tmpoptype in [GETREQ, CACHE_POP_INSWITCH, PUTREQ, DELREQ, WARMUPREQ, SCANREQ, LOADREQ, CACHE_EVICT_LOADFREQ_INSWITCH, CACHE_EVICT_LOADDATA_INSWITCH, LOADSNAPSHOTDATA_INSWITCH, SETVALID_INSWITCH, PUTREQ_SEQ, DELREQ_SEQ, GETRES_LATEST_SEQ_SERVER, GETRES_DELETED_SEQ_SERVER, GETRES_LATEST_SEQ, GETRES_DELETED_SEQ]:
                    ...
                    for i in range(leafswitch_total_logical_num):
                        ...
                        matchspec0 = distfarreachlimitspine_range_partition_tbl_match_spec_t(\
                                op_hdr_optype = tmpoptype,
                        ...
                        if tmpoptype == GETRES_LATEST_SEQ_SERVER or tmpoptype == GETRES_DELETED_SEQ_SERVER:
                            ...
                            actnspec0 = distfarreachlimitspine_range_partition_for_special_response_action_spec_t(eport)
                            ...
                            actnspec0 = distfarreachlimitspine_range_partition_action_spec_t(eport, global_leafswitch_logical_idx)
                            self.client.range_partition_tbl_table_add_with_range_partition(\
                                    self.sess_hdl, self.dev_tgt, matchspec0, 0, actnspec0) # 0 is priority (range may be overlapping)
                        key_start = key_end + 1
            else:
                # Table: hash_partition_tbl (default: nop; size <= 16 * 128)
                ...
                for tmpoptype in [GETREQ, CACHE_POP_INSWITCH, PUTREQ, DELREQ, WARMUPREQ, LOADREQ, CACHE_EVICT_LOADFREQ_INSWITCH, CACHE_EVICT_LOADDATA_INSWITCH, LOADSNAPSHOTDATA_INSWITCH, SETVALID_INSWITCH, PUTREQ_SEQ, DELREQ_SEQ, GETRES_LATEST_SEQ_SERVER, GETRES_DELETED_SEQ_SERVER, GETRES_LATEST_SEQ, GETRES_DELETED_SEQ]:
                    ...
                    for i in range(leafswitch_total_logical_num):
                        ...
                        # NOTE: both start and end are included
                        matchspec0 = distfarreachlimitspine_hash_partition_tbl_match_spec_t(\
                                op_hdr_optype = convert_u16_to_i16(tmpoptype),
                        ...
                        if tmpoptype == GETRES_LATEST_SEQ_SERVER or tmpoptype == GETRES_DELETED_SEQ_SERVER:
                            # NOTE: we do NOT change switchidx which is leafswitchidx set by server
                            actnspec0 = distfarreachlimitspine_hash_partition_for_special_response_action_spec_t(eport)
                            ...
                            actnspec0 = distfarreachlimitspine_hash_partition_action_spec_t(eport, global_leafswitch_logical_idx)
            ...
            if RANGE_SUPPORT == True:
                ...
                for i in range(leafswitch_total_logical_num):
                    ...
                    # NOTE: both start and end are included
                    matchspec0 = distfarreachlimitspine_range_partition_for_scan_endkey_tbl_match_spec_t(\
                    ...
                    actnspec0 = distfarreachlimitspine_range_partition_for_scan_endkey_action_spec_t(end_globalswitchidx_plus_one)
            ...
            for tmpoptype in [PUTREQ, DELREQ]:
                matchspec0 = distfarreachlimitspine_hash_for_seq_tbl_match_spec_t(\
                        op_hdr_optype = tmpoptype,
                        meta_need_recirculate = 0)
            ...
            for client_physical_idx in range(client_physical_num):
                ...
                for tmpoptype in [GETREQ, PUTREQ, DELREQ]:
                    matchspec0 = distfarreachlimitspine_prepare_for_cachehit_tbl_match_spec_t(\
                            op_hdr_optype = tmpoptype,
                            ...
                            meta_need_recirculate = 0)
                    actnspec0 = distfarreachlimitspine_set_client_sid_action_spec_t(self.clientleafswitch_sid)
            ...
            for tmp_client_physical_idx in range(client_physical_num):
                ...
                for tmpoptype in [GETRES, PUTRES, DELRES, WARMUPACK, SCANRES_SPLIT, LOADACK]:
                    matchspec0 = distfarreachlimitspine_ipv4_forward_tbl_match_spec_t(\
                            op_hdr_optype = convert_u16_to_i16(tmpoptype),
                            ipv4_hdr_dstAddr = ipv4addr0,
                            ipv4_hdr_dstAddr_prefix_length = 32,
                            meta_need_recirculate = 0)
                    actnspec0 = distfarreachlimitspine_forward_normal_response_action_spec_t(eport)
                ...
                for tmpoptype in [GETRES_LATEST_SEQ, GETRES_DELETED_SEQ]:
                    matchspec0 = distfarreachlimitspine_ipv4_forward_tbl_match_spec_t(\
                            ...
                            ipv4_hdr_dstAddr_prefix_length = 32,
                            meta_need_recirculate = 0)
                    actnspec0 = distfarreachlimitspine_forward_special_get_response_action_spec_t(tmpsid)
            ...
            for tmpoptype in [GETREQ, PUTREQ]:
                matchspec0 = distfarreachlimitspine_sample_tbl_match_spec_t(\
                        op_hdr_optype = tmpoptype,
                        meta_need_recirculate = 0)
            ...
            # Table: ig_port_forward_tbl (default: nop; size: 12)
            print "Configuring ig_port_forward_tbl"
            matchspec0 = distfarreachlimitspine_ig_port_forward_tbl_match_spec_t(\
            ...
            if RANGE_SUPPORT:
                ...
                for is_clone in [0, 1]:
                    for i in range(leafswitch_total_logical_num):
                        global_leafswitch_logical_idx = leafswitch_logical_idxes[i]
                        matchspec0 = distfarreachlimitspine_process_scanreq_split_tbl_match_spec_t(\
                        ...
                        if is_clone == 0:
                            ...
                            actnspec0 = distfarreachlimitspine_process_scanreq_split_action_spec_t(tmp_serverleaf_sid)
                            ...
                            tmp_serverleaf_sid = self.serverleafswitch_sid
                            actnspec0 = distfarreachlimitspine_process_cloned_scanreq_split_action_spec_t(tmp_serverleaf_sid)
            ...
            for tmpoptype in [GETREQ_INSWITCH, PUTREQ_INSWITCH]:
                matchspec0 = distfarreachlimitspine_access_cache_frequency_tbl_match_spec_t(\
                        op_hdr_optype = tmpoptype,
            ...
            for tmpoptype in [GETREQ_INSWITCH, GETRES_LATEST_SEQ_INSWITCH, GETRES_DELETED_SEQ_INSWITCH, PUTREQ_INSWITCH, DELREQ_INSWITCH]:
                matchspec0 = distfarreachlimitspine_access_validvalue_tbl_match_spec_t(\
                        op_hdr_optype = tmpoptype,
                        inswitch_hdr_is_cached = 1)
            ...
            for tmpoptype in [PUTREQ_INSWITCH, DELREQ_INSWITCH]:
                matchspec0 = distfarreachlimitspine_access_seq_tbl_match_spec_t(\
                        op_hdr_optype = tmpoptype)
            ...
            for tmpoptype in[GETREQ_INSWITCH, PUTREQ_INSWITCH, DELREQ_INSWITCH]:
                matchspec0 = distfarreachlimitspine_save_client_info_tbl_match_spec_t(\
                        op_hdr_optype = tmpoptype)
            ...
            for is_cached in cached_list:
                for validvalue in validvalue_list:
                    matchspec0 = distfarreachlimitspine_access_latest_tbl_match_spec_t(\
                            op_hdr_optype = GETREQ_INSWITCH,
                            inswitch_hdr_is_cached = is_cached,
                            validvalue_hdr_validvalue = validvalue)
                    if is_cached == 1:
                ...
                for validvalue in validvalue_list:
                    for is_latest in latest_list:
                        for is_stat in stat_list:
                            matchspec0 = distfarreachlimitspine_access_deleted_tbl_match_spec_t(\
                                    op_hdr_optype = GETREQ_INSWITCH,
                                    inswitch_hdr_is_cached = is_cached,
                                    validvalue_hdr_validvalue = validvalue,
                                    meta_is_latest = is_latest,
            ...
            for is_cached in cached_list:
                for validvalue in validvalue_list:
                    for is_latest in latest_list:
                        matchspec0 = distfarreachlimitspine_update_vallen_tbl_match_spec_t(\
                                op_hdr_optype = GETREQ_INSWITCH,
                                inswitch_hdr_is_cached = is_cached,
                                validvalue_hdr_validvalue = validvalue,
                                meta_is_latest = is_latest)
            ...
            for is_cached in cached_list:
                for validvalue in validvalue_list:
                    for is_latest in latest_list:
                        for tmpoptype in [PUTREQ_INSWITCH, DELREQ_INSWITCH]:
                            matchspec0 = distfarreachlimitspine_access_savedseq_tbl_match_spec_t(\
                                    op_hdr_optype = tmpoptype,
                                    inswitch_hdr_is_cached = is_cached,
                                    validvalue_hdr_validvalue = validvalue,
                                    meta_is_latest = is_latest)
            ...
            for is_latest in latest_list:
                for tmpoptype in [GETRES_LATEST_SEQ_INSWITCH, GETRES_DELETED_SEQ_INSWITCH]:
                    matchspec0 = distfarreachlimitspine_access_case1_tbl_match_spec_t(\
                            op_hdr_optype = tmpoptype,
                            inswitch_hdr_is_cached = 1,
            ...
            if RANGE_SUPPORT == False:
                ...
                for tmpoptype in [GETRES_LATEST_SEQ_INSWITCH_CASE1, GETRES_DELETED_SEQ_INSWITCH_CASE1, PUTREQ_SEQ_INSWITCH_CASE1, DELREQ_SEQ_INSWITCH_CASE1]:
                    matchspec0 = distfarreachlimitspine_lastclone_lastscansplit_tbl_match_spec_t(\
                            op_hdr_optype = tmpoptype,
            ...
            for i in range(switch_max_vallen/8 + 1): # i from 0 to 16
                ...
                matchspec0 = distfarreachlimitspine_update_pktlen_tbl_match_spec_t(\
                        op_hdr_optype=GETRES,
                        vallen_hdr_vallen_start=vallen_start,
                        vallen_hdr_vallen_end=vallen_end) # [vallen_start, vallen_end]
                actnspec0 = distfarreachlimitspine_update_pktlen_action_spec_t(val_stat_udplen, val_stat_iplen)
                ...
                for tmpoptype in [GETRES_LATEST_SEQ_INSWITCH_CASE1, GETRES_DELETED_SEQ_INSWITCH_CASE1, PUTREQ_SEQ_INSWITCH_CASE1, DELREQ_SEQ_INSWITCH_CASE1]:
                    matchspec0 = distfarreachlimitspine_update_pktlen_tbl_match_spec_t(\
                            ...
                            vallen_hdr_vallen_start=vallen_start,
                            vallen_hdr_vallen_end=vallen_end) # [vallen_start, vallen_end]
                    actnspec0 = distfarreachlimitspine_update_pktlen_action_spec_t(val_seq_inswitch_stat_clone_udplen, val_seq_inswitch_stat_clone_iplen)
                    self.client.update_pktlen_tbl_table_add_with_update_pktlen(\
                            self.sess_hdl, self.dev_tgt, matchspec0, 0, actnspec0) # 0 is priority (range may be overlapping)
                ...
                for tmpoptype in [PUTREQ_SEQ]:
                    matchspec0 = distfarreachlimitspine_update_pktlen_tbl_match_spec_t(\
                            ...
                            vallen_hdr_vallen_start=vallen_start,
                            vallen_hdr_vallen_end=vallen_end) # [vallen_start, vallen_end]
                    actnspec0 = distfarreachlimitspine_update_pktlen_action_spec_t(val_seq_udplen, val_seq_iplen)
                    self.client.update_pktlen_tbl_table_add_with_update_pktlen(\
                            self.sess_hdl, self.dev_tgt, matchspec0, 0, actnspec0) # 0 is priority (range may be overlapping)
                matchspec0 = distfarreachlimitspine_update_pktlen_tbl_match_spec_t(\
                        ...
                        vallen_hdr_vallen_start=vallen_start,
                        vallen_hdr_vallen_end=vallen_end) # [vallen_start, vallen_end]
                actnspec0 = distfarreachlimitspine_update_pktlen_action_spec_t(val_seq_stat_udplen, val_seq_stat_iplen)
                self.client.update_pktlen_tbl_table_add_with_update_pktlen(\
                        self.sess_hdl, self.dev_tgt, matchspec0, 0, actnspec0) # 0 is priority (range may be overlapping)
                matchspec0 = distfarreachlimitspine_update_pktlen_tbl_match_spec_t(\
                        ...
                        vallen_hdr_vallen_start=vallen_start,
                        vallen_hdr_vallen_end=vallen_end) # [vallen_start, vallen_end]
                actnspec0 = distfarreachlimitspine_update_pktlen_action_spec_t(val_seq_inswitch_stat_udplen, val_seq_inswitch_stat_iplen)
                self.client.update_pktlen_tbl_table_add_with_update_pktlen(\
                        self.sess_hdl, self.dev_tgt, matchspec0, 0, actnspec0) # 0 is priority (range may be overlapping)
            onlyop_udplen = 28
            onlyop_iplen = 48
            ...
            actnspec0 = distfarreachlimitspine_update_pktlen_action_spec_t(onlyop_udplen, onlyop_iplen)
            self.client.update_pktlen_tbl_table_add_with_update_pktlen(\
                    self.sess_hdl, self.dev_tgt, matchspec0, 0, actnspec0) # 0 is priority (range may be overlapping)
            for tmpoptype in [PUTRES, DELRES]:
                matchspec0 = distfarreachlimitspine_update_pktlen_tbl_match_spec_t(\
                        ...
                        vallen_hdr_vallen_start=0,
                        vallen_hdr_vallen_end=switch_max_vallen) # [0, 128]
                actnspec0 = distfarreachlimitspine_update_pktlen_action_spec_t(stat_udplen, stat_iplen)
                self.client.update_pktlen_tbl_table_add_with_update_pktlen(\
                        self.sess_hdl, self.dev_tgt, matchspec0, 0, actnspec0) # 0 is priority (range may be overlapping)
            for tmpoptype in [DELREQ_SEQ]:
                matchspec0 = distfarreachlimitspine_update_pktlen_tbl_match_spec_t(\
                        ...
                        vallen_hdr_vallen_start=0,
                        vallen_hdr_vallen_end=switch_max_vallen) # [0, 128]
                actnspec0 = distfarreachlimitspine_update_pktlen_action_spec_t(seq_udplen, seq_iplen)
                self.client.update_pktlen_tbl_table_add_with_update_pktlen(\
                        self.sess_hdl, self.dev_tgt, matchspec0, 0, actnspec0) # 0 is priority (range may be overlapping)
            matchspec0 = distfarreachlimitspine_update_pktlen_tbl_match_spec_t(\
                    op_hdr_optype=SCANREQ_SPLIT,
                    vallen_hdr_vallen_start=0,
                    vallen_hdr_vallen_end=switch_max_vallen) # [0, 128]
            actnspec0 = distfarreachlimitspine_update_pktlen_action_spec_t(scanreqsplit_udplen, scanreqsplit_iplen)
            self.client.update_pktlen_tbl_table_add_with_update_pktlen(\
                    self.sess_hdl, self.dev_tgt, matchspec0, 0, actnspec0) # 0 is priority (range may be overlapping)
            matchspec0 = distfarreachlimitspine_update_pktlen_tbl_match_spec_t(\
                    ...
                    vallen_hdr_vallen_start=0,
                    vallen_hdr_vallen_end=switch_max_vallen) # [0, 128]
            actnspec0 = distfarreachlimitspine_update_pktlen_action_spec_t(frequency_udplen, frequency_iplen)
            self.client.update_pktlen_tbl_table_add_with_update_pktlen(\
                    self.sess_hdl, self.dev_tgt, matchspec0, 0, actnspec0) # 0 is priority (range may be overlapping)
            matchspec0 = distfarreachlimitspine_update_pktlen_tbl_match_spec_t(\
            ...
            actnspec0 = distfarreachlimitspine_update_srcipmac_srcport_server2client_action_spec_t(\
            ...
            for tmpoptype in [GETRES, PUTRES, DELRES, SCANRES_SPLIT, WARMUPACK, LOADACK]:
                matchspec0 = distfarreachlimitspine_update_ipmac_srcport_tbl_match_spec_t(\
                        op_hdr_optype = convert_u16_to_i16(tmpoptype), 
            ...
            actnspec2 = distfarreachlimitspine_update_ipmac_srcport_switch2switchos_action_spec_t(\
            ...
            for tmpoptype in [PUTREQ_SEQ, PUTREQ_POP_SEQ, GETRES_LATEST_SEQ_INSWITCH_CASE1, GETRES_DELETED_SEQ_INSWITCH_CASE1, PUTREQ_SEQ_INSWITCH_CASE1, DELREQ_SEQ_INSWITCH_CASE1, GETRES, LOADREQ_SPINE, CACHE_EVICT_LOADDATA_INSWITCH_ACK, LOADSNAPSHOTDATA_INSWITCH_ACK, PUTREQ_SEQ_INSWITCH, GETRES_LATEST_SEQ_SERVER_INSWITCH, GETRES_DELETED_SEQ_SERVER_INSWITCH]:
                for i in range(switch_max_vallen/8 + 1): # i from 0 to 16
                    if i == 0:
                        vallen_start = 0
                        vallen_end = 0
                        ...
                        vallen_start = (i-1)*8+1 # 1, 9, ..., 121
                        vallen_end = (i-1)*8+8 # 8, 16, ..., 128
                    matchspec0 = distfarreachlimitspine_add_and_remove_value_header_tbl_match_spec_t(\
                            op_hdr_optype=tmpoptype,
                            vallen_hdr_vallen_start=vallen_start,
                            vallen_hdr_vallen_end=vallen_end) # [vallen_start, vallen_end]
                    if i == 0:
            ...
            matchspec0 = distfarreachlimitspine_drop_tbl_match_spec_t(\
                    op_hdr_optype = GETRES_LATEST_SEQ_INSWITCH)
                                ...
                                for snapshot_flag in snapshot_flag_list:
                                    for is_case1 in case1_list:
                                        ...
                                        if is_lastclone_for_pktloss == 0 and snapshot_flag == 0 and is_case1 == 0 and tmp_client_sid != 0:
                                            ...
                                            matchspec0 = distfarreachlimitspine_eg_port_forward_tbl_match_spec_t(\
                                                op_hdr_optype = GETREQ_INSWITCH,
                                                inswitch_hdr_is_cached = is_cached,
                                                validvalue_hdr_validvalue = validvalue,
                                                meta_is_latest = is_latest,
                                            ...
                                            if is_cached == 0:
                                                ...
                                                #actnspec0 = distfarreachlimitspine_update_getreq_inswitch_to_getreq_action_spec_t(self.devPorts[1])
                                                ...
                                                if validvalue == 0:
                                                    ...
                                                    if is_latest == 0:
                                                        ...
                                                        #actnspec0 = distfarreachlimitspine_update_getreq_inswitch_to_getreq_nlatest_action_spec_t(self.devPorts[1])
                                                        ...
                                                        actnspec0 = distfarreachlimitspine_update_getreq_inswitch_to_getres_by_mirroring_action_spec_t(tmp_client_sid, server_worker_port_start, tmpstat)
                                        ...
                                        if tmp_client_sid == 0 and is_lastclone_for_pktloss == 0:
                                            ...
                                            if is_cached == 1 and validvalue == 1 and is_latest == 0 and snapshot_flag == 1 and is_case1 == 0:
                                                ...
                                                if is_deleted == 0: # is_deleted=0 -> stat=1
                                                    #actnspec0 = distfarreachlimitspine_update_getres_latest_seq_inswitch_to_getres_latest_seq_inswitch_case1_clone_for_pktloss_action_spec_t(self.reflector_sid, self.devPorts[1], 1)
                                                    actnspec0 = distfarreachlimitspine_update_getres_latest_seq_inswitch_to_getres_latest_seq_inswitch_case1_clone_for_pktloss_action_spec_t(self.reflector_sid, 1, reflector_dp2cpserver_port)
                                                elif is_deleted == 1: # is_deleted=1 -> stat=0
                                                    #actnspec0 = distfarreachlimitspine_update_getres_latest_seq_inswitch_to_getres_latest_seq_inswitch_case1_clone_for_pktloss_action_spec_t(self.reflector_sid, self.devPorts[1], 0)
                                                    actnspec0 = distfarreachlimitspine_update_getres_latest_seq_inswitch_to_getres_latest_seq_inswitch_case1_clone_for_pktloss_action_spec_t(self.reflector_sid, 0, reflector_dp2cpserver_port)
                                        ...
                                        if is_cached == 1 and snapshot_flag == 1 and validvalue == 0 and is_latest == 0 and is_deleted == 0 and tmp_client_sid == 0 and is_case1 == 0:
                                            ...
                                            if is_lastclone_for_pktloss == 0:
                                                ...
                                                actnspec0 = distfarreachlimitspine_forward_getres_latest_seq_inswitch_case1_clone_for_pktloss_action_spec_t(self.reflector_sid)
                                        ...
                                        if tmp_client_sid == 0 and is_lastclone_for_pktloss == 0:
                                            ...
                                            if is_cached == 1 and validvalue == 1 and is_latest == 0 and snapshot_flag == 1 and is_case1 == 0:
                                                ...
                                                if is_deleted == 0: # is_deleted=0 -> stat=1
                                                    #actnspec0 = distfarreachlimitspine_update_getres_deleted_seq_inswitch_to_getres_deleted_seq_inswitch_case1_clone_for_pktloss_action_spec_t(self.reflector_sid, self.devPorts[1], 1)
                                                    actnspec0 = distfarreachlimitspine_update_getres_deleted_seq_inswitch_to_getres_deleted_seq_inswitch_case1_clone_for_pktloss_action_spec_t(self.reflector_sid, 1, reflector_dp2cpserver_port)
                                                elif is_deleted == 1: # is_deleted=1 -> stat=0
                                                    #actnspec0 = distfarreachlimitspine_update_getres_deleted_seq_inswitch_to_getres_deleted_seq_inswitch_case1_clone_for_pktloss_action_spec_t(self.reflector_sid, self.devPorts[1], 0)
                                                    actnspec0 = distfarreachlimitspine_update_getres_deleted_seq_inswitch_to_getres_deleted_seq_inswitch_case1_clone_for_pktloss_action_spec_t(self.reflector_sid, 0, reflector_dp2cpserver_port)
                                        ...
                                        if is_cached == 1 and snapshot_flag == 1 and validvalue == 0 and is_latest == 0 and is_deleted == 0 and tmp_client_sid == 0 and is_case1 == 0:
                                            ...
                                            if is_lastclone_for_pktloss == 0:
                                                ...
                                                actnspec0 = distfarreachlimitspine_forward_getres_deleted_seq_inswitch_case1_clone_for_pktloss_action_spec_t(self.reflector_sid)
                                        ...
                                        if is_cached == 0 and validvalue == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0 and snapshot_flag == 0 and is_case1 == 0:
                                            ...
                                            actnspec0 = distfarreachlimitspine_update_cache_pop_inswitch_to_cache_pop_inswitch_ack_drop_and_clone_action_spec_t(self.reflector_sid, reflector_dp2cpserver_port)
                                        ...
                                        if is_cached == 0 and validvalue == 0 and is_latest == 0 and is_deleted == 0 and tmp_client_sid == 0 and snapshot_flag == 0 and is_case1 == 0 and is_lastclone_for_pktloss == 0:
                                            ...
                                            #    actnspec0 = distfarreachlimitspine_forward_cache_pop_inswitch_ack_clone_for_pktloss_action_spec_t(self.reflector_sid)
                                            ...
                                            if is_cached == 0:
                                                ...
                                                if validvalue == 0 or validvalue == 3:
                                                    ...
                                                    if snapshot_flag == 1 and is_case1 == 0:
                                                        ...
                                                        if is_deleted == 0: # is_deleted=0 -> stat=1
                                                            #actnspec0 = distfarreachlimitspine_update_putreq_inswitch_to_putreq_seq_inswitch_case1_clone_for_pktloss_and_putres_action_spec_t(self.reflector_sid, self.devPorts[1], 1)
                                                            actnspec0 = distfarreachlimitspine_update_putreq_inswitch_to_putreq_seq_inswitch_case1_clone_for_pktloss_and_putres_action_spec_t(self.reflector_sid, 1, reflector_dp2cpserver_port)
                                                        elif is_deleted == 1: # is_deleted=1 -> stat=0
                                                            #actnspec0 = distfarreachlimitspine_update_putreq_inswitch_to_putreq_seq_inswitch_case1_clone_for_pktloss_and_putres_action_spec_t(self.reflector_sid, self.devPorts[1], 0)
                                                            actnspec0 = distfarreachlimitspine_update_putreq_inswitch_to_putreq_seq_inswitch_case1_clone_for_pktloss_and_putres_action_spec_t(self.reflector_sid, 0, reflector_dp2cpserver_port)
                                                        ...
                                                        actnspec0 = distfarreachlimitspine_update_putreq_inswitch_to_putres_by_mirroring_action_spec_t(tmp_client_sid, server_worker_port_start)
                                        ...
                                        if is_cached == 1 and validvalue == 0 and is_latest == 0 and is_deleted == 0 and snapshot_flag == 1 and is_case1 == 0:
                                            ...
                                            if is_lastclone_for_pktloss == 0:
                                                ...
                                                actnspec0 = distfarreachlimitspine_forward_putreq_seq_inswitch_case1_clone_for_pktloss_and_putres_action_spec_t(self.reflector_sid)
                                                ...
                                                actnspec0 = distfarreachlimitspine_update_putreq_seq_inswitch_case1_to_putres_by_mirroring_action_spec_t(tmp_client_sid, server_worker_port_start)
                                            ...
                                            if is_cached == 0:
                                                ...
                                                if validvalue == 0 or validvalue == 3:
                                                    ...
                                                    if snapshot_flag == 1 and is_case1 == 0:
                                                        ...
                                                        if is_deleted == 0: # is_deleted=0 -> stat=1
                                                            #actnspec0 = distfarreachlimitspine_update_delreq_inswitch_to_delreq_seq_inswitch_case1_clone_for_pktloss_and_delres_action_spec_t(self.reflector_sid, self.devPorts[1], 1)
                                                            actnspec0 = distfarreachlimitspine_update_delreq_inswitch_to_delreq_seq_inswitch_case1_clone_for_pktloss_and_delres_action_spec_t(self.reflector_sid, 1, reflector_dp2cpserver_port)
                                                        elif is_deleted == 1: # is_deleted=1 -> stat=0
                                                            #actnspec0 = distfarreachlimitspine_update_delreq_inswitch_to_delreq_seq_inswitch_case1_clone_for_pktloss_and_delres_action_spec_t(self.reflector_sid, self.devPorts[1], 0)
                                                            actnspec0 = distfarreachlimitspine_update_delreq_inswitch_to_delreq_seq_inswitch_case1_clone_for_pktloss_and_delres_action_spec_t(self.reflector_sid, 0, reflector_dp2cpserver_port)
                                                        ...
                                                        actnspec0 = distfarreachlimitspine_update_delreq_inswitch_to_delres_by_mirroring_action_spec_t(tmp_client_sid, server_worker_port_start)
                                        ...
                                        if is_cached == 1 and validvalue == 0 and is_latest == 0 and is_deleted == 0 and snapshot_flag == 1 and is_case1 == 0:
                                            ...
                                            if is_lastclone_for_pktloss == 0:
                                                ...
                                                actnspec0 = distfarreachlimitspine_forward_delreq_seq_inswitch_case1_clone_for_pktloss_and_delres_action_spec_t(self.reflector_sid)
                                                ...
                                                actnspec0 = distfarreachlimitspine_update_delreq_seq_inswitch_case1_to_delres_by_mirroring_action_spec_t(tmp_client_sid, server_worker_port_start)
                                        ...
                                        if is_cached == 1 and validvalue == 0 and is_latest == 0 and is_deleted == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0 and snapshot_flag == 0 and is_case1 == 0:
                                            ...
                                            actnspec0 = distfarreachlimitspine_update_cache_evict_loadfreq_inswitch_to_cache_evict_loadfreq_inswitch_ack_drop_and_clone_action_spec_t(self.reflector_sid, reflector_dp2cpserver_port)
                                        ...
                                        if is_cached == 1 and validvalue == 0 and is_latest == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0 and snapshot_flag == 0 and is_case1 == 0:
                                            ...
                                            actnspec0 = distfarreachlimitspine_update_cache_evict_loaddata_inswitch_to_cache_evict_loaddata_inswitch_ack_drop_and_clone_action_spec_t(self.reflector_sid, reflector_dp2cpserver_port, tmpstat)
                                        ...
                                        if validvalue == 0 and is_latest == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0 and snapshot_flag == 0 and is_case1 == 0:
                                            ...
                                            actnspec0 = distfarreachlimitspine_update_loadsnapshotdata_inswitch_to_loadsnapshotdata_inswitch_ack_drop_and_clone_action_spec_t(self.reflector_sid, reflector_dp2cpserver_port, tmpstat)
                                        ...
                                        if is_latest == 0 and is_deleted == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0 and snapshot_flag == 0 and is_case1 == 0:
                                            ...
                                            actnspec0 = distfarreachlimitspine_update_setvalid_inswitch_to_setvalid_inswitch_ack_drop_and_clone_action_spec_t(self.reflector_sid, reflector_dp2cpserver_port)
                                            ...
                                            for tmp_server_sid in tmp_server_sids:
                                                ...
                                                if is_cached == 0 and validvalue == 0 and is_latest == 0 and is_deleted == 0 and tmp_client_sid == 0 and is_lastclone_for_pktloss == 0 and snapshot_flag == 0 and is_case1 == 0 and tmp_server_sid != 0:
                                                    ...
                                                    if is_last_scansplit == 1:
                                                        ...
                                                        actnspec0 = distfarreachlimitspine_forward_scanreq_split_and_clone_action_spec_t(tmp_server_sid)
