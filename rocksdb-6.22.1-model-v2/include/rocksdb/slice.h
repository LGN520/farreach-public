...
class Slice {
 public:
  ...
  //NetBuffer
  void to_model_key(double* array, uint32_t array_len) const {
	...
	char *curptr = (char *)data_;
	for (uint32_t i = 0; i < key_len; i++) {
		if (i != key_len - 1 || remainder == 0) {
			array[i] = double(*((uint64_t*)(curptr)));
			curptr += 8;
		}
		else {
			char tmp[8];
			memset(tmp, 0, 8);
			memcpy(tmp, curptr, remainder); // emulate little-endian
			//memcpy(tmp + 8 - remainder, curptr, remainder);
			array[i] = double(*((uint64_t*)(tmp)));
			break;
		}
	}
  }
...
inline int Slice::compare(const Slice& b) const {
  ...
  // NetBuffer (sort based on unsigned integeer under little-endian)
  uint32_t keylen_a = get_key_len();
  uint32_t keylen_b = b.get_key_len();
  uint32_t max_keylen = (keylen_a > keylen_b) ? keylen_a : keylen_b;
  double model_key_a[max_keylen] = {0};
  double model_key_b[max_keylen] = {0};
  to_model_key(model_key_a, max_keylen);
  b.to_model_key(model_key_b, max_keylen);
  // From max_keylen-1 to 0 = from latter byte to previous byte = from most significant to least significant under little-endian
  int result = 0;
  for (int i = max_keylen - 1; i >= 0; i--) {
	if (model_key_a[i] > model_key_b[i]) {
		result = 1;
		break;
	}
	else if (model_key_a[i] < model_key_b[i]) {
		result = -1;
		break;
	}
  }
