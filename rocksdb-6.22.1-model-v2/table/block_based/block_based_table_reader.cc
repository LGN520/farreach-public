...
bool BlockBasedTable::PrefixMayMatch(
  ...
  if (filter != nullptr) {
    ...
    if (!filter->IsBlockBased()) {
      ...
      iiter->Seek(internal_prefix, fd_);
...
Status BlockBasedTable::Get(const ReadOptions& read_options, const Slice& key,
  ...
  if (!may_match) {
    ...
    bool done = false;
    for (iiter->Seek(key, fd_); iiter->Valid() && !done; iiter->Next()) {
      ...
      bool may_exist = biter.SeekForGet(key, fd_, iiter->index_);
...
Status BlockBasedTable::Prefetch(const Slice* const begin,
  ...
  for (begin ? iiter->Seek(*begin, fd_) : iiter->SeekToFirst(); iiter->Valid();
       iiter->Next()) {
