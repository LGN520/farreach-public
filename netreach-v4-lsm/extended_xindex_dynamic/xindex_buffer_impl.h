...
inline bool AltBtreeBuffer<key_t, val_t>::update(const key_t &key,
  ...
  int slot = leaf_ptr->find_first_larger_than_or_equal_to(key);
  bool res = (slot < leaf_ptr->key_n && leaf_ptr->keys[slot] == key)
                 ? leaf_ptr->vals[slot].update_ignoring_ptr(val, snapshot_id)
                 : false;
  // no version changed
  leaf_ptr->unlock();
  return res;
...
inline void AltBtreeBuffer<key_t, val_t>::insert(const key_t &key,
                                                 const val_t &val, int32_t snapshot_id) {
  leaf_t *leaf_ptr = locate_leaf_locked(key);
  insert_leaf(key, val, leaf_ptr, snapshot_id);  // lock is released within
}

template <class key_t, class val_t>
inline bool AltBtreeBuffer<key_t, val_t>::remove(const key_t &key, int32_t snapshot_id) {
  leaf_t *leaf_ptr = locate_leaf_locked(key);
  int slot = leaf_ptr->find_first_larger_than_or_equal_to(key);
  bool res = (slot < leaf_ptr->key_n && leaf_ptr->keys[slot] == key)
                 ? leaf_ptr->vals[slot].remove_ignoring_ptr(snapshot_id)
                 : false;
  // no version changed
  leaf_ptr->unlock();
  return res;
...
void AltBtreeBuffer<key_t, val_t>::insert_leaf(const key_t &key,
  ...
  if (slot < target->key_n && target->keys[slot] == key) {
    if (target->vals[slot].update_ignoring_ptr(val, snapshot_id)) {
      memory_fence();
      target->unlock();  // didn't insert anything
      return;
    } else {
      target->vals[slot] = atomic_val_t(val, snapshot_id);
  ...
  if (!target->is_full()) {
    ...
    split_n_insert_leaf(key, val, slot, target, snapshot_id);
    size_est++;
  }
}
...
void AltBtreeBuffer<key_t, val_t>::split_n_insert_leaf(const key_t &insert_key,
  ...
  if (slot >= mid) {  // insert to the new sibling
    ...
    ((leaf_t *)sib_ptr)->vals[slot - mid] = atomic_val_t(val, snapshot_id);
    ...
    ((leaf_t *)node_ptr)->vals[slot] = atomic_val_t(val, snapshot_id);
