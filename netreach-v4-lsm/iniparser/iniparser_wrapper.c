...
int IniparserWrapper::get_snapshot_data_ack() {
	int tmp = iniparser_getint(ini, "snapshot:snapshot_data_ack", -1);
	if (tmp == -1) {
		printf("Invalid entry of [snapshot:snapshot_data_ack]: %d\n", tmp);
		exit(-1);
	}
	return tmp;
}
