...
ssh ssy@dl15 "cd projects/NetBuffer/${DIRNAME}; bash stop_server.sh >/dev/null 2>&1"
echo "stop clients"
...
cp configs/config.ini.farreach-rotation-transaction1p config.tmp
sed '78s/server_logical_idxes=95/server_logical_idxes='${bottleneck_serveridx}'/g' config.tmp > config.ini
rm config.tmp
...
do
	...
	ssh ssy@dl15 "cd projects/NetBuffer/${DIRNAME}; bash stop_server.sh >/dev/null"
	echo "stop clients"
	bash stop_client.sh >/dev/null 2>&1
	...
	echo "prepare and sync config.ini"
	cp configs/config.ini.farreach-rotation-transaction2p config.tmp
