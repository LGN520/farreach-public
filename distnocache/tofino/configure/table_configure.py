...
from distnocache.p4_pd_rpc.ttypes import *
...
class TableConfigure(pd_base_tests.ThriftInterfaceDataPlane):
    def __init__(self):
        ...
        pd_base_tests.ThriftInterfaceDataPlane.__init__(self, ["distnocache"])
...
#            self.mirror.mirror_session_create(self.sess_hdl, self.dev_tgt, info)
            ...
            for i in range(client_physical_num):
                matchspec0 = distnocache_l2l3_forward_tbl_match_spec_t(\
                ...
                actnspec0 = distnocache_l2l3_forward_action_spec_t(self.client_devports[i])
            ...
            for i in range(server_physical_num):
                matchspec0 = distnocache_l2l3_forward_tbl_match_spec_t(\
                        ...
                        ipv4_hdr_dstAddr_prefix_length = 32)
                actnspec0 = distnocache_l2l3_forward_action_spec_t(self.server_devports[i])
                self.client.l2l3_forward_tbl_table_add_with_l2l3_forward(\
                        self.sess_hdl, self.dev_tgt, matchspec0, actnspec0)
            ...
            if RANGE_SUPPORT == False:
                ...
                for tmpoptype in [GETREQ, PUTREQ, DELREQ, LOADREQ]:
                    matchspec0 = distnocache_hash_for_partition_tbl_match_spec_t(\
            ...
            if RANGE_SUPPORT == True:
                ...
                for tmpoptype in [GETREQ, PUTREQ, DELREQ, SCANREQ, LOADREQ]:
                    ...
                    for global_server_logical_idx in range(server_total_logical_num):
                        ...
                        matchspec0 = distnocache_range_partition_tbl_match_spec_t(\
                        ...
                        if server_physical_idx == -1:
                            ...
                            if tmpoptype != SCANREQ:
                                actnspec0 = distnocache_range_partition_action_spec_t(udp_dstport, eport)
                                ...
                                actnspec0 = distnocache_range_partition_for_scan_action_spec_t(udp_dstport, eport, global_server_logical_idx)
                ...
                for tmpoptype in [GETREQ, PUTREQ, DELREQ, LOADREQ]:
                    ...
                    for global_server_logical_idx in range(server_total_logical_num):
                        ...
                        # NOTE: both start and end are included
                        matchspec0 = distnocache_hash_partition_tbl_match_spec_t(\
                                op_hdr_optype = convert_u16_to_i16(tmpoptype),
                        ...
                        if server_physical_idx == -1:
                            ...
                            udp_dstport = server_worker_port_start + local_server_logical_idx
                            eport = self.server_devports[server_physical_idx]
                            actnspec0 = distnocache_hash_partition_action_spec_t(udp_dstport, eport)
            ...
            if RANGE_SUPPORT == True:
                ...
                for global_server_logical_idx in range(server_total_logical_num):
                    ...
                    # NOTE: both start and end are included
                    matchspec0 = distnocache_range_partition_for_scan_endkey_tbl_match_spec_t(\
                    ...
                    #actnspec0 = distnocache_range_partition_for_scan_endkey_action_spec_t(last_udpport_plus_one)
                    end_globalserveridx_plus_one = global_server_logical_idx + 1 # used to calculate max_scannum in data plane
                    actnspec0 = distnocache_range_partition_for_scan_endkey_action_spec_t(end_globalserveridx_plus_one)
            ...
            for tmp_client_physical_idx in range(client_physical_num):
                ...
                for tmpoptype in [GETRES, PUTRES, DELRES, SCANRES_SPLIT, LOADACK]:
                    matchspec0 = distnocache_ipv4_forward_tbl_match_spec_t(\
                            op_hdr_optype = convert_u16_to_i16(tmpoptype),
                            ipv4_hdr_dstAddr = ipv4addr0,
                            ipv4_hdr_dstAddr_prefix_length = 32)
                    actnspec0 = distnocache_forward_normal_response_action_spec_t(eport)
            ...
            if RANGE_SUPPORT:
                ...
                matchspec0 = distnocache_ig_port_forward_tbl_match_spec_t(\
                        op_hdr_optype = SCANREQ)
            ...
            if RANGE_SUPPORT:
                ...
                for is_clone in [0, 1]:
                    for global_server_logical_idx in range(server_total_logical_num):
                        ...
                        matchspec0 = distnocache_process_scanreq_split_tbl_match_spec_t(\
                        ...
                        if is_clone == 0:
                            ...
                            if server_physical_idx == -1:
                                ...
                                actnspec0 = distnocache_process_scanreq_split_action_spec_t(tmp_server_sid)
                            ...
                            if current_local_server_logical_idx == -1 or next_server_physical_idx == -1:
                                ...
                                actnspec0 = distnocache_process_cloned_scanreq_split_action_spec_t(tmp_udpport, tmp_server_sid)
            ...
            if RANGE_SUPPORT == True:
                ...
                matchspec0 = distnocache_lastscansplit_tbl_match_spec_t(\
                        op_hdr_optype = SCANREQ_SPLIT,
            ...
            if RANGE_SUPPORT == True:
                ...
                matchspec0 = distnocache_update_pktlen_tbl_match_spec_t(\
                        op_hdr_optype=SCANREQ_SPLIT)
                actnspec0 = distnocache_update_pktlen_action_spec_t(scanreqsplit_udplen, scanreqsplit_iplen)
            ...
            for tmp_client_physical_idx in range(client_physical_num):
                ...
                actnspec0 = distnocache_update_ipmac_srcport_server2client_action_spec_t(\
                ...
                for tmpoptype in [GETRES, PUTRES, DELRES, SCANRES_SPLIT, LOADACK]:
                    matchspec0 = distnocache_update_ipmac_srcport_tbl_match_spec_t(\
                            op_hdr_optype = convert_u16_to_i16(tmpoptype), 
            ...
            for tmp_server_physical_idx in range(server_physical_num):
                ...
                actnspec1 = distnocache_update_dstipmac_client2server_action_spec_t(\
                        macAddr_to_string(tmp_server_mac), \
                        ipv4Addr_to_i32(tmp_server_ip))
    ...
    def configure_eg_port_forward_tbl_with_range(self):
        ...
        for is_last_scansplit in [0, 1]:
            for tmp_server_sid in tmp_server_sids:
                ...
                if tmp_server_sid != 0:
                    matchspec0 = distnocache_eg_port_forward_tbl_match_spec_t(\
                        op_hdr_optype = SCANREQ_SPLIT,
                    ...
                    if is_last_scansplit == 1:
                        ...
                        actnspec0 = distnocache_forward_scanreq_split_and_clone_action_spec_t(tmp_server_sid)
