...
control netcacheIngress(inout headers hdr,
	...
	action hash_for_bf2() {
		// modify_field_with_hash_based_offset(hdr.inswitch_hdr.hashval_for_bf2, 0, hash_calc2, BF_BUCKET_COUNT);
		hash(tmp, HashAlgorithm.crc32_custom, (bit<32>)0, {
			hdr.op_hdr.keylolo,
			hdr.op_hdr.keylohi,
			hdr.op_hdr.keyhilo,
	...
	/*action set_client_sid(client_sid, eport) {
		modify_field(hdr.inswitch_hdr.client_sid, client_sid);
		...
		//modify_field(hdr.inswitch_hdr.eport_for_res, ig_intr_md.ingress_port);
		modify_field(hdr.inswitch_hdr.eport_for_res, eport);
	}*/
