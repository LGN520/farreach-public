...
control netcacheEgress (inout headers hdr,
	...
	action set_is_report() {
		meta.meta.is_report = 1;
	}

	action reset_is_report() {
		meta.meta.is_report = 0;
	}

	@pragma stage 7
	table is_report_tbl {
		key = {
			meta.meta.is_report1: exact;
			meta.meta.is_report2: exact;
			meta.meta.is_report3: exact;
		}
		actions = {
			set_is_report;
			reset_is_report;
	...
	table eg_port_forward_tbl {
		key = {
			...
			meta.meta.is_report: exact; // only work for GETREQ_INSWITCH
	...
	action update_onlyop_pktlen() {
		...
		//hdr.udp_hdr.hdrlen = 27;
		//hdr.ipv4_hdr.totalLen = hdr.47;
		hdr.udp_hdr.hdrlen = 26;
		hdr.ipv4_hdr.totalLen = hdr.46;
	}
	...
	action update_val_stat_pktlen(aligned_vallen) {
		...
		//add(hdr.udp_hdr.hdrlen, aligned_vallen, 35);
		//add(hdr.ipv4_hdr.totalLen, aligned_vallen, 55);
		add(hdr.udp_hdr.hdrlen, aligned_vallen, 34);
		add(hdr.ipv4_hdr.totalLen, aligned_vallen, 54);
	}
	...
	action update_stat_pktlen() {
		...
		//hdr.udp_hdr.hdrlen = 33;
		//hdr.ipv4_hdr.totalLen = hdr.53;
		hdr.udp_hdr.hdrlen = 32;
		hdr.ipv4_hdr.totalLen = hdr.52;
	}
	...
	action update_scanreqsplit_pktlen() {
		...
		hdr.udp_hdr.hdrlen = hdr.49;
		hdr.ipv4_hdr.totalLen = hdr.69;
	}
	...
	action update_ophdr_inswitchhdr_clonehdr_pktlen() {
		...
		hdr.udp_hdr.hdrlen = hdr.64;
		hdr.ipv4_hdr.totalLen = hdr.84;
	}
