...
- [IMPORTANT] to get reasonable latency statistics under server rotation, we should fix the same aggregate throughput for all methods for fair comparions
	- Given the number of simulated servers, we can fix the aggregate throughput as the maximum throughput that all methods can achieve
	- -> e.g., in exp2, 0.8MOPS for 16 servers; 1.2MOPS for 32 servers; 1.6MOPS for 64 servers; and 2.0MOPS for 128 servers
...
- 10.28
	+ Siyuan
		* Survey and think about upstream backup (snapshot for consistency and backup for durability)
			...
			- TODO: SIGCOMM rollback-recovery for middleboxes -> output commit (may not necessary for us)
